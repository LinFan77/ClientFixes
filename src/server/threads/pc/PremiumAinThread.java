...
public class PremiumAinThread extends Thread {
				...
				for (L1PcInstance _client : L1World.getInstance().getAllPlayers()) {
					...
					if (_client == null || _client.getNetConnection() == null) {
						...
						try {
							...
							if (Config.Tam_Ok) {
								if (ttc >= tamt) {
									_client.settamtimecount(0); // Add 1 minute to 6 (minutes).
									...
									if (tamcount > 0) {
										...
										_client.sendPackets(new S_SystemMessage("Round of Growth: " + tamcount + " stage, Paid: Tam Points (" + addtam + ") points paid!"));
										try {
									...
									_client.settamtimecount(ttc + 1); // Add 1 minute to 6 (minutes).
								}
							}

							if (Config.Chopa_Event && Config.초파드랍진행중) {
								if (ctc >= 60) {
									_client.set쵸파카운트(0); // Add 1 minute to 6 (minutes).
									_client.getInventory().storeItem(50757, 1); // 드상
									_client.sendPackets(new S_SystemMessage("Chopper Event: Obtain [Chopper's Horn] as a 1-hour login reward!"), true);
								} else {
									_client.set쵸파카운트(ctc + 1); // Add + 1 minute to 6 (minutes).
								}
							}
							...
							if (Config.Dragon_14_12_12_Event && Config.드다1212드랍진행중) {
								if (dtc >= 60) {
									...
									_client.sendPackets(new S_SystemMessage("Sturdy Knight Event: 1시간 접속 보상 [드래곤의 다이아몬드] 획득."), true);
								} else {
							...
							if (Config.Dragon_3DAY_Event && Config.드다드랍진행중) {
								if (dtc >= 300) {
									_client.setdtimecount(0);// 6(분)에서 + 1분을더해준다.
									_client.getInventory().storeItem(437010, 1); // 드상
							...
							// feather payment note

							if (Config.할로윈_Event && Config.할로윈진행중) {
								if (_client.getInventory().checkEquipped(21123)) { // Halloween Festival Hat 2012
									할로윈축제(_client);
								}
							}
								...
								for (L1DollInstance doll : _client.getDollList()) {
									...
									if (doll.getDollType() == L1DollInstance.DOLLTYPE_HW_HUSUABI) {
										if (_client.마법인형_할로윈허수아_Count >= 60) {
											...
											_client.sendPackets(new S_SystemMessage("Halloween Scarecrow Magic Doll Summon Reward: Obtain Baru's Gift Box."));
									...
									if (doll.getDollType() == L1DollInstance.DOLLTYPE_GREMLIN) {
										if (_client.마법인형_그렘린_Count >= 15) {
											_client.getInventory().storeItem(9057, 1); // 할로윈 호박씨 지급
											_client.sendPackets(new S_SystemMessage("Red Cockatrice Magic Doll Summon Reward: Obtain a Red Cockatrice Gift Box."));
											// _client.sendPackets(new
											// S_ServerMessage(403,
											// "그렘린의 선물 상자"), true);
							...
							if (_client.isDead()) {
								if (deadtime >= 5) {
									...
									_client.setDeadTimeCount(deadtime + 1); // Add +1 minute to 6 (minutes).
								}
							} else {
							...
							_log.warning("Primeum distribution failure.");
					...
					if (_client.PCROOM_BUFF) {
						if (_client.getNetConnection().getAccount().getBuff_PC방() != null) {
							if (sysTime <= _client.getNetConnection().getAccount().getBuff_PC방().getTime()) {
								...
								_client.PCROOM_BUFF = false;
								_client.PC방_버프삭제중 = true;
								_client.sendPackets(new S_NewSkillIcons(L1SkillId.PC_ROOM, false, -1));
								...
								_client.sendPackets(new S_ACTION_UI(S_ACTION_UI.ATTENDANCE_COMPLETE, _client.getAccount(), _client.PCROOM_BUFF));
							}
						}
					}
	...
	private void giveFeather(L1PcInstance pc) {
		...
		if (pc.isPrivateShop() || (pc instanceof L1RobotInstance)) {
			return;
		}
...
//			pc.getInventory().storeItem(600272, 8);
	...
	private void DragonkeyTimeCheck(L1PcInstance pc, int count) {
		...
		if (count == 1) {
			...
			for (L1ItemInstance element : itemList) {
				if (nowtime > element.getEndTime().getTime())
					pc.getInventory().removeItem(element);
			}
			itemList = null;
		}
	}
			...
			if (pc.getLevel() <= 65) {
				try {
					for (int element : allBuffSkill) {
						skilluse.handleCommands(pc, element, pc.getId(), pc.getX(), pc.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
		...
		public void run() {
			try {
				...
				if (player != null) {
					...
					if (player.isCrown()) {
						if (!player.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.GLOWING_WEAPON)) {
							Broadcaster.broadcastPacket(player, da);
							skilluse.handleCommands(player, L1SkillId.GLOWING_WEAPON, player.getId(), player.getX(), player.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
							Thread.sleep(7000);
						}
						if (!player.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.SHINING_SHIELD) && player.getLevel() >= 55) {
							Broadcaster.broadcastPacket(player, da);
							skilluse.handleCommands(player, L1SkillId.SHINING_SHIELD, player.getId(), player.getX(), player.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
							Thread.sleep(7000);
						}
