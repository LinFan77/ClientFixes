...
public class LineageClient implements PacketOutput {
	...
	ClientThreadObserver _observer = new ClientThreadObserver(Config.AUTOMATIC_KICK * 60 * 1000);
	...
	public LineageClient(Channel _chnnel, long key) {
		...
		// monitor client thread until auto kick
	...
	public void close() {
		if (!close) {
			...
			try {
				...
				stopObsever();
				DecoderManager.getInstance().removeClient(this, _threadIndex);
	...
	public String getHostname() {
		...
		if ((chnnel == null) || (chnnel.getRemoteAddress() == null))
			return null;
	...
	public byte[] encryptD(byte[] data) {
		try {
			...
			_log.log(Level.WARNING, "LineageClient.encryptD Exception raised.", e);
		}
		return null;
	}
	...
	public byte[] encryptE(byte[] data) {
		try {
			...
			_log.log(Level.WARNING, "LineageClient.encryptE Exception raised.", e);
		}
		return null;
	}
