			...
			try {
				...
				synchronized (this){
					...
					/** Processing for Glodio index */
						...
						if (DTmapid != 0){
							/** Check if it is a waiting room, move if it is long */
							_DL.SystemGo(player, _ind._type, _ind._dungeonInfo);
						}
					} else if (_ind._type == 5) { // Hwadong Dawn
						...
						if (FDmapId != 0) {
							if (player.getMapId() == 2699) {
								...
								L1NpcInstance lightDeath = GameList.getwdeath();
								if (lightDeath != null) {
									lightDeath.Wchat_start(player);
								}
								player.endOfDungeon();
							}
						}
					} else if (_ind._type == 6) { // monsom
