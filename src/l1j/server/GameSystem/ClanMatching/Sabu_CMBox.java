...
public class Sabu_CMBox {
	...
	private static ArrayList<Sabu_cm> cmlist;	// It might be appropriate to implement it as a map.
	private static ArrayList<Sabu_um> umlist;	// When the user browses the application list, all registered clans are not searched.
												// added to avoid
	...
	public void Load() {
		Init();
		System.out.println(":: Clan Matching Table Load Complete");
	}
	...
	public ArrayList<Sabu_um> getjoinlist() {
		ArrayList<Sabu_um> l = new ArrayList<>();
		synchronized (umlist) {
			l.addAll(umlist);
		}
		return l;
		...
		try {
			synchronized (cmlist) {
				if (cmlist.contains(cm)) {
					cmlist.remove(cm);
					pledgeRegistrationRemove(cm.getClanId());
					cm = null;
				}
			}
		} catch (Exception e) {
	...
	public void update(int clanid, String info, int type) {
		try {
			synchronized (cmlist) {
				ArrayList<Sabu_cm> l = new ArrayList<>();
				l.addAll(cmlist);
				for (Sabu_cm cm : l) {
					if (cm.getClanId() == clanid) {
						cm.setInfo(info);
						cm.settype(type);
						modifyRegistration(clanid, type, info);
					}
				}
			}
		} catch (Exception e) {
	...
	private void add(Sabu_cm cm) {
		try {
			synchronized (cmlist) {
				if (!cmlist.contains(cm)) {
					pledgeRegistration(cm.getClanId(), cm.gettype(), cm.getInfo());
					cmlist.add(cm);
				}
			}
		} catch (Exception e) {
	...
	private static void Init() {
		...
		try {
			...
			pstm.close();
			rs.close();
			Sabu_cm[] scm = cmlist.toArray(new Sabu_cm[cmlist.size()]);
			int clanid = 0;
	...
	private void applicationForMembership(String name, int clanid, String info, int num, int type) {
		try {
			Sabu_um um = new Sabu_um();
			um.setcharname(name);
			um.setClanId(clanid);
		...
		try {
			synchronized (cmlist) {
				...
				for (Sabu_cm cm : l) {
					if (cm.getnum() == num) {
						cm.add(charname);
						applicationForMembership(charname, cm.getClanId(), cm.getInfo(), num, cm.gettype());
						userRegistrationApplication(charname, cm.getClanId(), num);
					}
				}
			}
		} catch (Exception e) {
				...
				for (Sabu_um um : l) {
					if (um.getcharname().equalsIgnoreCase(charname)) {
						if (clanId == um.getClanId()) {
							cancelUserRegistration(um.getcharname(), um.getClanId());
	...
	public ArrayList<Sabu_um> joinList(String charname) {
		try {
			synchronized (umlist) {
				ArrayList<Sabu_um> list = new ArrayList<>();
	...
	private synchronized static void userRegistrationApplication(String charname, int clanid, int num) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	private synchronized static void cancelUserRegistration(String charname, int clanid) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	private synchronized static void pledgeRegistration(int clanid, int type, String info) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	private synchronized static void modifyRegistration(int clanid, int type, String info) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	private synchronized static void pledgeRegistrationRemove(int clanid) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
