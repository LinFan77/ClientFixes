		...
		while (true) {
			try {
				if (System.currentTimeMillis() >= nw_time.getTime()) {
					try {
						nw_time.setTime(nw_time.getTime() + 60000 * 60 * 24 * 7);
						navalReset();
						nwupdate(nw_time);
					} catch (Exception e) {
					}
				...
				if (System.currentTimeMillis() >= sandW_time.getTime()) {
					try {
						...
						sandW_time.setTime(sandW_time.getTime() + 60000 * 60 * 24 * 2);
						swupdate(sandW_time);
					} catch (Exception e) {
					}
				}
			...
			while (rs.next()) {
				for (int i = 1; i <= 8; i++) {
					...
					if (pc == null) {
						...
						pc.getInventory().storeItem(60104, 1); // Asmodian Weapon Protection Scroll
					}
				}
			}
