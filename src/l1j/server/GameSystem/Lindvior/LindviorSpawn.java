/**
 * Wind Dragon Lindvior, which has been renewed
 * Lindvior map related spawn handling class
 * by. Kane
 */
...
public class LindviorSpawn {
	private static Logger _log = Logger.getLogger(LindviorSpawn.class.getName());
	private static LindviorSpawn _instance;
	private Random _rnd = new Random(System.nanoTime());

	public static LindviorSpawn getInstance() {
		if (_instance == null) {
			_instance = new LindviorSpawn();
		}
		return _instance;
	}

	private LindviorSpawn() {
	}
			...
			while (rs.next()) {
				...
				if (l1npc != null) {
					try {
						if (type == 2) {
							if (level2_randomck == 0) {
								if (rs.getString("location").equalsIgnoreCase(
										"린드2차 왼쪽")) // lind 2nd left
									continue;
							} else if (level2_randomck == 1) {
								if (rs.getString("location").equalsIgnoreCase(
										"린드2차 오른쪽")) // lind 2nd right
									continue;
							}
						}
						...
						if (field.getSpawnLocation().equalsIgnoreCase("린드2차 왼쪽")) { // Lind 2nd Left
							field.getGfxId().setTempCharGfx(8047);
						} else if (field.getSpawnLocation().equalsIgnoreCase("린드2차 오른쪽")) { // Lind 2nd Right
