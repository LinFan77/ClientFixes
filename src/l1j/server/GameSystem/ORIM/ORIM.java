...
public class ORIM {
		...
		for (L1PcInstance Ppc : pc.getParty().getMembers()) {
			if (Ppc != null) {
				L1Teleport.teleport(Ppc, 32792 + _rnd.nextInt(5), 32801 + _rnd
						.nextInt(5), (short) mapid, Ppc.getMoveState().getHeading(), true);
				nwfc.addMember(Ppc);
				databaseInsert(Ppc);
			}
		}
	...
	/*
	 * Balakas: Who wakes me up? 18869 Death Knight: Balakas! I'm finally meeting you.. 18870 Balakas: Who woke me up
	 * The price... will become my slave and pay for the rest of my life...18871 Death Knight: It's not too late to say that even when I'm defeated.
	 * Balakas: Heck.. You are full of confidence..18873 Balakas: Die human! 18874
	 */

	private void MSG(L1NpcInstance npc, int id, String msg, int x, int y) {
		Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(id, msg, x, y), true);
	}
