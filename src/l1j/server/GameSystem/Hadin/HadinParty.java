...
public class HadinParty {
	public HadinParty(L1Party p, short _mapid) {
		...
		for (L1NpcInstance npc : BasicNpcList) {
			if (npc != null) {
				if (npc.getName().equalsIgnoreCase("Hardin")) {
					Npc_Hadin = npc;
				} else if (npc.getName().equalsIgnoreCase("Hardin Floor Effect")) {
					Hadin_Effect = npc;
				} else if (npc.getName().equalsIgnoreCase("Cerenis")) {
					Kere = npc;
				}
		...
		for (L1NpcInstance npc : Leader_BasicNpcList) {
			if (npc.getSpawnLocation().equalsIgnoreCase("Orc"))
				Npc_Orc = npc;
			else if (npc.getSpawnLocation().equalsIgnoreCase("Orc Warrior"))
				Npc_OrcWarrior = npc;
			else if (npc.getSpawnLocation().equalsIgnoreCase("Orc Archer"))
				Npc_OrcAcher = npc;
