		...
		try {
			...
			if (map == null) {
				S_SystemMessage sm = new S_SystemMessage(
						"Instance dungeons can no longer be created.");
		...
		try {
			...
			if (rs.next()) {
				...
				try {
					...
					pstm2 = con2.prepareStatement("UPDATE instance_dungeon_in SET hadin_count=? WHERE charName=?");
				...
				try {
					con2 = L1DatabaseFactory.getInstance().getConnection();
					pstm2 = con.prepareStatement("INSERT INTO instance_dungeon_in SET charName=?, hadin_count=?");
