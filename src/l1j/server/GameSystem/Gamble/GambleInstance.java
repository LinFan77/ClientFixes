...
public class GambleInstance extends L1NpcInstance {
	...
	public String getClanname() { // clan name
		return clanname;
	}
			...
			if (!npc.Npc_trade && !play) {
				if (!npc.isParalyzed()) {
					if (player.getTradeReady())
						return;
					player.setTradeID(getId()); // Save the opponent's object ID
	...
	private static String chat9 = "Oh! Congratulations... Dividend paid...";
	private static String chat11 = "It's a pity... Please try next time!";
		...
		public void run() {
			try {
				...
				String chat = pc.getName() + "님 " + item_count
						+ "You made a bet on Aden~ Every time you hit one, it's 5 times bigger~!";
				...
				while (count-- > 0) {
					...
					if (pc.Gamble_Text == null || pc.Gamble_Text.equals("")) {
						Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(
								npc, "The name of the mob to bet on " + (count * 4) + "Please tell me in seconds",
								0), true);
					} else
						break;
				}
				if (pc.Gamble_Text == null || pc.Gamble_Text.equals("")) {
					...
					Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc,
							"I didn't enter a mob name within 60 seconds, so I'll just have Aden~", 0), true);
					return;
				}
				Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc, pc.Gamble_Text + "bet on", 0), true);
				Thread.sleep(2000);
				if (ck())
					return;
				Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc, "Come on! I'm going!", 0), true);
				Thread.sleep(2000);
				if (ck())
					return;
				boolean win = false;
				...
				if (ck())
					return;
				Broadcaster.broadcastPacket(npc.wand_npc[0], new S_DoActionGFX(
								npc.wand_npc[0].getId(), ActionCodes.ACTION_Wand), true);
				Broadcaster.broadcastPacket(npc.wand_npc[1], new S_DoActionGFX(
								npc.wand_npc[1].getId(), ActionCodes.ACTION_Wand), true);
				Broadcaster.broadcastPacket(npc.wand_npc[2], new S_DoActionGFX(
								npc.wand_npc[2].getId(), ActionCodes.ACTION_Wand), true);
				Broadcaster.broadcastPacket(npc.wand_npc[3], new S_DoActionGFX(
								npc.wand_npc[3].getId(), ActionCodes.ACTION_Wand), true);
				Broadcaster.broadcastPacket(npc.wand_npc[4], new S_DoActionGFX(
								npc.wand_npc[4].getId(), ActionCodes.ACTION_Wand), true);
				Broadcaster.broadcastPacket(npc.wand_npc[5], new S_DoActionGFX(
								npc.wand_npc[5].getId(), ActionCodes.ACTION_Wand), true);
				Thread.sleep(1200);
				if (ck())
					return;
				...
				GeneralThreadPool.getInstance().execute(new attack(npc.wand_npc[0], npc1));
				GeneralThreadPool.getInstance().execute(new attack(npc.wand_npc[1], npc2));
				GeneralThreadPool.getInstance().execute(new attack(npc.wand_npc[2], npc3));
				GeneralThreadPool.getInstance().execute(new attack(npc.wand_npc[3], npc4));
				GeneralThreadPool.getInstance().execute(new attack(npc.wand_npc[4], npc5));
				GeneralThreadPool.getInstance().execute(new attack(npc.wand_npc[5], npc6));
				Thread.sleep(1500);
				if (ck())
					return;
				if (win) {
					...
					for (int i = 0; i < 4; i++) {
						...
						if (npc.getTileLineDistance(pc) == 1) {
							if (FaceToFace.faceToFaceforNpc(pc, true,
									npc.getId()) != null) {
								pc.setTradeID(npc.getId()); // Save the opponent's object ID
								pc.sendPackets(new S_Message_YN(252, npc.getName()),
										true); // %0%s wants to trade items with you. do you trade? (Y/N)
							}
						}
						Thread.sleep(2000);
						if (ck())
						...
						if (i == 3 && !npc.Npc_trade) {
							...
							Broadcaster.broadcastPacket(npc,
									new S_NpcChatPacket(npc, pc.getName()
											+ "You didn't receive the exchange, so I put it in your inventory.", 0),
									true);
							npc.play = false;
						}
					}
					...
					try {
						...
						attacker.getMoveState().setHeading(
								CharPosUtil.targetDirection(attacker,
										mon.getX(), mon.getY())); // direction set
