...
public class PetRacing implements Runnable {
	...
	private final int LIMIT_ENTERMEMBER_COUNT = 2; // Number of people required to print admission message (Bonsub: 2)
	private final int LIMIT_STARTMEMBER_COUNT = 2; // Number of people required to start the game (Bonseop: 2)
	...
	private final ArrayList<L1PcInstance> entermember = new ArrayList<>();
	private final ArrayList<L1PcInstance> playmember = new ArrayList<>();
	...
	public void addMember(L1PcInstance pc) {
		if (!isEnterMember(pc)) {
			...
			pc.sendPackets(new S_SystemMessage("You have been booked to enter the Pet Match. Position: " + count), true);
			count = null;
			if (getStatus() == STATUS.REST)
				setStatus(STATUS.ENTERREADY);
		} else {
			S_SystemMessage sm1230 = new S_SystemMessage("Pet Match has already been filled.");
	...
	private void sendMessage(MSG message) {
		switch (message) {
		case ENTER:
			S_Message_YN ms = new S_Message_YN(1256, ""); // Do you want to participate in the race? (Y/N)
			L1PcInstance[] list = getEnterMemberArray();
			for (L1PcInstance pc : list) {
				if (pc != null)
					pc.sendPackets(ms);
			...
			break;
		case WAIT_START:
			S_ServerMessage sm = new S_ServerMessage(1257); // The race will start soon.
		...
		case NOT_ENOUGH_STARTMEMBERS:
			...
			//The race will be terminated because the
			//number of participants is less than the minimum of two. I returned 1,000 Adena.
		...
		case GAMEEND:
			...
			//The race time of 5 minutes has passed. The race will be terminated.
