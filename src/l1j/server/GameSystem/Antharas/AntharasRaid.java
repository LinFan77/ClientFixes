...
import static l1j.server.server.model.skill.L1SkillId.ILLUSION_GOLEM;
...
import l1j.server.GameSystem.Antharas.AntharasRaidSystem.AntharasMsgTimer;
import l1j.server.GameSystem.Antharas.AntharasRaidSystem.antamsg;
...
public class AntharasRaid {

	public AntharasRaidTimer art = null;

	/*
	 * private L1Party Party1 = null; private L1Party Party2 = null; private
	 * L1Party Party3 = null; private L1Party Party4 = null;
	 */
	private final Map<Integer, Boolean> MiniBoss1 = new ConcurrentHashMap<>();
	private final Map<Integer, Boolean> MiniBoss2 = new ConcurrentHashMap<>();
	private final Map<Integer, Boolean> MiniBoss3 = new ConcurrentHashMap<>();
	...
	private final ArrayList<L1PcInstance> _antalist = new ArrayList<>();

	private int _id;
	private long _endtime;
	private boolean _isAntharas = false;
	private L1NpcInstance anta = null;
	private Random random = new Random(System.nanoTime());

	public AntharasRaid(int id) {
		_id = id;
		_endtime = System.currentTimeMillis() + 7200000; // 2 hours later
	...
	public void timeOverRun(int type) {
		switch (type) {
		case -4: // 4th room 2 minute over mob summon
			AntharasMsgTimer room4 = new AntharasMsgTimer(_id, 4);
			GeneralThreadPool.getInstance().execute(room4);
			break;
		case -3:
			AntharasMsgTimer room3 = new AntharasMsgTimer(_id, 3);
			GeneralThreadPool.getInstance().execute(room3);
			break;
		case -2:
			AntharasMsgTimer room2 = new AntharasMsgTimer(_id, 2);
			GeneralThreadPool.getInstance().execute(room2);
			break;
		case -1:
			AntharasMsgTimer room1 = new AntharasMsgTimer(_id, 1);
			GeneralThreadPool.getInstance().execute(room1);
			break;

		case 0:// At the end of the 20-minute check time in Room 1
			break;
		case 1:// At the end of the 20-minute check time in Room 1
			break;
		case 2:// At the end of the 20-minute check in room 2
			break;
		case 3:// At the end of the 20-minute check in room 3
			break;
		case 4:// 4th room 2 minute over mob summon
			break;
		...
		case 5: // Antharas
			...
			// Summon Mob
			break;
		case 6: // Tell everyone in the Anta Room
			if (isAntharasUp()) {
				antamsg anta1 = new antamsg(_id, 1);
				GeneralThreadPool.getInstance().execute(anta1);
			}
			break;
		case 7: // When the 1st hit is behind, the comment is ppooing
			/*
			...
			GeneralThreadPool.getInstance().execute(anta1);
			break;
		case 8: // When the 2nd hit is behind, the comment is ppooing
			/*
			...
			break;
		case 9: // When the 3rd hit is back, the comment
			L1World.getInstance().broadcastServerMessage(
					"Dwarf's Cry: The warriors who stopped the black breath of Antharas were born!!");
			GeneralThreadPool.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					for (L1PcInstance player : L1World.getInstance().getAllPlayers()) {
						if (player.getMapId() == anta().getMapId()) {
							createNewItem(player, 5000064, 1); // trail 1
							for (L1Object temp : L1World.getInstance()
									.getVisibleObjects(player.getMapId()).values()) {
								if (temp instanceof L1PcInstance)
									((L1PcInstance) temp).sendPackets(new S_ServerMessage(
											813, "Antharas", "Dragon's Mark", player.getName()));
							}
		...
		case 11: { // Anta Ticetor
			...
			try {
				synchronized (anta().synchObject) {
					...
					gfx1 = null;
					// Changing the location of spawn...
								...
								for (L1Object obj2 : L1World.getInstance()
									...
									if (obj2 instanceof L1PcInstance) {
										...
										L1Teleport.teleport(pc2, ax, ay,
												(short) am, pc2.getMoveState().getHeading(), true);
									}
									break;
								}
							}
				...
				try {
					...
					for (L1Object obj : L1World.getInstance()
						...
						if (obj instanceof L1PcInstance) {
							...
							if (pc.isDead())
								continue;
							int[] stunTimeArray = { 3000, 3500, 4000, 4500, 5000, 5500 };
							_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
							S_SkillSound ss1 = new S_SkillSound(pc.getId(), 4434);
							pc.sendPackets(ss1);
							Broadcaster.broadcastPacket(pc, ss1);
							pc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.SHOCK_STUN, _shockStunDuration);
		...
		case 13: { //hit nuts 3
			try {
				...
				synchronized (anta().synchObject) {
					...
					if (anta().isDead() || anta()._destroyed)
						return;
					Broadcaster.broadcastPacket(anta(), new S_DoActionGFX(anta().getId(), 47));
					int _shockStunDuration;
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 4)) {
						if (obj instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) obj;
							if (pc.isDead()) {
								continue;
					...
					time = SprTable.getInstance().getAttackSpeed(anta().getNpcTemplate().get_gfxid(), 47);
					if (time > 0)
						Thread.sleep(anta().calcSleepTime(time, 2));
					if (anta().isDead() || anta()._destroyed)
						return;
					...
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 13)) {
						if (obj instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) obj;
							if (pc.isDead()) {
								continue;
					...
					time = SprTable.getInstance().getAttackSpeed(anta().getNpcTemplate().get_gfxid(), 30);
					if (time > 0)
						Thread.sleep(anta().calcSleepTime(time, 2));
					anta().setParalyzed(false);
				}
		...
		case 14: { // anta tif 3
			try {
				...
				synchronized (anta().synchObject) {
					...
					Broadcaster.broadcastPacket(anta(), new S_DoActionGFX(anta().getId(), 19));
					int time = SprTable.getInstance().getNodirSpellSpeed(anta().getNpcTemplate().get_gfxid());
					int _shockStunDuration;
					for (L1Object obj : L1World.getInstance()
							.getVisibleObjects(anta(), 4)) {
						if (obj instanceof L1PcInstance) {
							...
							int[] stunTimeArray = { 3000, 3500, 4000, 4500, 5000, 5500 };
							_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
							S_SkillSound ss3 = new S_SkillSound(pc.getId(), 4434);
							pc.sendPackets(ss3);
							Broadcaster.broadcastPacket(pc, ss3);
							ss3 = null;
							pc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.SHOCK_STUN, _shockStunDuration);
							L1EffectSpawn.getInstance().spawnEffect(81162, _shockStunDuration, pc.getX(), pc.getY(),
									pc.getMapId());
							S_Paralysis par = new S_Paralysis(S_Paralysis.TYPE_STUN, true);
							pc.sendPackets(par);
							par = null;
					...
					if (anta().isDead() || anta()._destroyed)
						return;
					Broadcaster.broadcastPacket(anta(), new S_DoActionGFX(anta().getId(), 20));
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 13)) {
						if (obj instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) obj;
							if (pc.isDead())
					...
					time = SprTable.getInstance().getMoveSpeed(anta().getNpcTemplate().get_gfxid(), 20);
					if (time > 0)
						Thread.sleep(anta().calcSleepTime(time, 2));
					if (anta().isDead() || anta()._destroyed)
						return;
		...
		case 15: { // Hit Lirap
			...
			try {
				synchronized (anta().synchObject) {
					...
					Broadcaster.broadcastPacket(anta(), gfx);
					gfx = null;
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 5)) {
						if (obj instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) obj;
							if (pc.isDead()) {
								continue;
		...
		case 16: { // Hit Ken Lowe
			...
			try {
				synchronized (anta().synchObject) {
					...
					Broadcaster.broadcastPacket(anta(), gfx);
					gfx = null;
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 3)) {
						if (obj instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) obj;
							if (pc.isDead()) {
								continue;
							}
							new L1SkillUse().handleCommands(null, 40055, pc.getId(), pc.getX(), pc.getY(), null, 0,
									L1SkillUse.TYPE_NORMAL, anta());
							break;
						}
					}
					int time = SprTable.getInstance().getDirSpellSpeed(anta().getNpcTemplate().get_gfxid());
					if (time > 0)
						Thread.sleep(anta().calcSleepTime(time, 2));
					anta().setParalyzed(false);
				}
				...
				synchronized (anta().synchObject) {
					...
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 3)) {
						if (obj instanceof L1PcInstance) {
							...
							anta().getMoveState().setHeading(calcheading(anta().getX(), anta().getY(), pc.getX(), pc.getY()));
							Broadcaster.broadcastPacket(anta(), new S_ChangeHeading(anta()));
							Broadcaster.broadcastPacket(anta(), new S_DoActionGFX(anta().getId(), 5));
							int ran2 = random.nextInt(300) + 50;
							pc.receiveDamage(anta(), ran2, false);
							int time = SprTable.getInstance().getAttackSpeed(anta().getNpcTemplate().get_gfxid(), 5);
							if (time > 0)
								Thread.sleep(anta().calcSleepTime(time, 2));
							if (anta().isDead() || anta()._destroyed || pc.isDead()) {
								break;
							}
							S_DoActionGFX gfx3 = new S_DoActionGFX(anta().getId(), 19);
							Broadcaster.broadcastPacket(anta(), gfx3);
							gfx3 = null;
							int ran3 = random.nextInt(500) + 200;
							pc.receiveDamage(anta(), ran3, false);
							time = SprTable.getInstance().getNodirSpellSpeed(anta().getNpcTemplate().get_gfxid());
							if (time > 0)
								Thread.sleep(anta().calcSleepTime(time, 2));
							break;
						}
				...
				try {
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 3)) {
						if (obj instanceof L1PcInstance) {
							...
							S_DoActionGFX gfx2 = new S_DoActionGFX(anta().getId(), 5);
							...
							S_DoActionGFX gfx = new S_DoActionGFX(anta().getId(), 12);
							Broadcaster.broadcastPacket(anta(), gfx);
							gfx = null;
							int ran = random.nextInt(300) + 50;
							pc.receiveDamage(anta(), ran, false);
							time = SprTable.getInstance().getAttackSpeed(anta().getNpcTemplate().get_gfxid(), 12);
							if (time > 0)
								Thread.sleep(anta().calcSleepTime(time, 2));
							if (anta().isDead() || anta()._destroyed)
								break;
							...
							int ran3 = random.nextInt(500) + 200;
							pc.receiveDamage(anta(), ran3, false);
							time = SprTable.getInstance().getDirSpellSpeed(anta().getNpcTemplate().get_gfxid());
							if (time > 0)
								Thread.sleep(anta().calcSleepTime(time, 2));
							if (anta().isDead() || anta()._destroyed)
								break;
					...
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 5)) {
						if (obj instanceof L1PcInstance) {
							...
							if (pc.getResistance().getMr() < 150) {
								S_SkillSound ss1 = new S_SkillSound(pc.getId(), 870);
								Broadcaster.broadcastPacket(pc, ss1);
								pc.sendPackets(ss1);
								ss1 = null;
								// Cancel
							...
							L1ParalysisPoison.doInfection(pc, 8000, 15000); // paralysis
							if (!ck) {
								ck = true;
								new L1SkillUse().handleCommands(null, 40055, pc.getId(), pc.getX(), pc.getY(), null,
				...
				synchronized (anta().synchObject) {
					...
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 4)) {
						if (obj instanceof L1PcInstance) {
							...
							if (pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.COUNTER_MAGIC)) {
								pc.getSkillEffectTimerSet().removeSkillEffect(COUNTER_MAGIC);
								int castgfx = SkillsTable.getInstance().getTemplate(COUNTER_MAGIC).getCastGfx();
								S_SkillSound ss2 = new S_SkillSound(pc.getId(), castgfx);
					...
					int time = SprTable.getInstance().getMoveSpeed(anta().getNpcTemplate().get_gfxid(), 20);
					if (time > 0)
						Thread.sleep(anta().calcSleepTime(time, 2));
					anta().setParalyzed(false);
				}
		...
		case 22: { // Anta Nuts Shim<<
			...
			synchronized (anta().synchObject) {
				anta().setParalyzed(true);
				try {
					int time = SprTable.getInstance().getAttackSpeed(anta().getNpcTemplate().get_gfxid(), 47);
					if (time > 0)
						Thread.sleep(anta().calcSleepTime(time, 2));
					if (anta().isDead() || anta()._destroyed)
						return;
					...
					time = SprTable.getInstance().getAttackSpeed(anta().getNpcTemplate().get_gfxid(), 41);
					if (time > 0)
						Thread.sleep(anta().calcSleepTime(time, 2));
					if (anta().isDead() || anta()._destroyed)
						return;
		...
		case 23: { // Anta Tiff Sim<<
			...
			try {
				synchronized (anta().synchObject) {
					...
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 4)) {
						if (obj instanceof L1PcInstance) {
							...
							if (pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.COUNTER_MAGIC)) {
								pc.getSkillEffectTimerSet().removeSkillEffect(COUNTER_MAGIC);
								int castgfx = SkillsTable.getInstance().getTemplate(COUNTER_MAGIC).getCastGfx();
								S_SkillSound ss3 = new S_SkillSound(pc.getId(), castgfx);
								Broadcaster.broadcastPacket(pc, ss3);
								pc.sendPackets(ss3);
								ss3 = null;
								continue;
					 ...
					 * null;
					 */
					Broadcaster.broadcastPacket(anta(), new S_DoActionGFX(anta().getId(), 30));
					for (L1Object obj : L1World.getInstance().getVisibleObjects(anta(), 13)) {
						if (obj instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) obj;
							if (pc.isDead()) {
							...
							if (pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.COUNTER_MAGIC)) {
								pc.getSkillEffectTimerSet().removeSkillEffect(COUNTER_MAGIC);
								int castgfx = SkillsTable.getInstance().getTemplate(COUNTER_MAGIC).getCastGfx();
								S_SkillSound ss4 = new S_SkillSound(pc.getId(), castgfx);
	...
	private boolean isNotCancelable(int skillNum) {
		return skillNum == ENCHANT_WEAPON || skillNum == BLESSED_ARMOR
				...
				|| skillNum == AWAKEN_VALAKAS || skillNum == BOUNCE_ATTACK
				|| skillNum == ILLUSION_OGRE || skillNum == ILLUSION_LICH
				|| skillNum == ILLUSION_GOLEM
	...
	public void can(L1PcInstance pc) {
		...
		for (int element : cancelForceDelete) {
			if (pc.getSkillEffectTimerSet().hasSkillEffect(element))
				pc.getSkillEffectTimerSet().removeSkillEffect(element);
		}
	...
	/** Get the number of users who entered Antharas lair*/
	public int countUsersInLair() {
		return _antalist.size();
	}

	/** list the user to enter Antharas lair */
	...
	/** user to enter Antharas lair*/
	public void clLairUser() {
		_antalist.clear();
	}

	/** returns the hits object */
	...
	/** Tells if Antharas is up */
	public boolean isAntharasUp() {
		return _isAntharas;
	}

	/** Set whether Antharas is popping up */
	...
	private int comaA = 4038003;
	private int comaB = 4038004;
	private int comaC = 4038005;
	...
	private void BossSpawn(int step, int type) {
		...
		case 1:
			switch (step) {
			case 1:
				L1SpawnUtil.spawn2(32636, 32799, (short) _id, comaA, 0, 0, 0);
				break;
			case 2:
				L1SpawnUtil.spawn2(32655, 32849, (short) _id, comaB, 0, 0, 0);
				break;
			case 3:
				L1SpawnUtil.spawn2(32649, 32921, (short) _id, comaC, 0, 0, 0);
				break;
			}
			break;
		case 2:
			switch (step) {
			case 1:
				L1SpawnUtil.spawn2(32893, 32607, (short) _id, comaA, 0, 0, 0);
				break;
			case 2:
				L1SpawnUtil.spawn2(32911, 32657, (short) _id, comaB, 0, 0, 0);
				break;
			case 3:
				L1SpawnUtil.spawn2(32905, 32905, (short) _id, comaC, 0, 0, 0);
				break;
			}
			break;
		case 3:
			switch (step) {
			case 1:
				L1SpawnUtil.spawn2(32892, 32799, (short) _id, comaA, 0, 0, 0);
				break;
			case 2:
				L1SpawnUtil.spawn2(32911, 32849, (short) _id, comaB, 0, 0, 0);
				break;
			case 3:
				L1SpawnUtil.spawn2(32905, 32921, (short) _id, comaC, 0, 0, 0);
				break;
			}
			break;
		case 4:
			switch (step) {
			case 1:
				L1SpawnUtil.spawn2(32765, 32799, (short) _id, comaA, 0, 0, 0);
				break;
			case 2:
				L1SpawnUtil.spawn2(32783, 32849, (short) _id, comaB, 0, 0, 0);
				break;
			case 3:
				L1SpawnUtil.spawn2(32777, 32921, (short) _id, comaC, 0, 0, 0);
				break;
			}
			break;
		}

		S_SystemMessage sm = new S_SystemMessage("Dwarf cry: The gatekeeper has appeared! Defeat him to get through here.");
	...
	private ArrayList<L1PcInstance> _list = new ArrayList<>();
				...
				try {
					...
					if (list != null && size > 0) {
						...
						for (L1PcInstance pc : list) {
							if (pc == null || pc.getMapId() != _id || pc.getNetConnection() == null
									|| !((pc.getX() >= 32589 && pc.getX() <= 32687) && (pc.getY() >= 32781 && pc.getY() <= 32946))) {
								ck2 = true;
							}
						}
						if (ck2) {
