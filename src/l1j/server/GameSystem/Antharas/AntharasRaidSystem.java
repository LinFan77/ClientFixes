...
public class AntharasRaidSystem {
	// private static Logger _log =
	// Logger.getLogger(AntarasRaidSystem.class.getName());

	private static AntharasRaidSystem _instance;
	private final Map<Integer, AntharasRaid> _list = new ConcurrentHashMap<>();
	private int[] _mapid2 = { 0, 0, 0, 0, 0, 0 };
	private static Random random = new Random(System.nanoTime());

	public static AntharasRaidSystem getInstance() {
		if (_instance == null) {
			_instance = new AntharasRaidSystem();
		}
		return _instance;
	}
	...
	static class antamsg implements Runnable {
		...
		public void run() {
			try {
				...
				case 0:// 용레어 처음 입장시
					try {
						AntaTrapSpawn();
						AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(
								_mapid);
						ar.setAntaras(true);
						...
						for (L1PcInstance pc : list) {
							...
							Timestamp deleteTime = new Timestamp(
									System.currentTimeMillis()
											+ (86400000 * Config.RAIDTIME));// 3일
							pc.sendPackets(new S_PacketBox(
									S_PacketBox.DRAGONRAIDBUFF, 86400 * 2), true);
		...
		public void exitAR(int id) {
			...
			AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(id);
			ar.clLairUser();
			ar.setAntaras(false);
			ar.setanta(null);
			ar.MiniBossReset();
	...
	static class AntharasMsgTimer implements Runnable {
		private int _mapid = 0;
		private int _type = 0;

		public AntharasMsgTimer(int mapid, int type) {
			_mapid = mapid;
			_type = type;
		}
		...
		public void run() {
			try {
				...
				AntharasRaidSpawn.getInstance().fillSpawnTable(_mapid, _type); // 알
																				// 스폰
				AntharasRaid PT = AntharasRaidSystem.getInstance().getAR(_mapid);
				// L1Party PT =
	...
	public boolean startRaid(L1PcInstance pc) {
		...
		AntharasRaid ar = new AntharasRaid(id);

		L1WorldMap.getInstance().cloneMap(1005, id);
		World.cloneMap(1005, id);
		AntharasRaidSpawn.getInstance().fillSpawnTable(id, 0);
	...
	public AntharasRaid getAR(int id) {
		return _list.get(id);
	}
	...
	public int blankMapId() {
		...
		if (a0 != 0) {
			System.out.println("Create a hit raid map : 1005");
			return a0;
		}
		if (a1 != 0) {
			System.out.println("Create a hit raid map : 1006");
			return a1;
		}
		if (a2 != 0) {
			System.out.println("Create a hit raid map : 1007");
			return a2;
		}
		if (a3 != 0) {
			System.out.println("Create a hit raid map : 1008");
			return a3;
		}
		if (a4 != 0) {
			System.out.println("Create a hit raid map : 1009");
			return a4;
		}
		if (a5 != 0) {
			System.out.println("Create a hit raid map : 1010");
			return a5;
		}
		return mapid;
	}
