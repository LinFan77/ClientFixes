...
public class ValaRaidSystem {
	...
	private final ArrayList<Integer> _map = new ArrayList<>();
	private final Map<Integer, ValaRaid> _list = new ConcurrentHashMap<>();
	...
	public void startValakas(L1PcInstance pc){
		if(countVala() >= 6){
			pc.sendPackets(new S_ChatPacket(pc,"Instance dungeons can no longer be created."));
			return;
		}
		int id = blankMapId();
		if(id != ValaKasMapID)
		...
		L1SpawnUtil.spawn2(pc.getX(), pc.getY(), pc.getMapId(), 910008, 0, 3600 * 1000, id); // portal
		L1SpawnUtil.spawn2(32731, 32922, (short) id, 3310015, 0, fire_delay * 1000, id); // Death Knight Effect
		L1SpawnUtil.spawn2(32732, 32922, (short) id, 3310018, 0, fire_delay * 1000, id); // Death Knight Buff
		L1SpawnUtil.spawn2(32733, 32937, (short) id, 60032, 0, fire_delay * 1000, id); // warehouse keeper
		L1SpawnUtil.spawn2(32727, 32937, (short) id, 3310016, 0, fire_delay * 1000, id); // general merchandiser
		L1SpawnUtil.spawn2(32751, 32928, (short) id, 3310017, 0, fire_delay * 1000, id); // rare entrance
