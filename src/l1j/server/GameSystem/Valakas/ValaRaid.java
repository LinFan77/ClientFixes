...
public class ValaRaid implements Runnable {
	...
	public void AllShockStun(){
		...
		for (L1PcInstance pc : PcStageCK()) {
		 if((pc.getX() >= 32753 && pc.getX() <= 32787) && (pc.getY() >= 32875 && pc.getY() <= 32910)) {
			...
			pc.sendPackets(new S_SkillSound(pc.getId(), 4434)); // Stern
		...
		while(Running){
			try {
				...
				case StageTwo:
					...
					Thread.sleep(sleep * 1000);
					for (L1PcInstance pc : PcStageCK()){
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "Orim: Everyone be careful, he's already awake."));
					}
					Thread.sleep(5 * 1000);
					for (L1PcInstance pc : PcStageCK()){
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "Orim: The flames around... I think they've set a trap to trap you, so be careful."));
					}
					Thread.sleep(5 * 1000);
					for (L1PcInstance pc : PcStageCK()){
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,S_PacketBox.RED_MESSAGE, "Valakas: Kreurrrr... the annoying bugs have come..", true));

					}
					Thread.sleep(5 * 1000);
					for (L1PcInstance pc : PcStageCK()){
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,S_PacketBox.RED_MESSAGE, "Valakas: Are you also part of Halfas' family?", true));

					}
					// black screen rain effect
					...
					for (L1PcInstance pc : PcStageCK()){
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,S_PacketBox.RED_MESSAGE, "Valakas: It doesn't matter... if it's a bug, it's enough to wipe it away"), true);

					}
					Thread.sleep(5 * 1000);
					//  Seeing and Valakas Appearing

					L1SpawnUtil.spawn2(32773, 32889, (short) _map, 3310032, 0, 1 * 1000, 3310032);

					Thread.sleep(7 * 1000);
					for (L1PcInstance pc : PcStageCK()){
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,S_PacketBox.RED_MESSAGE, "Valakas: I'll make you regret stepping dirty feet into the sanctuary of surveillance...!", true));
				...
				case StageThree:
					...
					// Valakas Summons Halfas at 70% Blood
					if(cur <= max * 3/4)
						stage = StageFour;
					break;
				case StageFour:
					...
					// Halfas Appearance Stage
					valakas_talk(vala, 4);
					for (L1PcInstance pc : PcStageCK()) {
						pc.sendPackets(new S_SkillSound (pc.getId(), 15837));
						pc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.HALPAS_FOLLOWER_BUFF,60);
					}
					Thread.sleep(2000);
				...
				case StageFive:
					...
					break;
				case StageSix:
					for (L1PcInstance pc : PcStageCK()){ // dragon buff
						pc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.DRAGONRAID_BUFF, (86400 * 2) * 1000);
						Timestamp deleteTime = new Timestamp(System.currentTimeMillis()+ (86400000 * Config.RAIDTIME)); // 7 days
						pc.sendPackets(new S_PacketBox(S_PacketBox.DRAGONRAIDBUFF, 86400 * 2),true);
					...
					L1World.getInstance().broadcastServerMessage("Dwarf Cry: The warriors who broke the wings of Valakas are born!!");
					Thread.sleep(2000);

					for (L1PcInstance pc : PcStageCK()) {
						pc.sendPackets(new S_ServerMessage(1476)); // Teleport after 30 seconds
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,S_PacketBox.RED_MESSAGE, "Valakas: Today I will take a step back.... But the day when the shackles of meanness are lifted and strength is restored...", true));
					}
					Thread.sleep(30000);
					stage = StageStop;
					break;
	...
	public static String []talkStr = { "Valakas: Zercuo Samkeronu...", "Valakas: Jercuo Kaoff...", "Valakas: Kur Dum Kras Harfaum..", "Valakas: Kurdum Lirascum..",
	"Valakas: Kur Dum Kras Harfaum.." };
	...
	private void health() { // L1SkillUse l1skilluse;
		L1World.getInstance().broadcastServerMessage("Dwarves yell: The warriors who broke the wings of Valakas are born!!");
	}
	...
	private boolean createNewItem(L1PcInstance pc, int item_id, int count) {
		...
		if (item != null) {
			...
			for (L1PcInstance pc1 : PcStageCK()) {
				pc1.sendPackets(new S_ServerMessage(813, "Valakas", item.getName(), pc1.getName()));
			}
			return true;
		} else {
			return false;
