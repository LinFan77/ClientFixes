...
public class UserRankingController implements Runnable {
	...
	public static long rankingUpdate = System.currentTimeMillis() / 1000;
	...
	public UserRankingController() {
		...
		System.out.print("[UserRankingController] loading...");

		list = new ArrayList<>();
		listPrince = new ArrayList<>();
		listKnight = new ArrayList<>();
		listElf = new ArrayList<>();
		listWizard = new ArrayList<>();
		listDarkElf = new ArrayList<>();
		listDragonKnight = new ArrayList<>();
		listIllusionist = new ArrayList<>();
		listWarrior = new ArrayList<>();
		listFencer = new ArrayList<>();
		load();
	...
	public void run() {
		try {
			...
			if ((isRenewal || cal.get(분) == 0)){
				/** Store updated time information whenever ranking information is updated */
				rankingUpdate = System.currentTimeMillis() / 1000;
				isRenewal = false;
				load();
	...
	public void setBuffSetting(L1PcInstance pc)
	{
		...
		if(rank != null)
		{
			...
			if(curRank >= 1 && curRank <= 10)
			{
				...
				if(!pc.getInventory().checkItem(600258)) // If there is no protection item
				{
					pc.getInventory().storeItem(600258, 1); // create item
				}
				pc.addMaxHp(200);
				pc.getAC().addAc(-3);
				pc.sendPackets(new S_HPUpdate(pc.getCurrentHp(), pc.getMaxHp()));
	...
	private void load() {
		...
		ArrayList<L1UserRanking> templist = new ArrayList<>();
		ArrayList<L1UserRanking> templistPrince = new ArrayList<>();
		ArrayList<L1UserRanking> templistKnight = new ArrayList<>();
		ArrayList<L1UserRanking> templistElf = new ArrayList<>();
		ArrayList<L1UserRanking> templistWizard = new ArrayList<>();
		ArrayList<L1UserRanking> templistDarkElf = new ArrayList<>();
		ArrayList<L1UserRanking> templistDragonKnight = new ArrayList<>();
		ArrayList<L1UserRanking> templistIllusionist = new ArrayList<>();
		ArrayList<L1UserRanking> templistWarrior = new ArrayList<>();
		ArrayList<L1UserRanking> templistFencer = new ArrayList<>();
