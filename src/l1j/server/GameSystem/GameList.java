...
public class GameList {
	...
	private static boolean dragonLandSpawn = false;
	...
	public synchronized static boolean get용땅() {
		return dragonLandSpawn;
	}

	public static ConcurrentHashMap<Integer, DreamsTemple> DTList = new ConcurrentHashMap<>(
			0);
	...
	public static ConcurrentHashMap<Integer, FireDragon> FDList = new ConcurrentHashMap<>(
			0);
	...
	public static ConcurrentHashMap<Integer, ORIM> ORList = new ConcurrentHashMap<>(
			0);
	...
	public static int getORIMMapId() {
		...
		for (int i = 9103; i <= 9200; i++) {
			...
			if (OR != null) {
				...
				for (String element : OR._pcname) {
					pc = L1World.getInstance().getPlayer(element);
					if (pc != null) {
						if (pc.getMapId() == i) {
							count++;
						}
	...
	public static ConcurrentHashMap<Integer, IceQeen> IQList = new ConcurrentHashMap<>(
			0);
	...
	public static ConcurrentHashMap<Integer, DreamsLaboratory> DLList = new ConcurrentHashMap<>(0);
