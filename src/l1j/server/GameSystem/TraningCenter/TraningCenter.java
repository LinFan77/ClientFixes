...
public class TraningCenter {
	...
	public static final int time = 3600000 * 2;
	...
	public TraningCenter() {
		...
		String[] qlist = { "character_warehouse", "character_elf_warehouse", "character_items", "clan_warehouse" };
		for (String qu : qlist) {
			if (qu == null)
				continue;
			Connection con = null;
			...
			try {
				...
				pstm = con.prepareStatement("delete FROM " + qu + " WHERE item_id=?");
				pstm.setInt(1, 60285);
				pstm.executeUpdate();
			} catch (Exception e) {
				e.printStackTrace();
	...
	public L1Map start() {
		synchronized (mapList) {
			...
			L1SpawnUtil.spawn2(32902, 32818, (short) map.getId(), 100563, 0, 0, 0);
			new ExitTimer(map);
			return map;
		}
	}
	...
	public L1Map mapCheck() {
		for (FastMap.Entry<L1Map, Boolean> e = mapList.head(), mapEnd = mapList.tail(); (e = e.getNext()) != mapEnd;) {
			if (!e.getValue()) {
				return e.getKey();
			}
		}
	...
	class ExitTimer implements Runnable {
		...
		public void run() {
			try {
				...
				for (L1Object ob : L1World.getInstance().getVisibleObjects(map.getId()).values()) {
					...
					if (ob instanceof L1NpcInstance) {
						...
						L1Teleport.teleport((L1PcInstance) ob, 33489, 32764, (short) 4, 5, true);
					}
				}
