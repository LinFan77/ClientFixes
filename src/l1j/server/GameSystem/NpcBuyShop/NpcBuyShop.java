...
public class NpcBuyShop {
	...
	private FastMap<Integer, L1Shop> _npcShops = new FastMap<>();
			...
			synchronized (_npcShops) {
				for (FastMap.Entry<Integer, L1Shop> e = _npcShops.head(), mapEnd = _npcShops
					...
					for (L1ShopItem item : list) {
						...
						try {
							if (!NpcBuyShopSell.getInstance().serchItem(shop.getNpcId(), te)) {
								shop.getSellingItems().remove(item);
								continue;
							}
						} catch (Exception e1) {
					...
					FastTable<String> check = new FastTable<>();
	...
	private NpcBuyShop() {
		...
		System.out.print("[NpcBuyShop] loading...");
		NpcChatThread.get();
	...
	public boolean shopItemCheck(int objid) {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT COUNT(*) FROM shop_npc WHERE item_objid=?");
			pstm.setInt(1, objid);
	...
	public void deleteShopItem(int npcid, int objid) { // Delete after sorting sales and purchases by item ID
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("DELETE FROM shop_npc WHERE npc_id=? AND item_objid=?");
			pstm.setInt(1, npcid);
			pstm.setInt(2, objid);
			pstm.executeUpdate();
		} catch (SQLException e) {
	...
	public void storeItemUpdate(int npcid, int objid, int count1) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE shop_npc SET count=? WHERE npc_id=? AND item_objid=?");
	...
	public void deleteShopItem(int npcid) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	public void SaveShop(L1NpcInstance npc, L1ShopItem item, int price,
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO shop_npc SET npc_id=?, npc_name=?, id=?,  item_id=?, Item_name=?, count=?, enchant=?, selling_price=?, locx=?, locy=?, locm=?, iden=?, attr=?, item_objid=?");
			...
			System.out.println("Duplicate objid -> " + npc.getName() + " >>>> [" + item.getItem().getName() + "] ID: " + item.getId());
			e.printStackTrace();
		} catch (Exception e) {
			System.out.println("Duplicate objid -> " + npc.getName() + " >>>> [" + item.getItem().getName() + "] ID: " + item.getId());
			e.printStackTrace();
	...
	public synchronized void insert(int itemid, int enchant, int attr,
		...
		while (count-- > 1) {
			...
			if ((ns == null) || (ns.getNpcId() >= 8110000 && ns.getNpcId() <= 8111000) || ns.deleting)
				continue;
			if (ns.getDefaultName() != null && ns.getShopName() != null
					&& !ns.getShopName().equalsIgnoreCase("")) {
				L1Shop s = get(ns.getNpcTemplate().get_npcId());
				if (s == null || s.getSellingItems().size() < 5) { //Absolutely only onechange to sell.
					ck = false;
					break;
				}
			}
		...
		if (ck || ns == null) {
			S_SystemMessage sm = new S_SystemMessage("> No or insufficient stores for selling through buy-in.");
			...
			sm = null;
			System.out.println("> There is no or insufficient sales shop through purchase.");
			return;
		}
		...
		if (item == null) {
			System.out.println(":: Items without price during purchase store purchase >>" + itemid);
			return;
		}
		if (time != null)
		...
		if (shop == null) {
			...
			_npcShops.put(npcId, new L1Shop(npcId, sellingList, purchasingList));
		} else {
	...
	class show implements Runnable {
		...
		public void run() {
			...
			try {
				if (ss != null) {
					GeneralThreadPool.getInstance().schedule(this, _rnd.nextInt(1000));
					return;
				}
				ss = this;
