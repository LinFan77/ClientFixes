...
public class NpcChatThread extends Thread {
	private static final String[] chat = { "\\\\fTf3Buy an item.",
			"\\\\fTf2Every single bite!", "\\\\f=f=Go go every mouth!", "\\\\fTf2Buy an item",
			"\\\\f=f=Honest manual purchase", "\\\\fTf3I buy", "I buy it. Come here!",
			"Buy expensive!", "buy high price" };
					...
					if (time < System.currentTimeMillis()) {
						if (npc.getState() == 3 && npc.show) {
							if (npc.getNormalChat() != null
								...
								Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc, npc.getNormalChat(), 0), true);
							// if(rnd.nextInt(2) == 0)
