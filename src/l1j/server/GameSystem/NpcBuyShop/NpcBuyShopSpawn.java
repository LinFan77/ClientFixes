...
public class NpcBuyShopSpawn {
	...
	private FastTable<L1NpcShopInstance> list = new FastTable<>();

	private NpcBuyShopSpawn() {
	}

	private static final int gfxlist[] = { 11322 /* 11326<-Previous Image Youngja Store Image
	...
	public FastTable<L1NpcShopInstance> load() {
		...
		ResultSet rs = null;
		FastTable<L1NpcShopInstance> npcShoplist = new FastTable<>();
				...
				if (l1npc != null) {
					try {
						L1NpcShopInstance npc = (L1NpcShopInstance) NpcTable
								.getInstance().newNpcInstance(rs.getInt("npc_id"));
				...
				try {
					...
					if (title != null
						...
						if (L1World.getInstance().getNpcShop(npc.getName()) != null) {
							Broadcaster.broadcastPacket(npc, new S_CharTitle(npc.getId(), title), true);
						}
					}
					if (normalChat != null) {
