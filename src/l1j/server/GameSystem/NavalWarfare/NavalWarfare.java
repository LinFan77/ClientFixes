...
public class NavalWarfare {
	...
	public boolean start(L1PcInstance pc) {
		...
		synchronized (mapList) {
			map = mapCheck();
			if (map == null) {
				pc.sendPackets(new S_SystemMessage("We cannot enter because there are too many people."));
				return true;
			}
		...
		for (L1PcInstance Ppc : pc.getParty().getMembers()) {
			if (Ppc != null) {
				L1Teleport.teleport(Ppc, 32792 + _rnd.nextInt(5), 32801 + _rnd
						.nextInt(5), (short) map.getId(), Ppc.getMoveState().getHeading(), true);
				nwfc.addMember(Ppc);
				databaseInsert(Ppc);
			}
		}
