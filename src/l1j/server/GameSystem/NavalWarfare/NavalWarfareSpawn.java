...
public class NavalWarfareSpawn {
						...
						if (type != 0) {
							do {
								...
								if (field.getMap().isInMap(field.getLocation())
										&& field.getMap().isPassable(field.getLocation())) {
									break;
								}
							} while (tryCount < 50);
						}
	...
	public FastTable<L1NpcInstance> monster_spawn(int mapid, int type, boolean level) {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		L1Npc l1npc = null;
			...
			while (rs.next()) {
				...
				if (l1npc != null) {
					try {
						...
						int randomCount = count > 1 ? _rnd.nextInt(count) + 1 : 1;
						if (l1npc.get_npcId() == 100155 || l1npc.get_npcId() == 100156)
							randomCount = 3;
						for (int i = 0; i < randomCount; i++) {
							if (level) {
								field = NpcTable.getInstance().newNpcInstance(rs.getInt("npc_id") + 1000000);
							} else {
								field = NpcTable.getInstance().newNpcInstance(rs.getInt("npc_id"));
							}
