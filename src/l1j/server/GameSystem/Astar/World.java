...
public final class World {
	...
	static public void init() {
		TimeLine.start("Loading the world map... no");
	...
	static private void writeCache() throws Exception {
		try {
			...
			System.out.println("Creating a cache file.");
	...
	static private void read(boolean type) throws Exception {
		// For notification of reading from text
		try {

			if (type)
				System.out.println("World map information is being extracted from the text file.");
	...
	static public void moveTheDoor(int x, int y, int map, boolean h, boolean flag) {
	...
	static public boolean moveTheDoor(int x, int y, int map, int h) {
		if (h < 0 || h > 7) {
			return false;
		}
