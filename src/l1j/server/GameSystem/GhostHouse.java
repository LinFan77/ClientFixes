...
public class GhostHouse implements Runnable {
	...
	private static final int LIMIT_MIN_PLAYER_COUNT = 2; //people required to start the game (2)
	private static final int LIMIT_MIN_ENTER_PLAYER_COUNT = 2; //people required to enter the game (5)
		...
		for (L1PcInstance pc : list) {
			if (pc != null) {
				if (getStatus() == Status.READY) {
					pc.getInventory().storeItem(40308, 1000); // 1000 adena payment
				}
	...
	public void addEnterMember(L1PcInstance pc) {
		...
		if (!isEnterMember(pc)) {
			entermember.add(pc);
			S_ServerMessage sm1253 = new S_ServerMessage(1253, Integer.toString(getEnterMemberCount()));
