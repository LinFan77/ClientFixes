...
public class Robot {
	...
	public static boolean 클래스버프(L1RobotInstance bot) {
		...
		if (bot.isKnight()) {
			bot.REDUCTION_ARMOUR_VETERAN = true;
			bot.RAGING_FORCE = true;
			...
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.BRAVE_MENTAL)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.BRAVE_MENTAL, bot.getId(), bot.getX(), bot.getY(), null, 0, L1SkillUse.TYPE_NORMAL);
				return true;
			}
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.GLOWING_WEAPON)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.GLOWING_WEAPON, bot.getId(), bot.getX(), bot.getY(), null, 0, L1SkillUse.TYPE_NORMAL);
				return true;
			}
			...
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.SHINING_ARMOUR)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.SHINING_ARMOUR, bot.getId(), bot.getX(), bot.getY(), null, 0, L1SkillUse.TYPE_NORMAL);
				return true;
			}
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.SHINING_SHIELD)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.SHINING_SHIELD, bot.getId(), bot.getX(), bot.getY(), null, 0, L1SkillUse.TYPE_NORMAL);
				return true;
			}
		} else if (bot.isElf()) {
			bot.글로리어스 = true;
			...
			bot.setSkillMastery(L1SkillId.DESTROY_HORROR);
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.AWAKEN_VALAKAS)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.AWAKEN_VALAKAS,bot.getId(), bot.getX(), bot.getY(), null, 0, L1SkillUse.TYPE_NORMAL);
				return true;
			}
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.AWAKEN_LINDVIOR)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.AWAKEN_LINDVIOR,bot.getId(), bot.getX(), bot.getY(), null, 0, L1SkillUse.TYPE_NORMAL);
				return true;
			}
			...
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.REDUCE_WEIGHT)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.REDUCE_WEIGHT,bot.getId(), bot.getX(), bot.getY(), null, 0, L1SkillUse.TYPE_NORMAL);
				return true;
			}
			...
			bot.FINALBURN = true;
			bot.BURNINGSPIRIT = true;
			bot.DRESSEVASION = true;
			bot.LUCIFER_DESTINY = true;
			...
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.FREEZING_ARMOUR)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.FREEZING_ARMOUR,bot.getId(), bot.getX(), bot.getY(), null, 960, L1SkillUse.TYPE_NORMAL);
				return true;
			}
			...
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.GIGANTIC)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.GIGANTIC,bot.getId(), bot.getX(), bot.getY(), null, 60, L1SkillUse.TYPE_NORMAL);
				return true;
			}
		}
		return false;
	...
	public static void clan_join(L1RobotInstance bot) {
		// TODO 자동 생성된 메소드 스텁
		if (bot.getClanid() != 0 || bot.isCrown() || (_random.nextInt(10) == 0))
			return;
		if (joinTime == 0) {
	...
	private static String robot_clan_count() {
		...
		int count = 0;
		FastMap<String, Integer> list = new FastMap<>();
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
		...
		public void run() {
			...
			try {
				...
				if (crown._userTitle == null
						|| crown._userTitle.equalsIgnoreCase("") || L1World.getInstance().getPlayer(crown.getName()) == null
	...
	public static void Doll_Spawn(L1RobotInstance bot) {
		// TODO 자동 생성된 메소드 스텁
		if (!인형 || bot.인형스폰 || (bot.getDollList().size() > 0))
			return;
