...
public class NurtureChatController implements Runnable {
	...
	class chat implements Runnable {
		...
		public void run() {
			try {
				S_ChatPacket s_chatpacket = new S_ChatPacket(bot, "flagged for removal", Opcodes.S_SAY, 0);
	...
	public void OnOff(L1PcInstance gm, boolean ck) {
		if (ck) {
			...
			on = true;
			gm.sendPackets(new S_SystemMessage("The upbringing chatting robot has been activated."), true);
			return;
		} else {
			if (L1World.getInstance().findObject(list.get(0).getId()) == null) {
			...
			on = false;
			gm.sendPackets(new S_SystemMessage("The upbringing chat robot has ended."), true);
			return;
		}
	}
	...
	private void spawn() {
		for (int i = 1; i <= x.length; i++) {
			...
			newPc.setTitle("Kakao Talk lvl79");
