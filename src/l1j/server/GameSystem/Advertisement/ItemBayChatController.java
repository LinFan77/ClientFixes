...
public class ItemBayChatController implements Runnable {
	...
	class chat implements Runnable {
		...
		public void run() {
			try {
				S_ChatPacket s_chatpacket = new S_ChatPacket(bot,"\\aDAden Buying and Selling Item Mania, Game Name: Frandis-Others",
						Opcodes.S_SAY, 0);
	...
	public void OnOff(L1PcInstance gm, boolean ck) {
		if (ck) {
			...
			on = true;
			gm.sendPackets(new S_SystemMessage("The item bay robot has been activated."), true);
			return;
		} else {
			if (L1World.getInstance().findObject(list.get(0).getId()) == null) {
			...
			on = false;
			gm.sendPackets(new S_SystemMessage("The item bay robot has been shut down."), true);
			return;
		}
	}
