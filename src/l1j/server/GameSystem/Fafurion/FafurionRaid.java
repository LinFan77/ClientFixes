...
import l1j.server.GameSystem.Fafurion.FafurionRaidSystem.FafurionMSG;
import l1j.server.GameSystem.Fafurion.FafurionRaidSystem.FafurionMsgTimer;
...
public class FafurionRaid {
	...
	private final ArrayList<L1PcInstance> _PaPoolist = new ArrayList<>();
	...
	public void timeOverRun(int type) {
		...
		case -4: // 4th room 2 minute over mob summon
			FafurionMsgTimer room4 = new FafurionMsgTimer(_id, 4);
			GeneralThreadPool.getInstance().execute(room4);
			break;
		case -3:
			FafurionMsgTimer room3 = new FafurionMsgTimer(_id, 3);
			GeneralThreadPool.getInstance().execute(room3);
			break;
		case -2:
			FafurionMsgTimer room2 = new FafurionMsgTimer(_id, 2);
			GeneralThreadPool.getInstance().execute(room2);
			break;
		case -1:
			FafurionMsgTimer room1 = new FafurionMsgTimer(_id, 1);
			GeneralThreadPool.getInstance().execute(room1);
			break;

		case 0: // Tell everyone in the Room
			if (isPaPoo()) {
				FafurionMSG anta1 = new FafurionMSG(_id, 0);
		...
		case 1:// 1차안타 뒤질때 멘트 뿌잉
			 ...
			 */
			// sm = null;
			FafurionMSG anta1 = new FafurionMSG(_id, 2);
			GeneralThreadPool.getInstance().execute(anta1);
			break;
		case 2:// 2차안타 뒤질때 멘트 뿌잉
			 ...
			 */
			// sm2 = null;
			FafurionMSG anta2 = new FafurionMSG(_id, 3);
		...
		case 3:// 3차안타 뒤질때 멘트 뿌잉
			...
			L1World.getInstance().broadcastServerMessage(
					"Dwarf's Cry: 파푸리온의 검은 숨결을 멈추게 한 용사들이 탄생 하였습니다.!!");
			...
			// sm3 = null;
			FafurionMSG anta3 = new FafurionMSG(_id, 4);
			GeneralThreadPool.getInstance().execute(anta3);
			break;

		case 4:// 입장대기
			FafurionMSG anta4 = new FafurionMSG(_id, 1);
	...
	/** 사엘1 오브젝트를 세팅 한다. */
	public void setSael(L1NpcInstance npc) {
		_사엘 = npc;
	}
	...
	/** 사엘2 오브젝트를 세팅 한다. */
	public void setSael2(L1NpcInstance npc) {
		_사엘2 = npc;
	}
	...
	public void 쓰레기정리() {
		for (L1Object obj : L1World.getInstance()
			...
			if (obj instanceof L1NpcInstance) {
				...
				if ((obj instanceof L1DollInstance) || (obj instanceof L1SummonInstance)) {
					continue;
				}
