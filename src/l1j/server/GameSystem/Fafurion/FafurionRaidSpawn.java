...
public class FafurionRaidSpawn {
	private static Logger _log = Logger.getLogger(FafurionRaidSpawn.class.getName());
	private static FafurionRaidSpawn _instance;

	public static FafurionRaidSpawn getInstance() {
		if (_instance == null) {
			_instance = new FafurionRaidSpawn();
		}
		return _instance;
	}

	private FafurionRaidSpawn() {
			...
			while (rs.next()) {
				...
				if (l1npc != null) {
					...
					try {
						...
						if (field.getNpcId() == 4039012 || field.getNpcId() == 4039013) {
						...
						if (field.getNpcId() == 4039000 || field.getNpcId() == 4039006
