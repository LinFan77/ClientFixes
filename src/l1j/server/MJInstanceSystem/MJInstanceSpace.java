...
public class MJInstanceSpace {
	...
	private MJInstanceSpace(){
		_mapQ = new ArrayDeque<>(MJInstanceLoadManager.MIS_COPYMAP_SIZE);
		...
		_objMap = new HashMap<>(MJInstanceLoadManager.MIS_COPYMAP_SIZE);
	}
	...
	public void releaseInstance(MJInstanceObject obj){
		...
		try{
			...
			for (L1Object l1obj : L1World.getInstance().getVisibleObjects(copyMapId).values()) {
			...
			for (L1ItemInstance element : itemArray) {
				item = element;
				if((item == null) || (copyMapId != item.getMapId()))
					continue;
