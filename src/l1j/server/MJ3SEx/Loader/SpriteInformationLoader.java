...
public class SpriteInformationLoader {
	...
	public static int levelToIndex(int level, int sprId) {
		/*
		 * TODO Specify the speed level to uncomment when using so that the same speed comes out regardless of the additional level of event transformation
		 */
	...
	private HashMap<Integer, Long> loadSpellDelayInformation() {
		...
		try {
			...
			while (rs.next())
				sps.put(rs.getInt("skill_id"), rs.getLong("delay"));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public SpriteInformationLoader loadSpriteInformation() {
	    ...
	    try {
	        con = L1DatabaseFactory.getInstance().getConnection();
	        pstm = con.prepareStatement("select spr_id, width, height, action_count from spr_info", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
	        rs = pstm.executeQuery();
	        _sprInfos = new HashMap<>(SQLUtil.calcRows(rs));
	        while (rs.next()) {
	...
	private Integer[] loadExceptionTargetSprite(Document doc) {
		return (Integer[]) MJArrangeParser.parsing(((Element) doc.getElementsByTagName("Sprite").item(0)).getAttribute("target"), ",",
	...
	private void loadExceptionActionRates(Document doc) {
		...
		for (int i = size - 1; i >= 0; --i) {
			Node node = nodes.item(i);
			if ((Node.ELEMENT_NODE != node.getNodeType()) || !node.getNodeName().equalsIgnoreCase("Frame"))
				continue;
