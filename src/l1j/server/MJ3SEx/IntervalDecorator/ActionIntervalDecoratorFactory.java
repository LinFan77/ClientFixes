...
public class ActionIntervalDecoratorFactory {
	...
	public static final ActionIntervalDecorator<L1PcInstance> PC_WALK_DECORATOR = new ActionIntervalDecorator<>(){
		@Override
	...
	public static final ActionIntervalDecorator<L1PcInstance> PC_ATT_DECORATOR = new ActionIntervalDecorator<>(){
		@Override
		public double decoration(L1PcInstance owner, double interval) {
			if(owner.isHaste())
				interval *= HASTE_ACCELERATION;
			...
			if(owner.hasSkillEffect(L1SkillId.FOCUS_WAVE)){
				for(int i=owner.getFocusWave() - 1; i>=0; --i)
					interval *= THIRD_ACCELERATION;
			} else if (owner.hasSkillEffect(L1SkillId.STATUS_FRUIT)) {
				if (owner.isDarkHorse()) {
					interval *= THIRD_ACCELERATION;
				}
			}
	...
	public static final ActionIntervalDecorator<L1Character> CHA_WALK_DECORATOR = new ActionIntervalDecorator<>(){
		@Override
	...
	public static final ActionIntervalDecorator<L1Character> CHA_ATT_DECORATOR = new ActionIntervalDecorator<>(){
		@Override
		public double decoration(L1Character owner, double interval) {
				if(owner.isHaste())
					interval *= HASTE_ACCELERATION;
	...
	public static final ActionIntervalDecorator<L1Character> NULL_DECORATOR = new ActionIntervalDecorator<>(){
		@Override
		public double decoration(L1Character owner, double interval) {
			return interval;
		}
