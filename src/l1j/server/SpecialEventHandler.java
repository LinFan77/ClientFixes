...
// Responsible for handling all events within the game
public class SpecialEventHandler {
	...
	public void giveFeather() {
		...
		try {
			...
			while (r.next()) {
				...
				p1 = c.prepareStatement("UPDATE hongbo SET excutecount = excutecount + '" + r.getInt("remaincount")
						+ "',remaincount = 0 WHERE account = '" + accountName + "'");
				p1.executeUpdate();
			}
		} catch (Exception e) {
		} finally {
	...
	private static final int[] allBuffSkill = {
			L1SkillId.HASTE, L1SkillId.DECREASE_WEIGHT, ADVANCE_SPIRIT,
			// FIRE_WEAPON,
			BLESS_WEAPON, NATURES_TOUCH, L1SkillId.AQUA_PROTECTER, IRON_SKIN,
			L1SkillId.SHINING_SHIELD, L1SkillId.CONCENTRATION, L1SkillId.PATIENCE,
			L1SkillId.INSIGHT, 7895, L1SkillId.REMOVE_CURSE,
			// L1SkillId.IMMUNE_TO_HARM,
			L1SkillId.ILLUSION_OGRE, L1SkillId.ILLUSION_GOLEM, PHYSICAL_ENCHANT_DEX,
			PHYSICAL_ENCHANT_STR, L1SkillId.FEATHER_BUFF_C, 10528 };
		...
		GeneralThreadPool.getInstance().execute(new Runnable() {
			...
			public void run() {
				try {
					...
					for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(gm, -1)) {
						if (pc == null || pc.isPrivateShop()) {
							continue;
						}
						l1skilluse = new L1SkillUse();
						l1skilluse.handleCommands(pc, STATUS_COMA_5, pc.getId(), pc.getX(), pc.getY(), null, 0,
					...
					for (L1NpcShopInstance pc : L1World.getInstance()
							.getAllNpcShop()) {
						if (pc == null || !(pc.getNpcTemplate().get_npcId() >= 8100000
								&& pc.getNpcTemplate().get_npcId() <= 8100002))
							continue;
						L1Skills _skill = SkillsTable.getInstance().getTemplate(STATUS_COMA_5);
						if (_skill != null)
							Broadcaster.broadcastPacket(pc, new S_SkillSound(pc.getId(), _skill.getCastGfx()));
					}
	...
	private static final int[] allBuffComaSkill = {
			...
			// FIRE_WEAPON,
			BLESS_WEAPON, NATURES_TOUCH, L1SkillId.AQUA_PROTECTER, IRON_SKIN,
			L1SkillId.SHINING_SHIELD, L1SkillId.CONCENTRATION,
			L1SkillId.PATIENCE, L1SkillId.INSIGHT,
			7895,
			L1SkillId.REMOVE_CURSE,
			// L1SkillId.IMMUNE_TO_HARM,
			L1SkillId.ILLUSION_OGRE, L1SkillId.ILLUSION_GOLEM,
					...
					for (int element : allBuffComaSkill) {
						for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(gm, -1)) {
							...
							if (element == 7895) {
								...
								if (!pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.ENERGY_OF_BLACK)) {
									pc.getAC().addAc(-2);
									pc.addMaxHp(20);
									pc.addMaxMp(13);
								...
								pc.sendPackets(new S_SkillSound(pc.getId(), 4914), true);
								Broadcaster.broadcastPacket(pc, new S_SkillSound(pc.getId(), 4914));
								pc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.ENERGY_OF_BLACK, 1800 * 1000);
							} else {
								l1skilluse = new L1SkillUse();
								l1skilluse.handleCommands(pc, element, pc.getId(), pc.getX(), pc.getY(), null, 0,
										L1SkillUse.TYPE_GMBUFF);
							}
							// pc.sendPackets(new
							// S_SystemMessage("운영자에게 버프를 받았습니다. "));
						...
						for (L1NpcShopInstance pc : L1World.getInstance().getAllNpcShop()) {
							...
							if (element == 10528) {
								...
								L1Skills _skill = SkillsTable.getInstance().getTemplate(element);
								if (_skill != null)
									Broadcaster.broadcastPacket(pc, new S_SkillSound(pc.getId(), _skill.getCastGfx()));
							}
						}
			...
			public void run() {
				try {
					...
					for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
						...
						l1skilluse = new L1SkillUse();
						l1skilluse.handleCommands(pc, 1025, pc.getId(), pc.getX(), pc.getY(), null, 0,
								L1SkillUse.TYPE_GMBUFF);
					}
					for (L1NpcShopInstance pc : L1World.getInstance().getAllNpcShop()) {
						if (pc == null || !(pc.getNpcTemplate().get_npcId() >= 8100000 && pc
					...
					for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
						...
						pc.sendPackets(new S_SystemMessage("You received a coma buff from a GameMaster."));
					}
				} catch (Exception e) {
				}
			}
			...
			public void run() {
				try {
					...
					for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
						...
						pc.sendPackets(new S_SystemMessage("You have been buffed by the GameMaster."));
					}
	...
	public void doNotChatEveryone() {
		...
		L1World.getInstance().broadcastPacketToAll(new S_SystemMessage("[****] Global chat has been disabled."));
	}

	public void doChatEveryone() {
		L1World.getInstance().set_worldChatElabled(true);
		L1World.getInstance().broadcastPacketToAll(new S_SystemMessage("[****] Global chat has been enabled."));
	}
	...
	public void doMapBuf(L1PcInstance base) {
		...
		L1SkillUse l1skilluse = null;
		for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
			if (pc == null || pc.isPrivateShop() || pc.getMapId() != base.getMapId()) {
				continue;
			}
			l1skilluse = new L1SkillUse();
			for (int element : allBuffSkill) {
