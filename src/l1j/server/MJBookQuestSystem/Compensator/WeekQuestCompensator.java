...
public class WeekQuestCompensator implements QuestCompensator{
	...
	private int 			_ingredientItemId;	// Material item id.
	private ExpCompensator 	_exp;				// An element that will provide reward experience.
	private ItemCompensator _item;				// The element that will provide the reward item.
	...
	public synchronized void serialize() throws Exception{
	    ...
	    try(MJBytesOutputStream mbos = new MJBytesOutputStream()) {
	        mbos.write(0x12);
	        mbos.write(0x10);
	        mbos.write(0x08);
	...
	public void compensate(L1PcInstance pc) {
		if(_ingredientItemId != 0){
			if(!pc.getInventory().checkItem(_ingredientItemId)){
				...
				sb.append(item.getName()).append("does not have.");
