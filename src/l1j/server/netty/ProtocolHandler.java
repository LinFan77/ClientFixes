...
public final class ProtocolHandler extends SimpleChannelUpstreamHandler {
	...
	private static Map<String, IpInfo> list_dos = new HashMap<>();

	private final CopyOnWriteArrayList<LineageClient> _Clientlist = new CopyOnWriteArrayList<>();
	...
	public class sessionCheck extends TimerTask {
		...
		public void run() {
			...
			if (lc != null) {
				if (!lc.packetvirsion && lc.web_login_ok) {
					lc.close();
					lc = null;
				}
			} else {
	...
	public void channelOpen(ChannelHandlerContext ctx, ChannelStateEvent event) throws Exception {
		try {
			...
			if (i <= 0) {
				...
				System.out.println("Close bad connection attempt: 901");
				return;
			}

			if (IpTable.getInstance().isBannedIp(array[0])) {
				String msg = "Incoming connection blocked. Reason: IP banned";
				ErrorMessage.reportConnection(ip, msg);
				//LinAllManager.serverMessageAppend(" [InCON] Blocked because IP is banned. IP: " + array[0]);
				//System.out.println("[Server] Blocked connection because IP is banned. IP: " + array[0]);
				event.getChannel().close();
				return;
			}

			if (isConnectBan(array[0], System.currentTimeMillis())) {
				String msg = "Blocked too many connection attempts.";
				ErrorMessage.reportConnection(ip, msg);
				System.out.println("[Server] Blocked because too many connection attempts. IP: " + array[0]);
				event.getChannel().close();
				return;
			}
			String msg = "Incoming connection...";
			ErrorMessage.reportConnection(ip, msg);
			System.out.println("[Server] Incoming connection...");
			//LinAllManager.serverMessageAppend(" [InCON]IP: " + array[0] + " Thread: " + Thread.activeCount() + " Memory: " + SystemUtil.getUsedMemoryMB() + " MB");
	...
	private boolean isConnectBan(String ip, long time) {
		...
		// Blocked ones are ignored.
		if (IP.isBlock())
			return true;
		// Ignore those who connect more than 3 times within 3 seconds.
