...
public class FaceToFace {
	...
	public static L1PcInstance faceToFace(L1PcInstance pc) {
		...
		if (players.size() == 0) { // If there is no PC within 1 cell
			// pc.sendPackets(new S_ServerMessage(93)); // \f1 There is no one there.
			return null;
		}
		for (L1PcInstance target : players) {
			...
			if (pcHeading == 0 && pcX == targetX && pcY == (targetY + 1)) {
				if (targetHeading == 4) {
					...
					// \f1%0They are not looking at you.
					return null;
				}
