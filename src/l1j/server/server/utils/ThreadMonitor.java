...
public class ThreadMonitor extends Thread {
			...
			try {
				...
				for (int i = 0; i < threadIds.length; i++) {
					...
					if (cpuTime != -1 && tinfos[i] != null) {
						...
						if (sec > 1.0) {
							System.out.println("Thread Name: " + tinfos[i].getThreadName() + "  CPU_TIME: " + sec);
	...
	public void whereami(long id) {
		...
		for (Map.Entry<Thread, StackTraceElement[]> e : st.entrySet()) {
			...
			Thread t = e.getKey();
			if (t.getId() == id) {
				System.out.println("\"" + t.getName() + "\"" + " " + (t.isDaemon() ? "daemon" : "") + " prio="
						+ t.getPriority() + " Thread id=" + t.getId() + " " + t.getState());
