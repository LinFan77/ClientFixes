	...
	public static void spawn(L1PcInstance pc, int npcId, int randomRange,
		...
		try {
			...
			if (npc instanceof L1CastleGuardInstance) {
				((L1CastleGuardInstance) npc).default_heading = pc.getMoveState().getHeading();
			}
			if (isUsePainwand) {
	...
	public static FastTable<L1ArrowInstance> ArrowSpawn() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (l1npc != null) {
					try {
						field = (L1ArrowInstance) NpcTable.getInstance().newNpcInstance(rs.getInt("npc_id"));
	...
	public static L1NpcInstance spawn4(int x, int y, short map, int heading,
		...
		try {
			...
			if (randomRange == 0) {
				npc.getLocation().set(x, y, map);
				/**
				 * Including dragon land~~~ Applied only to non-Hardin-related NPCs.
				 **/
				/*
			...
			if (npc.getNpcId() == 4038000 || npc.getNpcId() == 4200010 || npc.getNpcId() == 4200011) {
				AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(map);
				ar.setanta(npc);
			}
		...
		try {
			...
			if (randomRange == 0) {
				...
				do {
					...
					if ((npc.getX() >= 32936 && npc.getY() <= 32945) && (npc.getY() >= 32861 && npc.getY() <= 32870)
							&& npc.getMapId() == 410)
						continue;
					if (npc.getMap().isInMap(npc.getLocation()) && npc.getMap().isPassable(npc.getLocation())) {
						break;
			...
			if (npcId == 4039004) {
				FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(pc.getMapId());
				ar.setSael(npc);
			}
			if (npcId == 4039005) {
				FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(pc.getMapId());
