...
public class Teleportation {
	...
	public static void doTeleportation(L1PcInstance pc, boolean type) {
		...
		try {
			...
			/** If the map is different, it is separately managed so that the indungeon system information can be processed */
			boolean ServerInter = false;
			int ServerInterType = 0;
			if (mapId != pc.getMapId()) {
				/** If the Telmap is the dominant map and the character map is not the dominant map, the packet is processed by the interserver. */
			...
			/** Check a specific map and if it is that map, change to connect to the interserver */
			if (ServerInter) {
			...
			if ((pc.getMapId() >= 2600 && pc.getMapId() <= 2698) && !(mapId >= 2600 && mapId <= 2698)) {
				...
				if (pc.getInventory().checkItem(7236)) {
					L1ItemInstance item = pc.getInventory().checkEquippedItem(40005);
					if (item != null) {
						pc.getInventory().setEquipped(item, false, false, false);
					}
					pc.getInventory().consumeItem(40005, 1);
				}
			}
			...
			if (pc.isPinkName()) {
				...
				Broadcaster.broadcastPacket(pc, new S_PinkName(pc.getId(), pc.getSkillEffectTimerSet().getSkillEffectTimeSec(L1SkillId.STATUS_PINK_NAME)), true);
			}
						...
						if (pc.getPetList() != null && pc.getPetListSize() > 0) {
							for (L1NpcInstance petNpc : pc.getPetList()) {
								if (petNpc instanceof L1SummonInstance) {
									...
									((L1PetInstance) petNpc).setCurrentPetStatus(5); // boundary
								}
							}
						}
					} catch (Exception e) {}
			...
			if (pc.tempm != pc.getMapId()) {
				...
				if (pc.tempm == 14 && pc.getMapId() == 813) {
					if (pc.getgiranday() == null) {
						...
						pc.save();
						pc.sendPackets(new S_ServerMessage(1526, "3"));// time left.
					}
				}
				...
				} else if (pc.getMapId() >= 282 && pc.getMapId() <= 285) { // ivory tower
					if (pc.getLevel() <= 89) {
						int outtime = 60 * 60 * 2;
...
//					time = outtime - pc.get라던time();
					...
					int outtime = 60 * 60 * 2;
					time = outtime - pc.getGiranPrisonTime();
			...
			if (pc.FIND_MERCH_OBJID != 0) {
				pc.sendPackets(new S_NPCTalkReturn(pc.FIND_MERCH_OBJID, "usershop"), true);
				pc.FIND_MERCH_OBJID = 0;
			}
			...
			// Log recording when entering Yonggye Lucky Area
			// In-situ telecom release can also be entered.
			try {
			...
			System.out.println("Tele Server Error Code 100");
			e.printStackTrace();
