...
//			case 100878: craftlist = new int[] { 577,578,579,580,581 }; break;
	...
	private void buildPacket(int skillId, long time) {
		...
		writeC(0x30);
		writeC(0x00);
		// if(skillId == L1SkillId.블레이징스피릿){ // Blazing Spirit
		// writeC(0x38);
		// writeC(0x00);
		// }
	...
	/** Set up for message links to the indungeon system */
	...
	private void buildPacket(boolean Setting) {
		...
		if(Setting){
			for (int element : MESSAGE_OrimOpen) {
				writeC(0x0a);
				writeC(5 + size7B(MESSAGE_OrimImage) + size7B(element));
			...
			for (int element : MESSAGE_OrimEnd) {
				writeC(0x0a);
				writeC(5 + size7B(MESSAGE_OrimImage) + size7B(element));
