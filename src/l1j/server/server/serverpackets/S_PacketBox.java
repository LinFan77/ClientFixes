...
/**
 * A class of packets used for multiple purposes, such as displaying skill icons or block lists.
 */
public class S_PacketBox extends ServerBasePacket {
	...
	/** Aura skill icon and Erase Magic icon deleted */
	...
	/**
	 * 2 ambiguous packets followed by friendship ↓
	 */
	...
	public static final int DRAGONRAIDBUFF = 179;
	...
	public static final int NUM_STORE_OPEN = 198;
	public static final int USER_BACKSTAFF = 193;
	...
	public S_PacketBox(L1ItemInstance item, int subcode) {
		...
		if (!((item.getItemId() >= 437010 && item.getItemId() <= 437013)
			...
			writeD(7); // warehouse storage available
			/*
	...
	public S_PacketBox(L1PcInstance pc, int subCode) {
		...
		case PLEDGE_TWO:
			...
			ArrayList<ClanMember> clanMemberList = clan.getClanMemberList(); // 모든
																				...
																				// 이름과
																				// 등급
			for (ClanMember element : clanMemberList) {
				member = element;
				writeS(member.name);
				if (member.rank == 5) {
					writeC(3);
