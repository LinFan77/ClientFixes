...
public class S_ACTION_UI extends ServerBasePacket {
	...
	public static final int CRAFT_ITEM = 55; // crafting item
	public static final int CRAFT_ITEMLIST = 57; // crafting list
	public static final int CRAFT_OK = 59; // Complete crafting
	public static final int CRAFT_GAUGEUI = 93;

	public static final int NEWSKILL = 110;
	public static final int DOLL_START = 123; // start
	public static final int DOLL_RESULT = 125;
	public static final int PCBANG_SET = 126;
	public static final int DOLL_READY = 128; // start
	...
	public static final int LIMAWORLDMESSAGE = 592;
	...
	public static final int SUMMON_PET_NOTI = 2321; // Magic doll icon mark
	public static final int PETBALL_CONTENTS_START_ACK = 2383; // At the start of the magic doll
	...
	public S_ACTION_UI(int subType, int time, L1DollInstance doll, L1ItemInstance item, boolean repeat) {
		...
		case SUMMON_PET_NOTI:
			...
			if (item != null) {
				...
				writeBit(stats.length); // item mark length
				writeByte(stats); // Item notation
			}
			break;
		}
		writeH(0);
	...
	/**
	 * Used when setting a party mark.
	 */
	...
	public S_ACTION_UI(int type, L1PcInstance pc) {
		...
		case AINHASAD:
			...
			writeC(0x38);
			writeBit(0x01);
			/** If there is a clan buff, the buff time will be reversed. */
			if (pc.getClan() != null) {
				short mapId = pc.getMapId();
				if (pc.getClan().isHuntMapChoice()) {
					if (pc.getClan().getBlessHuntMapIds().contains((int) mapId)) { // Required blood
						writeC(0x28);
						writeBit(1); // blessing buff
					}
				}
			}
			break;
	...
	/**
	 * PC room, safety zone setting by Hit
	 *
	 * @param isOpen on/off
	 **/
	public S_ACTION_UI(int code, boolean isOpen) {
		...
		case PCBANG_SET: {
			...
			writeC(isOpen ? 1 : 0);
			writeC(0x10);
			writeC(0x01); // Ranking button activation packet
			writeH(0);
			break;
		}
		case SAFETYZONE: {
	...
	public S_ACTION_UI(L1NpcInstance npc) {
		...
		try {
			...
			//Ancient Item Alchemist^Aman
			case 7210073:
				craftlist = new int[] { 2652, 2653 };
				break;

			//Jeweler_Dio completed
			case 70027:
			...
			/** Kent Castle Town, Ibelbin (Breath Enhancer) **/
			case 70662:
				craftlist = new int[] { 5913, 5914, 5915, 5916, 5917, 5918, 5919, 5920, 5921, 5922, //Einhasad's Flash //Halfas Breath Rush
						5933, 5934, 5935, 5936, 5937, 5938, 5939, 5940, 5941, 5942, // Gran Kain's Judgment //Halfas Breath Rush

						5893, 5894, 5895, 5896, 5897, 5898, 5899, 5900, 5901, 5902, // Death's Sword //숨결
			...
			/** Kent Castle Town, Coop (Dragon Maker) **/
			case 70904:
				craftlist = new int[] { 203, 204, 205, 206, 207, 208, 209, 210, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 220, 226, 223,
						219, 225, 222, 221, 224, 216, 215, 217, 218, 70, 71, 72, 73, 74, 75, 76, 77, 78, 103, 104, 105, 106, 107, 108, 109, 6133, 6134 // 지룡,수룡,화룡,풍룡,탄생,형생,생명,흑룡,절대
																																						// 마안
						, 6135, 6136, 6137 // Crafting Halfas Armor
						, 6138, 6139, 6140, 6141, 6142, 6143, 6144, 6145, 6146/** Halfaswank Enchant */
						, 6147, 6148, 6149, 6150, 6151, 6152, 6153, 6154, 6155/** Halfas Foresight Enchantment */
						, 6156, 6157, 6158, 6159, 6160, 6161, 6162, 6163, 6164/** Halfas Magic Enchantment */
			...
			// 3408,3409 Ancient silver reel, ancient golden reel
	...
	public S_ACTION_UI(String clanname, int rank) {
		...
		if (length > 0) {
			writeC(length); // clan name SIZE
			writeByte(clanname.getBytes()); // clan name
			writeC(0x10);
			writeC(rank); // clan rank
		} else {
			writeC(0x00);
		}
		writeH(0x00);
