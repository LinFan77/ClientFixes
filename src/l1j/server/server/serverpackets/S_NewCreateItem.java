...
public class S_NewCreateItem extends ServerBasePacket {
	...
	public S_NewCreateItem(L1PcInstance pc, L1Object obj) {
		try {
			...
			writeC(0x12);
			writeC(0x07);
			FastTable<Integer> ck = new FastTable<>();
			if (obj instanceof L1NpcInstance) {
				L1NpcInstance npc = (L1NpcInstance) obj;
	...
	public S_NewCreateItem(String name, L1PcInstance pc) {
		...
		if (name.equalsIgnoreCase("무게")) {
			int maxCarryWeight = pc.getMaxWeight();
	...
	public S_NewCreateItem(int type, int 상태, int stat, int stat2, String name,
		...
		if (name.equalsIgnoreCase("힘")) {
			...
			int meleeDamage = CalcStat.calcDamageFromSTR(stat);
			int meleeHit = CalcStat.calcMeleeHitFromSTR(stat);
			int meleeCrit = CalcStat.meleeCrit(stat);
			int maxCarryWeight = CalcStat.maxCarryWeight(stat, stat2);
			...
			byte[] minus = { (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0x01 };
			int rangedDamage = CalcStat.calcDamageFromDEX(stat);
			int rangedHit = CalcStat.calcRangeHitUp(stat);
			int rangedCrit = CalcStat.calcRangeCrit(stat);
			int physicalDefense = CalcStat.calcACFromDEX(stat);
			int rangedAvoidance = CalcStat.calcRangedEvasion(stat);
			...
			writeC(0x20);
			writeC(physicalDefense);
			if (physicalDefense < 0)
				writeByte(minus);

			writeC(0x28);
			writeC(rangedAvoidance);
			if (rangedAvoidance < 0)
				writeByte(minus);
			...
			byte[] minus = { (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0x01 };
			int recoveryTick = CalcStat.hpRecoveryTick(stat);
			int increasePotionRecovery = CalcStat.hpPotionRecovery(stat);
			int maxCarryWeight = CalcStat.maxCarryWeight(stat, stat2);
			int levelUpIncrease = CalcStat.levelUpIncrease(classtype, stat);
			...
			if (increasePotionRecovery < 0) {
				len += 9;
			}
			if (levelUpIncrease < 0) {
				len += 9;
			}
			...
			byte[] minus = { (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0x01 };
			int magicDamage = CalcStat.calcMagicDmg(stat);
			int magicHit = CalcStat.magicHit(stat);
			int magicCrit = CalcStat.magicCrit(stat);
			int magicBonus = CalcStat.calcMagicBonus(stat);
			int mpCostReduction = CalcStat.calcMPCostReduction(stat);
			...
			byte[] minus = { (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff,
					(byte) 0xff, (byte) 0x01 };
			int 엠회복틱 = CalcStat.mpRecoveryTick(stat);
			int 엠피물약회복 = CalcStat.mpPotionRecovery(stat);
			int 마법방어 = CalcStat.calcMagicResistance(classtype, stat);
			int[] 레벨업엠피증가량 = CalcStat.levelIncreaseAmountMP(classtype, stat);
			int hp = 0;
			if (stat >= 45) {
				hp = 300;
			} else if (stat >= 35) {
