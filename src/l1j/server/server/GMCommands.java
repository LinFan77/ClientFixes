...
public class GMCommands {
	...
	public static boolean juicyIPCheck = false;
	public static boolean 무인상점구매체크 = true;
	public static boolean 케릭인증영자방 = false;
	public static boolean 트리플포우스핵 = false;
	public static boolean logDrop = true;
	public static boolean logDissolve = true;
	public static boolean logPickup = true;
	public static boolean checkConnectionName = false;
	...
	private boolean executeDatabaseCommand(L1PcInstance pc, String name, String arg) {
		try {
			...
			if (pc.getAccessLevel() < command.getLevel()) {
				pc.sendPackets(new S_ServerMessage(pc.getLanguage(), 74, "Command " + name), true); // \f1%0은 사용할 수 없습니다.
				return true;
			}
	...
	private static int halloweenList[] = { 90085, 90086, 90087, 90088, 90089, 90090, 90091, 90092, 160423, 435000, 160510, 160511, 21123, 21269 };
	private static int halloweenList2[] = { 90085, 90086, 90087, 90088, 90089, 90090, 90091, 90092, 160423, 435000, 160510, 160511, 21123 };
	...
	public synchronized static void EventItemDelete() {
		try {
			for (L1PcInstance tempPc : L1World.getInstance().getAllPlayers()) {
				...
				for (int element : halloweenList) {
					L1ItemInstance[] item = tempPc.getInventory().findItemsId(element);

					if (item != null && item.length > 0) {
						for (L1ItemInstance element2 : item) {
							if (element2.getItemId() == 21269) {
								if (element2.getEnchantLevel() >= 6) {
									if (element2.getBless() >= 128) {
										element2.setBless(128);
									} else {
										element2.setBless(0);
									}

									tempPc.getInventory().updateItem(element2, L1PcInventory.COL_BLESS);
									tempPc.getInventory().saveItem(element2, L1PcInventory.COL_BLESS);
									continue;
								}
							}

							if (element2.isEquipped()) {
								tempPc.getInventory().setEquipped(element2, false); // please add
							}

							tempPc.getInventory().removeItem(element2);
						}
					}

					try {
						PrivateWarehouse pw = WarehouseManager.getInstance().getPersonalStorage(tempPc.getAccountName());
						L1ItemInstance[] item2 = pw.findItemsId(element);
					...
					try {
						if (tempPc.getClanid() > 0) {
							ClanWarehouse cw = WarehouseManager.getInstance().getPledgeStorage(tempPc.getClanname());
							L1ItemInstance[] item3 = cw.findItemsId(element);
							if (item3 != null && item3.length > 0) {
								for (L1ItemInstance element2 : item3) {
									if (element2.getItemId() == 21269) {
										if (element2.getEnchantLevel() >= 6) {
											if (element2.getBless() >= 128) {
												element2.setBless(128);
					...
					try {
						...
						if (pList.size() > 0) {
							for (L1NpcInstance npc : pList) {
								L1ItemInstance[] pitem = npc.getInventory().findItemsId(element);
								if (pitem != null && pitem.length > 0) {
									for (L1ItemInstance element2 : pitem) {
										if (element2.getItemId() == 21269) {
											if (element2.getEnchantLevel() >= 6) {
												if (element2.getBless() >= 128) {
													element2.setBless(128);
										...
										npc.getInventory().removeItem(element2);
									}
								}
							}
						}
			...
			try {
				for (L1Object obj : L1World.getInstance().getAllItem()) {
					...
					for (int element : halloweenList) {
						if (element == temp_item.getItemId()) {
			...
			for (int i = 0; i < halloweenList2.length; i++) {
				sb.append(+halloweenList2[i]);
				if (i < halloweenList2.length - 1) {
					sb.append(",");
				}
			}
			Delete(sb.toString());
	...
	public void handleCommands(L1PcInstance gm, String cmdLine) {
        //System.out.println("Handling GM command");

		StringTokenizer token = new StringTokenizer(cmdLine);
		// The first space up to the first space is treated as a command, and the space after that is treated as a parameter separated by a space.
		...
		if (executeDatabaseCommand(gm, cmd, param)) {
			System.out.println("GM command confirmed");
		...
		System.out.println("GM Command not executed...");
		...
		// The command to open to the GM is written here
		if (cmd.equalsIgnoreCase("help")) {
		...
		} else if (cmd.equalsIgnoreCase("servershutdown")) {
			serverShutdown(gm, param);
			...
			GUILTY_CHECK = !GUILTY_CHECK;
			gm.sendPackets(new S_SystemMessage("길팅체크 : " + GUILTY_CHECK), true);
		} else if (cmd.equalsIgnoreCase("루팅체크")) {
			routingCheck = !routingCheck;
			gm.sendPackets(new S_SystemMessage("루팅체크 : " + routingCheck), true);
			...
			juicyIPCheck = !juicyIPCheck;
			gm.sendPackets(new S_SystemMessage("아이피체크 : " + juicyIPCheck), true);
		...
		} else if (cmd.equalsIgnoreCase("dissolvelog")) {
			logDissolve = !logDissolve;
			gm.sendPackets(new S_SystemMessage("Dissolve Log: " + logDissolve), true);
			...
			checkConnectionName = !checkConnectionName;
			gm.sendPackets(new S_SystemMessage("접속이름체크 : " + checkConnectionName), true);
	...
	public void killment(L1PcInstance pc, String param) { // by사부 멘트
		if (param.equalsIgnoreCase("off")) {
			pc.킬멘트 = false;
			pc.sendPackets(new S_SystemMessage("Doesn't show killment."));
		} else if (param.equalsIgnoreCase("on")) {
			pc.킬멘트 = true;
			pc.sendPackets(new S_SystemMessage("Show killment."));

		} else {
			pc.sendPackets(new S_SystemMessage(".killment [on/off] 으로 입력해 주세요. "));
		}
	}
	...
	private void charname(L1PcInstance pc, String cmd) {
		try {
			...
			if (pc.getClanid() > 0) {
				pc.sendPackets(new S_SystemMessage("\\fUYou can change your character name after leaving the clan."));
				return;
			}
			if (!pc.getInventory().checkItem(467009, 1)) { // check for
				pc.sendPackets(new S_SystemMessage("\\fUYou must have a secret book to change your character name."));
				return;
			}
			for (int i = 0; i < chaName.length(); i++) {
				if (chaName.charAt(i) == 'ㄱ' || chaName.charAt(i) == 'ㄲ' || chaName.charAt(i) == 'ㄴ' || chaName.charAt(i) == 'ㄷ' || // 한문자(char)단위로 비교.
					...
					pc.sendPackets(new S_SystemMessage("This character name cannot be used."));
					return;
				}
			}
			if (chaName.getBytes().length > 12) {
				pc.sendPackets(new S_SystemMessage("Your name is too long."));
				return;
			}
			if (chaName.length() == 0) {
			...
			if (BadNamesList.getInstance().isBadName(chaName) || isInvalidName(chaName)) {
	...
	private void 드랍추가(L1PcInstance gm, String param) {
		...
		boolean ck = DropTable.SabuDrop(gm._npcnum, itemid, 1, 1000000, gm._npcname, temp.getName(), gm._note);
		if (ck) {
	...
	private void autoIpcheck(L1PcInstance gm, String param) {
		try {
			...
			FastTable<ipAcc> list = new FastTable<>();
			for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
	...
	class ipAcc {
		String ip = "";
		FastTable<String> namelist = new FastTable<>();
	}
	...
	public static boolean GUILTY_CHECK = true;
	public static boolean routingCheck = false;
	...
	public static boolean log_Drop = false;
	public static boolean log_Pickup = false;
	public static boolean log_Dissolve = true;
	public static boolean log_Shop = true;
	public static boolean log_PShop = true;
	public static boolean log_Enchant = true;

	public static boolean logWhisper = true;
	public static boolean logTrade = true;
	public static boolean logSystem = true;
	public static boolean logConnection = true;
	...
	private void logSwitch(L1PcInstance gm, String param) {
		try {
			...
			if (type.equalsIgnoreCase("드랍")) {
				log_Drop = !log_Drop;
				gm.sendPackets(new S_SystemMessage("로그 드랍 : " + log_Drop), true);
			} else if (type.equalsIgnoreCase("픽업")) {
				log_Pickup = !log_Pickup;
				gm.sendPackets(new S_SystemMessage("로그 픽업 : " + log_Pickup), true);
			} else if (type.equalsIgnoreCase("용해")) {
				log_Dissolve = !log_Dissolve;
				gm.sendPackets(new S_SystemMessage("로그 용해 : " + log_Dissolve), true);
			} else if (type.equalsIgnoreCase("상점")) {
				log_Shop = !log_Shop;
				gm.sendPackets(new S_SystemMessage("로그 상점 : " + log_Shop), true);
			} else if (type.equalsIgnoreCase("개인상점")) {
				log_PShop = !log_PShop;
				gm.sendPackets(new S_SystemMessage("로그 개인상점 : " + log_PShop), true);
			} else if (type.equalsIgnoreCase("교환")) {
				logTrade = !logTrade;
				gm.sendPackets(new S_SystemMessage("로그 교환 : " + logTrade), true);
			} else if (type.equalsIgnoreCase("인첸")) {
				log_Enchant = !log_Enchant;
				gm.sendPackets(new S_SystemMessage("로그 인첸 : " + log_Enchant), true);
			}
		} catch (Exception e) {
	...
	private void 조작(L1PcInstance gm, String param) {
		try {
			...
			switch (DogFightName) {
			case "auto":
				DogFight.getInstance().aTotalFabrication(gm, "auto");
				break;
			...
			gm.sendPackets(new S_SystemMessage(".operation 자동, 정보 , 투견이름(예 : 블루 해츨링 .조작 블루"), true);
		}
	}

	private void portChange(L1PcInstance gm, String param) {
		try {
			/*
			 ...
			 */
			gm.sendPackets(new S_SystemMessage("Port change failed."));
		} catch (Exception e) {
			gm.sendPackets(new S_SystemMessage(".portchange [num]"));
		}
	}
		...
		try {
			...
			if (user != null) {
				...
				if (user instanceof L1RobotInstance) {
					...
					user.감옥 = false;
					L1Teleport.teleport(user, 33437, 32812, (short) 4, 5, true); // The point to go (the point where the user will drop off)
					// user.setTelType(77);
	...
	private void showHelp(L1PcInstance gm) {
		gm.sendPackets(new S_SystemMessage("[shutdown] [time] [랭킹갱신] [킬멘트] [자동압류] [판매상점] [악영오픈] [악영종료] [지배오픈] [지배종료]"), true);
	}

	private void 메모리반환(L1PcInstance gm) {
		System.out.println("Force garbage processing.");
		System.gc();
		System.out.println("Memory cleanup is complete.");
	}
	...
	private void 폰인증시작(L1PcInstance gm) {
		try {
			Config.phoneAuthentication = true;
	...
	private void 폰인증종료(L1PcInstance gm) {
		try {
			Config.phoneAuthentication = false;
	...
	private void serverShutdown(L1PcInstance gm, String param) {
		try {
			StringTokenizer tokenizer = new StringTokenizer(param);
	...
	private void SetEnchantRates(L1PcInstance gm, String param) {
		try {
			...
			if (NumberChk(weapon) && NumberChk(armor) && NumberChk(accessory)) {
				gm.sendPackets(new S_SystemMessage("Armour rates no longer used."), true);
				Config.ENCHANT_CHANCE_WEAPON = Integer.parseInt(weapon);
				//Config.ENCHANT_CHANCE_ARMOR = Integer.parseInt(armor);
				gm.sendPackets(new S_SystemMessage("[Weapon Enchant Rate: " + weapon + "%]"), true);
				//gm.sendPackets(new S_SystemMessage("[방어구 인챈율 = " + armor + " 배]"), true);
			} else { // If the entered value is not a number
				gm.sendPackets(new S_SystemMessage("잘못된 값을 입력하셨습니다. 모두 숫자로 입력해주세요"), true);
			}
		} catch (Exception e) {
	...
	private void chainfo(L1PcInstance gm, String param) {
		try {
			...
			PcName = pc.getName();
			lvl = pc.getLevel();
			exp = ExpTable.getExpPercentage(lvl, pc.getExp());
			hp = pc.getMaxHp();
			mp = pc.getMaxMp();
			ac = pc.getAC().getAc();
	...
	private void StartWar(L1PcInstance gm, String param) {
		try {
			...
			for (L1War war : L1World.getInstance().getWarList()) {
				if (war.CheckClanInSameWar(clan_name1, clan_name2)) {
	...
	private void searchObject(L1PcInstance gm, int type, String name) {
		...
		try {
			...
			try {
				switch (type) {
				case 0: // etcitem
					statement = con.prepareStatement("select item_id, name from etcitem where " + strQueryName + " Like '" + name + "'");
					break;
				case 1: // weapon
					statement = con.prepareStatement("select item_id, name from weapon where " + strQueryName + " Like '" + name + "'");
					break;
				case 2: // armor
					statement = con.prepareStatement("select item_id, name from armor where " + strQueryName + " Like '" + name + "'");
					break;
				case 3: // npc
					statement = con.prepareStatement("select npcid, name from npc where " + strQueryName + " Like '" + name + "'");
					break;
				case 4: // polymorphs
	...
	private void 데스구라(L1PcInstance gm) { // 데스
		...
		L1World.getInstance().broadcastPacketToAll(new S_ACTION_UI(S_ACTION_UI.LIMAWORLDMESSAGE, 4433, item), true);
	}
	...
	private void 패킷9(L1PcInstance gm) { // 데스
		L1PcInstance pc = gm;
		pc.sendPackets(new S_NewSkillIcons(L1SkillId.BLOWATTACK, true, 128));
	}

	private void 패킷10(L1PcInstance gm) { // 데스
		L1PcInstance pc = gm;
		pc.sendPackets(new S_NewSkillIcons(L1SkillId.FOCUS_SPIRIT, true, 128));
	}
	...
	private void 패킷13(L1PcInstance gm) { // 데스
		L1PcInstance pc = gm;
		pc.sendPackets(new S_NewSkillIcons(L1SkillId.TITAN_RISING, true, 128));
	}
	...
	private void 패킷22(L1PcInstance gm) { // 데스
		L1PcInstance pc = gm;
		pc.sendPackets(new S_NewSkillIcons(L1SkillId.GRACE_AVATAR, true, 128));
	}

	private void 패킷23(L1PcInstance gm) { // 데스
		L1PcInstance pc = gm;
		pc.sendPackets(new S_NewSkillIcons(L1SkillId.ABSOLUTE_BLADE, true, 128));
	}

	private void 패킷24(L1PcInstance gm) { // 데스
		L1PcInstance pc = gm;
		pc.sendPackets(new S_NewSkillIcons(L1SkillId.DEATH_HEAL, true, 128));
	}
	...
	private void 화면스폰(L1PcInstance pc, String param) {
		try {
			...
			for (int i = 0; i < id; i++) {
				...
				L1EffectSpawn.getInstance().spawnEffect(91164, 2000, pc.getX() - 15 + rnd, pc.getY() - 15 + rnd2, pc.getMapId());
			}
		} catch (Exception exception) {
	...
	/*private void 로봇채팅(L1PcInstance pc, String param) {
		try {
			...
			if (Robot != null) {
				...
				pc.sendPackets(new S_SystemMessage(Name + " does not exist in the world."), true);
		} catch (Exception e) {
