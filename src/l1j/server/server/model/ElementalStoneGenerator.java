...
public class ElementalStoneGenerator implements Runnable {
	...
	private ArrayList<L1GroundInventory> _itemList = new ArrayList<>(
			MAX_COUNT);
	...
	private boolean canPut(L1Location loc) {
		...
		// 가시 범위의 플레이어 체크
		if ((L1World.getInstance().getVisiblePlayer(_dummy).size() > 0) || !_dummy.getMap().isInMap(_dummy.getX(), _dummy.getY())
