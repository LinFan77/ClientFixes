...
public class L1BugBearRace {
	...
	public final ArrayList<L1LittleBugInstance> _littleBug = new ArrayList<>();

	private final ArrayList<L1NpcInstance> _merchant = new ArrayList<>();
	...
	public static boolean racing_im = false; // Was the race halfway through?
	...
	private L1BugBearRace() {
		...
		System.out.print("[BugBearRaces] loading...");
	...
	private void clearBug() {
		try {
			ArrayList<L1LittleBugInstance> clonn = new ArrayList<>();
	...
	private void calcDividend() {
		...
		for (int i = 0; i < 5; i++) {
			if (_betting[i] == 0) {
				...
				dividend[i] = (allBetting / _betting[i]) * (GMCommands.환수율 * 0.01f);
				if (dividend[i] < 1) {
					dividend[i] = 1f;
				}
			...
			if (!_goal && getBugRaceStatus() == STATUS_PLAYING) {
				...
				for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
					...
					pc.sendPackets(new S_SystemMessage(" 총티켓: " + (long) allBetting + " 우승티켓: " + (long) (_betting[i] * bug.getDividend() * 500) + " 차액: "
							+ (allBetting - (long) (_betting[i] * bug.getDividend() * 500))), true);
				}
	...
	class L1ReadyThread implements Runnable {
		...
		public void run() {
			try {
				buyTickets = true;
				broadCastTime("Race ticket sales have begun.");
				...
				buyTickets = false;
				broadCastTime("Race ticket sales have ended.");
	...
	class L1BroadCastDividend implements Runnable {
		...
		public L1BroadCastDividend() {
			for (L1NpcInstance npc : _merchant) {
				if (npc.getNpcTemplate().get_npcId() == 70041) { // perkin
					_npc = npc;
				}
			}
		}
	...
	class L1BugBearRacing implements Runnable {
		...
		public void run() {
			try {
				...
				// The point of return to the virtual robot
