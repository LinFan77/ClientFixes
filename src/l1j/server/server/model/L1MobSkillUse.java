...
public class L1MobSkillUse {
	...
	private boolean useSkill(int i) {
		...
		// Lind, Sandworm, Erzabe
		if ((_attacker.getNpcId() >= 100012 && _attacker.getNpcId() <= 100014)
				|| _attacker.getNpcId() == 100420 || _attacker.getNpcId() == 100338
						...
						if (i == 2) {
							for (L1PcInstance cha : L1World.getInstance().getVisiblePlayer(_attacker, 8)) {
								if (cha != null && !cha.isGhost() && !cha.isDead()) {
									...
									su.handleCommands(cha, 44, cha.getId(), cha.getX(), cha.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
									su = null;
								}
							}
						}
					}
					GeneralThreadPool.getInstance().schedule(new delayUseSkill(i, _target), 3000);
					return false;
				}
			}
		}
	...
	private void AddSkill(int i) {
		...
		case 100002: // Ugnus area cancel
			...
			break;
		case 46025: // Taros Greater Death Hill
			if (i == 0) {
				skillUse(10, 73);
			}else if (i == 2){
				skillUse(3, 87);
		...
		case 910075:
			if (i == 2) {
				L1SkillUse su = new L1SkillUse();
				su.handleCommands((L1PcInstance) _target, 15028, _target.getId(), _target.getX(), _target.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
				su = null;
			}
			break;
		case 45673: // Reaper Grim Reaper AoE Stun
			if (i == 5) { // Desperado
				L1SkillUse su = new L1SkillUse();
				su.handleCommands((L1PcInstance) _target, 230, _target.getId(), _target.getX(), _target.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
				su = null;
			} else
				if (i == 6) {
					skillUse(15, 87);
		...
		case 45672: // lich
			if (i == 0)
				...
				Broadcaster.broadcastPacket(_attacker, new S_SkillSound(_attacker.getId(), 761), true);
			break;
		case 45618: // 나발
			if (i == 1) {
				...
				GeneralThreadPool.getInstance().schedule(new Runnable() {
					@Override
					public void run() {
						_attacker.getSkillEffectTimerSet().setSkillEffect(L1SkillId.COUNTER_BARRIER, 6000 * 1000);
						Broadcaster.broadcastPacket(_attacker, new S_SkillSound(_attacker.getId(), 5832), true);
					}
			...
			if (i == 2) { // 포그
				for (L1PcInstance cha : L1World.getInstance().getVisiblePlayer(
					...
					if (cha != null) {// new L1SkillUse().handleCommands(cha,
						...
						L1SkillUse su = new L1SkillUse();
						su.handleCommands(null, 66, cha.getId(), cha.getX(), cha.getY(), null, 0, L1SkillUse.TYPE_GMBUFF, _attacker);
						su = null;
					}
				}
			} else if (i == 3) { // 전체스턴
		...
		case 45606: // 공포의 뱀파이어
			if (i == 2) {
				for (L1PcInstance cha : L1World.getInstance().getVisiblePlayer(
						_attacker, 10)) {
					if (cha != null)
						L1PolyMorph.doPoly(cha, 2885, 100, L1PolyMorph.MORPH_BY_NPC);
					}
				}
			break;
		case 45547: // 불신의 시어
		...
		case 45394: // 흑장로
			if (i == 2) {
				_target.getSkillEffectTimerSet().setSkillEffect(10518, 12 * 1000);
			}
			break;
		case 45545: // 흑장로
			if (i == 3) {
				SendGfx(3, 7977);
				Broadcaster.broadcastPacket(_attacker, new S_SkillSound(_attacker.getId(), 4849), true);
				Broadcaster.broadcastPacket(_attacker, new S_SkillSound(_attacker.getId(), 2552), true);
			} else if (i == 4) {
				...
				for (L1PcInstance cha : L1World.getInstance().getVisiblePlayer(
						_attacker, 6)) {
					if (cha != null) {
						L1PcInstance target = cha;
						target.getSkillEffectTimerSet().removeSkillEffect(10518);
						target.getSkillEffectTimerSet().setSkillEffect(10518, 12 * 1000);
						Broadcaster.broadcastPacket(_attacker, new S_SkillSound(cha.getId(), 7780), true);
					}
				}
			}
			if (12 > _rnd.nextInt(100)) {
				for (L1PcInstance cha : L1World.getInstance().getVisiblePlayer(_attacker, 10)) {
					if (cha != null && cha.getResistance().getMr() < 148) {
						L1SkillUse su = new L1SkillUse();
						su.handleCommands(cha, 44, cha.getId(), cha.getX(), cha.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
						su = null;
		...
		case 100030: // captain of the guard
			...
			if (_target.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.FOG_OF_SLEEPING))
				_target.getSkillEffectTimerSet().killSkillEffectTimer(L1SkillId.FOG_OF_SLEEPING);
			_target.getSkillEffectTimerSet().setSkillEffect(L1SkillId.FOG_OF_SLEEPING, 32 * 1000);
			Broadcaster.broadcastPacket(_attacker, new S_SkillSound(_target.getId(), 7467), true);
			break;
		case 100035: // 옛 늑인
			if (i == 0) {
				Broadcaster.broadcastPacket(_attacker, new S_SkillSound(_target.getId(), 6319), true);
			}
			break;
		...
		case 100036: // 옛 커츠
			if (i == 2) {
				...
				skillUse(6, 10518);
				for (L1PcInstance cha : L1World.getInstance().getVisiblePlayer(_attacker, 6)) {
					if (cha != null) {
						L1PcInstance target = cha;
						target.getSkillEffectTimerSet().removeSkillEffect(10518);
						target.getSkillEffectTimerSet().setSkillEffect(10518, 12 * 1000);
		...
		case 45529: // 드레이크
			if (i == 1) {
				ArrayList<L1PcInstance> tempList = new ArrayList<>();
				for (L1PcInstance cha : L1World.getInstance().getVisiblePlayer(_attacker, 10)) {
					if (cha != null) {
				...
				for (L1PcInstance cha : tempList) {
					boolean canUseSkill = skillUse.checkUseSkill(null, 40034, cha.getId(), cha.getX(), cha.getY(), null, 0, L1SkillUse.TYPE_NORMAL, _attacker);
					if (canUseSkill) {
						if (getMobSkillTemplate().getLeverage(i) > 0) {
							skillUse.setLeverage(getMobSkillTemplate().getLeverage(i));
						}
						skillUse.handleCommands(null, 40034, cha.getId(), cha.getX(), cha.getY(), null, 0, L1SkillUse.TYPE_LOGIN, _attacker);
						Broadcaster.broadcastPacket(_attacker, new S_SkillSound(cha.getId(), 762), true);
					}
				}
				skillUse = null;
				if (tempList.size() > 0)
		...
		case 100719: // 제로스
			if (i == 2) {
				...
				_attacker.getSkillEffectTimerSet().setSkillEffect(L1SkillId.ZERO_REDUC, 60 * 1000);
			}
	...
	private ArrayList<L1PcInstance> AreaBoxList(int width, int height) {
		...
		for (L1Object obj : L1World.getInstance()
			...
			if (!CharPosUtil.isAreaAttack(_attacker, cha.getX(), cha.getY(),
					cha.getMapId()) || !CharPosUtil.isAreaAttack(cha, _attacker.getX(),
	...
	private void runskill(int num) {
		...
		{ // 안타라스(리뉴얼) - 용언 스킬1
			...
			if (!_attacker.skilluse) {
				...
				FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
						_attacker.getMapId());
				FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 6, 2000);// 2초
				antaendtime.begin();
			} else {
				return;
			}
		...
		case PAPOO_SKILL2: { // 리오타! 라나 폰폰 켄슬+오른손 연속 공격
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 7,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL3: { // 리오타! 레포 폰폰 웨폰+왼손 연속 공격
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 8,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL4: { // 리오타! 테나 론디르 꼬리연속+아이스 브레스
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 9,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL5: { // 리오타! 네나 론디르 머리연속+아이스브레스
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 10,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL6: { // 리오타! 라나 오이므 데스포션 + 오른손 + 아이스이럽
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 15,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL7: { // 리오타! 레포 오이므 데스포션+오른손+고함
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 16,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL8: { // 리오타! 테나 웨인라크 데스힐+꼬리+아이스 브레스
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 13,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL9: { // 리오타! 레나 우누스 리듀스힐+머리공격+아이스브레스
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 14,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
		...
		case PAPOO_SKILL10: { // 리오타! 오니즈 웨인라크 범위캔슬+데스힐+아이스미티어+아이스이럽
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 11,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
			...
			if (_attacker instanceof L1NpcInstance) {
				if (!_attacker.skilluse) {
					...
					FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(
							_attacker.getMapId());
					_attacker.skilluse = true;
					FafurionTimer antaendtime = new FafurionTimer(_attacker, ar, 12,
							2000);// 2초
					antaendtime.begin();
				} else {
					return;
	...
	private boolean poly(int idx) {
		...
		for (L1PcInstance pc : L1World.getInstance()
			...
			if (pc.isDead() || pc.isGhost() || pc.isGmInvis() || !CharPosUtil.isAreaAttack(_attacker, pc.getX(), pc.getY(),
					pc.getMapId())) {
				continue;
			}
	...
	private boolean physicalAttack(int idx) {
		...
		if ((_attacker.getLocation().getTileLineDistance(_target.getLocation()) > range) || !CharPosUtil.isAreaAttack(_attacker, _target.getX(),
				_target.getY(), _target.getMapId()) || !CharPosUtil.isAreaAttack(_target, _attacker.getX(),
		...
		case L1MobSkill.CHANGE_TARGET_RANDOM:
			...
			for (L1Object obj : L1World.getInstance().getVisibleObjects(
				...
				if (obj instanceof L1PcInstance || obj instanceof L1PetInstance
					...
					if (!getMobSkillTemplate().isTriggerDistance(idx, distance) || !CharPosUtil.isAreaAttack(_attacker, cha.getX(),
							cha.getY(), cha.getMapId()) || !CharPosUtil.isAreaAttack(cha, _attacker.getX(),
							_attacker.getY(), _attacker.getMapId()) || !_attacker.getHateList().containsKey(cha)) {
						continue;
					}
	...
	class delayUseSkill implements Runnable {
		...
		public void run() {
			...
			synchronized (_attacker.synchObject) {
				if (_attacker.isDead() || _attacker._destroyed || _attacker.isParalyzed())
					return;
				try {
					_attacker.setParalyzed(true);
					int skillid = getMobSkillTemplate().getSkillId(skillNum);
					L1Skills skill = SkillsTable.getInstance().getTemplate(skillid);
					if (magicAttack(skillNum, target)) {
						skillUseCountUp(skillNum);
						AddSkill(skillNum);
						int time = SprTable.getInstance().getAttackSpeed(_attacker.getNpcTemplate().get_gfxid(), skill.getActionId());
						if (time == 0)
							time = SprTable.getInstance().getMoveSpeed(_attacker.getNpcTemplate().get_gfxid(), skill.getActionId());
						if (time == 0) {
							if (skill.getActionId() == 18)
								time = SprTable.getInstance().getDirSpellSpeed(_attacker.getNpcTemplate().get_gfxid());
							else if (skill.getActionId() == 19)
								time = SprTable.getInstance().getNodirSpellSpeed(_attacker.getNpcTemplate().get_gfxid());
						}
						if (time > 0) {
							Thread.sleep(_attacker.calcSleepTime(getSleepTime(), 8));
						}
					}
				} catch (Exception e) {}
	...
	private boolean KirMeteorAttack(int idx) {
		try {
			Broadcaster.broadcastPacket(_attacker, new S_EffectLocation(_target.getX(), _target.getY(), (short) 11469), true);
			GeneralThreadPool.getInstance().schedule(new KirMeteorThread(idx, _target.getX(), _target.getY()), 5500);
			// Broadcaster.broadcastPacket(_attacker, new
			// S_DoActionGFX(_attacker.getId(),
			// getMobSkillTemplate().getActid(idx)), true);
			Broadcaster.broadcastPacket(_attacker, new S_NpcChatPacket(_attacker, "voice_kirtas_12", 20), true);
		...
		public void run() {
			...
			synchronized (_attacker.synchObject) {
				if (_attacker.isDead()
						...
						|| (_attacker.getActionStatus() == 0 && _attacker.isParalyzed()))
					return;
				try {
					...
					Broadcaster.broadcastPacket(_attacker, new S_DoActionGFX(_attacker.getId(), 18), true);
					Broadcaster.broadcastPacket(_attacker, new S_EffectLocation(x, y, (short) 11473), true);
					...
					for (L1Object obj : list) {
						if (obj instanceof L1PcInstance) {
							...
							_magic.commit(dmg, 0, 0);
							if (dmg > 0 && !pc.isDead()) {
								pc.sendPackets(new S_DoActionGFX(pc.getId(), ActionCodes.ACTION_Damage), true);
								Broadcaster.broadcastPacket(pc, new S_DoActionGFX(pc.getId(), ActionCodes.ACTION_Damage), true);
							}
							_magic = null;
						}
					}
