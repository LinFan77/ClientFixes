...
public class L1CurseParalysis extends L1Paralysis {
	...
	private class ParalysisTimer extends Thread {
		...
		public void run() {
			try {
				...
				try {
					if (_target instanceof L1PcInstance)
						((L1PcInstance) _target).sendPackets(new S_PacketBox(S_PacketBox.커스종류, 2, _time / 1000), true);
					Thread.sleep(_time);
				} catch (InterruptedException e) {
					if (_target instanceof L1PcInstance)
						((L1PcInstance) _target).sendPackets(new S_PacketBox(S_PacketBox.커스종류, 2, 0), true);
				}
	...
	public static boolean curse(L1Character cha, int delay, int time) {
		if (!(cha instanceof L1PcInstance || cha instanceof L1MonsterInstance) || cha.getSkillEffectTimerSet().hasSkillEffect(
				L1SkillId.STATUS_CURSE_PARALYZING) || cha.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_CURSE_PARALYZED)) {
			return false;
		}
