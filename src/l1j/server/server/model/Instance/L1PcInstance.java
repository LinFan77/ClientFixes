...
import static l1j.server.server.model.skill.L1SkillId.SHADOW_STAFF;
...
import l1j.server.server.model.FafurionBlessing;
...
public class L1PcInstance extends L1Character {
	...
	public boolean ARMOUR_BREAK_DESTINY = false;
	public boolean DOUBLE_BREAK_DESTINY = false;
	public boolean COUNTER_BARRIER_VETERAN = false;
	public boolean REDUCTION_ARMOUR_VETERAN = false; // Reduction Armor Veteran
	public boolean RAGING_FORCE = false;
	public boolean THUNDER_GRAB_BRAVE = false;
	public boolean FOE_SLAYER_BRAVE = false;
	public boolean 디스트로이피어 = false; // Destroy Pier
	public boolean 디스트로이호러 = false; // Destroy Horror
	public boolean DRAGONSKIN = false;
	public boolean DARKHORSE = false;
	public boolean FINALBURN = false;
	public boolean DRESSEVASION = false;
	public boolean BURNINGSPIRIT = false;
	public boolean AURACIA = false;
	public boolean DESPERADOABSOLUTE = false;
	public boolean ASSASSIN = false;
	public boolean BRAVE_AVATAR = false;
	public boolean RESIST_ELEMENT = false;
	public boolean 글로리어스 = false;
	public boolean BLOOD_TO_SOUL = false;
	public boolean LUCIFER_DESTINY = false;
	...
	public int PRIME = 0;
	public int GRACE = 0;
	public int 저지먼트 = 0; // Judgment
	private int 문장레벨; // sentence level
	...
	public int DESPERADO_ATTACKER_LEVEL = 0;

	public boolean CHARACTER_NAME_CHANGE = false;
	...
	public boolean enteringDungeon = false;
	...
	public boolean 폰인증체크중 = false; // Checking phone authentication
	...
	public boolean 폰인증 = false; // phone authentication
	...
	public boolean isArmourGuard = false;
	public boolean isTitanRock = false;
	public boolean isTitanBlitz = false;
	public boolean isTitanMagic = false;
	...
	public boolean pledgeBuff = false;
	public int FIND_MERCH_OBJID = 0;
	public int orbOfMemory = 0;
	...
	public void Stat_Reset_All() {
		...
		setBaseMagicHitUp(CalcStat.magicHit(getAbility().getTotalInt()));
	...
	private static final int[] allBuffComaSkill = { L1SkillId.HASTE, ADVANCE_SPIRIT,
			// FIRE_WEAPON,
			BLESS_WEAPON, NATURES_TOUCH, L1SkillId.AQUA_PROTECTER, IRON_SKIN, L1SkillId.SHINING_SHIELD,
	...
	class 화면버프 extends TimerTask {
		...
		public void run() {
			...
			for (int element : allBuffComaSkill) {
				if (element == 7895) {
				...
				} else if (element == 10528) {
					if (!getSkillEffectTimerSet().hasSkillEffect(L1SkillId.ENERGY_OF_BLACK)) {
						getAC().addAc(-2);
						addMaxHp(20);
						addMaxMp(13);
					...
					getSkillEffectTimerSet().setSkillEffect(L1SkillId.ENERGY_OF_BLACK, 1800 * 1000);
				} else {
					l1skilluse = new L1SkillUse();
					l1skilluse.handleCommands(L1PcInstance.this, element, getId(), getX(), getY(), null, 0,
	...
	public void deathPenalty(boolean login) {
	...
	public int 던전시간체크(String s) { // Dungeon time check
		int maxtime = 0;
		int time = 0;
	...
	private int _baseMagicHitup = 0; // Magic hit by base stat
	private int _baseMagicCritical = 0; // Magic critical hit by base stat (%)
	private int _baseMagicDmg = 0; // Magic Damage by Base Stat
	private int _baseMagicDecreaseMp = 0; // Magic Damage by Base Stat

	private int _HitupByArmor = 0; // Melee Weapon Accuracy Rate by Defense Mechanism
	private int _bowHitupByArmor = 0; // Accuracy rate of bow by defense mechanism
	private int _DmgupByArmor = 0; // Melee weapon strike rate by defensive device
	private int _bowDmgupByArmor = 0; // Hitting rate of bows by defensive equipment

	private int _bowHitupBydoll = 0; // Ranged attack success rate by dolls
	private int _bowDmgupBydoll = 0; // Long-distance hit rate by dummies
	...
	public void 페어리경험치보상(int lv) {
		...
		if (addexp != 0) {
			...
			if (level > Config.MAXLEVEL) {
				sendPackets(new S_SystemMessage("You can no longer gain experience points."));
			} else {
				addExp(addexp);
			}
		}
	...
	public void 방어체크() {
		if (getAC().getAc() < -250) {
			if (Config.bugChatMonitor() > 0) {
				for (L1PcInstance gm : Config.toArrayBugChatMonitor()) {
	...
	// Delay cycle time, telecom release, etc.
	...
	public void onChangeExp() {
		...
		if (gap == 0) {
			...
			if (char_level >= 60 && char_level <= 64) {
				if (percent >= 10)
					getSkillEffectTimerSet().removeSkillEffect(L1SkillId.LEVEL_UP_BONUS);
		...
		if (gap > 0) {
			levelUp(gap);
			if (getLevel() >= 60) {
				getSkillEffectTimerSet().setSkillEffect(L1SkillId.LEVEL_UP_BONUS, 10800000);
	...
	public void onPerceive(L1PcInstance pc) {
		if (isGhost() || (pc.getMapId() == 2100) || (pc.getMapId() == 2699)) {
			return;
		}
	...
	public void deleteShopItem(int objid) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
...
//	}
	...
	public void logout() {
		try {
			...
			w.deletePersonalStorage(this.getAccountName());
			w.deleteElfStorage(this.getAccountName());
			w.deletePackageStorage(this.getAccountName());
	...
	public void sendPackets(ServerBasePacket serverbasepacket) {
		if (serverbasepacket.getType().equalsIgnoreCase("[S] S_SystemMessage"))
		  //System.out.println("[Server] System message has been sent.");
		...
		if ((getNetConnection() == null) || (getNetConnection().getActiveChar() == null) || (getNetConnection().getActiveChar().getId() != getId()) || 샌드백) {
			return;
		}
			...
			if (getCurrentHp() > 0 && !isDead()) {
				...
				if (attack.calcHit()) {
					if (attacker.getWeapon() != null && getWeapon() != null) {
						if (isTitanMagic && attacker.getWeapon().getItem().getType() == 17) {
							int hpRatio = 100;
							if (0 < getMaxHp()) {
							...
							if (getSecondWeapon() != null) {
								if ((isSlayer && getSecondWeapon().getItemId() == 30083) || (isSlayer && getSecondWeapon().getItemId() == 31083)
										|| (isSlayer && getSecondWeapon().getItemId() == 222208) || (isSlayer && getSecondWeapon().getItemId() == 30092)) {
									titanRatio += 5;
								}
							}
							if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.TITAN_RISING)) {
								if (getLevel() > 80) {
									titanRising += getLevel() - 80;
								}
								if (titanRising > 10) {
									titanRising = 10;
								}
								titanRatio += titanRising;
							}
						...
						// }else{
						if (isTitanRock) {
							if (attacker.getWeapon().getItem().getType() != 4 && attacker.getWeapon().getItem().getType() != 10
									&& attacker.getWeapon().getItem().getType() != 17 && attacker.getWeapon().getItem().getType() != 13) {
								int hpRatio = 100;
								if (0 < getMaxHp()) {
									hpRatio = 100 * getCurrentHp() / getMaxHp();
								}
					...
					if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.COUNTER_BARRIER)
						...
						if (getWeapon() != null && getWeapon().getItem().getType1() == 50) {
							int chan = random.nextInt(100) + 1;
							boolean isProbability = false;
							if (Config.COUNTER_BARRIER_PROC > chan) {
								isProbability = true;
							}
							boolean isShortDistance = attack.isShortDistance();
							if (isProbability && isShortDistance) {
			...
			if (getCurrentHp() > 0 && !isDead()) {
				...
				if (damage >= 0) {
					...
					if (getInventory().checkEquipped(420104) || getInventory().checkEquipped(420105)
						...
						if (chance <= 7 && getCurrentHp() != getMaxHp()) {
				...
				if (damage > 0) {
					// I need to test the incoming damage.
					if (this instanceof L1RobotInstance) {
				...
				System.out.println("[L1PcInstance] Warning: There are some places where the player's HP reduction processing is not done correctly. ※Or HP0 from the beginning");
				death(attacker);
			} else if (isDead() && getCurrentHp() > 0) {
				System.out.println(
						"[L1PcInstance] Warning: HP 0 or higher currently dead. Damage taken. Character:" + getName() + " attacker: " + attacker != null
								? attacker.getName()
								: "none");
			}
		} catch (
	...
	int stunskillid[] = { DEMOLITION, FORCE_STUN, SHOCK_STUN, ETERNITY, EMPIRE, MOB_SHOCKSTUN_30, MOB_RANGESTUN_19,
			MOB_RANGESTUN_18, EARTH_BIND, MOB_COCA, MOB_BASILL, ICE_LANCE, L1SkillId.DESPERADO, BONE_BREAK,
			PHANTASM, FOG_OF_SLEEPING, CURSE_PARALYZE, CURSE_PARALYZE2, L1SkillId.STATUS_CURSE_PARALYZED,
			L1SkillId.STATUS_POISON_PARALYZED, L1SkillId.PANTERA, L1SkillId.SHADOW_STAFF };
	...
	private class Death implements Runnable {
		...
		public void run() {
			...
			try {
				...
				for (int i = L1SkillId.SPICY_RAMEN; i <= L1SkillId.PSY_COOL_DRINK; i++) {
					if (getSkillEffectTimerSet().hasSkillEffect(i))
						getSkillEffectTimerSet().removeSkillEffect(i);
				}
				...
				if ((CharPosUtil.getZoneType(L1PcInstance.this) == 1) || !getMap().isEnabledDeathPenalty()) {
				...
				if (lastAttacker instanceof L1PcInstance) {
					...
					/** lawful concept theorem */
					if (!pinklawful && getLawful() >= 0) {
						int lawful;
				...
				/**
				 * True if a die-hard person wears a specific item, otherwise false!
				 * Delete the Succubus Queen and give the item that reads the Succubus energy to the character that finally attacked Immortal
				...
				if (getInventory().checkItem(42015)) {
					...
					if (lastAttacker instanceof L1PcInstance && Config.Robot_Gaho) {
						if (!isPinkName() && !(getLawful() < 0))
							return;
						if (isElf()) {
				...
				/** Purple disappears after cleaning up the protection system */
				if (isPinkName()) {
					pinklawful = true;
					setPinkName(false);
	...
	private void caoPenaltyResult(int count) { // Sealing item evaporation
		...
		for (int i = 0; i < count; i++) {
			...
			if (item != null) {
				if (item.getBless() > 3) {
					...
					sendPackets(new S_ServerMessage(158, item.getLogName())); // \f1%0%s It evaporates and disappears.
				} else {
					getInventory().tradeItem(item, item.isStackable() ? item.getCount() : 1,
							L1World.getInstance().getInventory(getX(), getY(), getMapId()));
					sendPackets(new S_ServerMessage(638, item.getLogName())); //%0 lost
			...
			for (L1War war : L1World.getInstance().getWarList()) {
				...
				if (getId() == clan.getLeaderId() && warType == 1 && isInWar && isAttackClan) {
					...
					if (enemyClanName != null) {
						if (war.GetWarType() == 1) { // In case of siege
							...
							for (L1PcInstance element : clan_member) {
								if (L1CastleLocation.checkInWarArea(castle_id, element)) { // Forced teleportation of blood members on board
									L1Teleport.teleport(element, locx, locy, mapid, 5, true);
								}
							}
							loc = null;
						}
						war.CeaseWar(getClanname(), enemyClanName); // closing
					}
					break;
				}
			}
	...
	public void resExpTo구호() {
		...
		if (level > Config.MAXLEVEL) {
			S_SystemMessage sm = new S_SystemMessage("You can no longer earn experience because you reached the level cap.");
			sendPackets(sm, true);
			return;
		}
		addExp(exp);
	...
	public int get_Er() {
		...
		int BaseEr = CalcStat.calcRangedEvasion(getAbility().getTotalDex());
	...
	public L1BookMark getBookMark(String name) {
		...
		for (L1BookMark _bookmark : _bookmarks) {
			element = _bookmark;
	...
	public int getMaxWeight() {
		...
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.DECREASE_WEIGHT)) {
			magicWeight = Config.DECREASE_WEIGHT_VALUE;
		}
		...
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.REDUCE_WEIGHT)) {
			magicWeight = Config.REDUCE_WEIGHT_VALUE;
		}
	...
	public boolean isSandstorm() {
		return getSkillEffectTimerSet().hasSkillEffect(L1SkillId.SANDSTORM);
	}
	...
	public boolean isDarkHorse() {
		return (DARKHORSE = true && getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_ELFBRAVE));
	}
	...
	private void levelUp(int gap) {
		...
		if (getLevel() <= Config.CLAUDIALEVEL) {
		...
		for (int i = 0; i < gap; i++) {
			int randomHp = CalcStat.levelUpHP(getType(), getBaseMaxHp(), getAbility().getTotalCon());
			int randomMp = CalcStat.levelUpMP(getType(), getBaseMaxMp(), getAbility().getTotalWis());
		...
		/*for (int i = 0; i < gap; i++) {
			...
			int randomMp = 0;
			if (old++ <= 51) {
				randomHp = CalcStat.levelUpHP(getType(), getBaseMaxHp(), getAbility().getTotalCon());
				randomMp = CalcStat.levelUpMP(getType(), getBaseMaxMp(), getAbility().getTotalWis());
			} else {
				randomHp = CalcStat.levelUpHP(getType(), getBaseMaxHp(), getAbility().getTotalCon());
				randomMp = CalcStat.levelUpMP(getType(), getBaseMaxMp(), getAbility().getTotalWis());
			}
	...
	private boolean fdcheck() {
		if ((this.getNetConnection() == null) || this.getNetConnection().isClosed() || !(getMapId() >= 2600 && getMapId() <= 2699))
			return false;
	...
	public void endOfDungeon() {// int x, int y, int m, int curm
		if (_텔 != null) {
			_텔.종료();
		}
		_텔 = new 텔스레드(this);
	...
	private void levelDown(int gap) {
		...
		getQuset();
		/** Reload notification message vertical when level up or down */
		// notification service(this, false);
		...
		for (int i = 0; i > gap; i--) {
			...
			int randomMp = 0;
			if (old-- <= 51) {
				randomHp = CalcStat.levelUpHP(getType(), 0, getAbility().getTotalCon());
				randomMp = CalcStat.levelUpMP(getType(), 0, getAbility().getTotalWis());
			} else {
				randomHp = CalcStat.levelUpHP(getType(), 0, getAbility().getTotalCon());
				randomMp = CalcStat.levelUpMP(getType(), 0, getAbility().getTotalWis());
			}
		...
		if (!isGm() && Config.LEVEL_DOWN_RANGE != 0) {
			if (getHighLevel() - getLevel() >= Config.LEVEL_DOWN_RANGE) {
				...
				if (getNetConnection() != null)
					getNetConnection().close();
				_log.info(String.format("[%s]: Because it exceeded the range of the lepta, it was forcibly cut.", getName()));
			}
		}
	...
	public void resetBaseDmgup() {
		...
		int newBaseStatDmgup = CalcStat.calcDamageFromSTR(getAbility().getBaseStr());
		int newBaseStatBowDmgup = CalcStat.calcDamageFromDEX(getAbility().getBaseDex());
		if (isWarrior() || isKnight() || isDarkelf() || isDragonknight() || isFencer()) {
	 ...
	 public void resetBaseHitup() {
		...
		int newBaseStatHitup = CalcStat.calcMeleeHitFromSTR(getAbility().getBaseStr());
		int newBaseStatBowHitup = CalcStat.calcRangeHitUp(getAbility().getBaseDex());
		if (isCrown()) {
			newBaseHitup = getLevel() / 5;
			newBaseBowHitup = getLevel() / 5;
		} else if (isKnight() || isWarrior() || isFencer()) {
	...
	public void resetBaseAc() {
		int newAc = CalcStat.calcACFromDEX(getAbility().getTotalDex());
	...
	public void resetBaseMr() {
		 ...
		 */
		newMr += CalcStat.calcMagicResistance(getType(), getAbility().getTotalWis());
		newMr += getLevel() / 2;
	...
	/** Somak and Dice Game (Old Version) */
	...
	/** Somak and Dice and Jelly Chippa Game */
	...
	public boolean is지배이반텔() {
		...
		if ((getMapId() == _용의계곡
				|| getMapId() == _오렌설벽
				|| getMapId() == _풍룡의둥지
				|| getMapId() == _화룡의둥지) && getInventory().checkItem(5001130)) { // dragon valley
			return true;
		}
	...
	public boolean isThirdSpeed() {
		return (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_DRAGONPERL) || getPearlSpeed() == 1);
	}

	private int _pearlSpeed; // Pearl state 0 Normal 1 Deviation Eve

	public int getPearlSpeed() {
		return _pearlSpeed;
	}

	public void setPearlSpeed(int i) {
		_pearlSpeed = i;
	}

	public int getPotionPlus() {
		return _PotionPlus + CalcStat.hpPotionRecovery(getAbility().getTotalCon());
	...
	public void calAinHasad(int i) {
		...
		if (_ainhasad > 0 && !_dragonbless_3) {
			sendPackets(new S_NewSkillIcons(L1SkillId.DRAGON_BLESS_STAGE3, true, -1));
			_dragonbless_3 = true;
		} else if (_ainhasad <= 0) {
			if (_dragonbless_3) {
				sendPackets(new S_NewSkillIcons(L1SkillId.DRAGON_BLESS_STAGE3, false, -1));
	...
	public void removePet(L1NpcInstance npc) {
		for (L1NpcInstance petV : getPetList()) {
			...
			if (petV.getId() == npc.getId()) {
				if (!(npc instanceof L1PetInstance)) {
					if (!this.getNetConnection().CharReStart()) {
	...
	public ArrayList<L1NpcInstance> getPetList() {
		ArrayList<L1NpcInstance> pet = new ArrayList<>();
	...
	public void ClearPlayerClanData(L1Clan clan) throws Exception {
		ClanWarehouse clanWarehouse = WarehouseManager.getInstance().getPledgeStorage(clan.getClanName());
	...
	public void init_hackTimer() {
		hackTimer = 0;
		// Added to thread related to speed hack
	}
...
//	}
	...
	public boolean Sabutelok() {
		if (텔대기() || isTeleport() || isDead() || getSkillEffectTimerSet().hasSkillEffect(L1SkillId.DESPERADO)
	...
	protected void autoattack() {
		boolean 얼던 = false;
		if ((target == null) || (getNetConnection() == null)) {
			run = false;
			return;
		}
		if (isAutoPlay() && !Config.PSS_CONTROLLER) {
			setAutoPlay(false);
		...
		if (isGmInvis() || isGhost() || isDead() || isTeleport() || isInvisDelay() || isParalyzed()) {
			run = false;
			return;
		}
		if (isInvisble()) {
		...
		if (getSkillEffectTimerSet().hasSkillEffect(SHOCK_STUN) 
				...
				|| getSkillEffectTimerSet().hasSkillEffect(EMPIRE) 
				|| getSkillEffectTimerSet().hasSkillEffect(PANTERA)
				|| getSkillEffectTimerSet().hasSkillEffect(SHADOW_STAFF)
		...
		if (getInventory() != null) {
			if (getInventory().calcWeightpercent() >= 83 && !getSkillEffectTimerSet().hasSkillEffect(L1SkillId.RANKING_BUFF_1)) {
				S_SystemMessage sm = new S_SystemMessage("Your belongings are too heavy for battle.");
				sendPackets(sm);
				run = false;
				return;
			}
		...
		if (getCurrentWeapon() == 20 || getCurrentWeapon() == 62) {
			...
			if (polyId == 7967 || polyId == 7968 || polyId == 10874 || polyId == 7846 || polyId == 7848 || polyId == 8719) {
				range = 1;
			}
		...
		if (target instanceof L1NpcInstance) {
			...
			if (npc.getNpcTemplate().get_npcId() == 90000
					...
					|| npc.getNpcTemplate().get_npcId() == 90016) { // frozen
				얼던 = true;
			}
		...
		// Handling when an attack action can be taken
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.ABSOLUTE_BARRIER)) { // Release of absortvaria
		...
		if (cha != null && target != null && !cha.isDead() && !얼던) {
			...
			if (getWeapon() != null) {
				if (getWeapon().getItem().getType() == 17) { // 키링크 모션
					...
					} else if (getWeapon().getItemId() == 6001
							|| getWeapon().getItemId() == 30081
							|| getWeapon().getItemId() == 31081
							|| getWeapon().getItemId() == 222207
							|| getWeapon().getItemId() == 30090
							|| getWeapon().getItemId() == 7238
	...
	/** Einhasad Renewal */
	/**
	 * PC Room Topaz Series Crystal Integrated Type If PC Room + Topaz,
	 * 220 Pro Setting Blessing Consumption Efficiency Topaz Use Only Basic Topaz
	 * Value Added as Efficiency appointed
	 */
	public int get축복경험치() {
		int 축복 = 0;
		if (PCROOM_BUFF)
	...
	public int get축복소모효율() {
		int 축복소모효율 = 0;
		/** Information about the item once passed */
		...
		for (L1DollInstance doll : getDollList()) {
			if (doll.getDollType() == L1DollInstance.DOLLTYPE_MUMMYLORD) {
				축복소모효율 += 8;
			} else if (doll.getDollType() == L1DollInstance.DOLLTYPE_BLESSEDMUMMYLORD) {
				축복소모효율 += 8;
			} else if (doll.getDollType() == L1DollInstance.DOLLTYPE_DEATHKNIGHT) {
				축복소모효율 += 10;
			} else if (doll.getDollType() == L1DollInstance.DOLLTYPE_BLESSEDDEATHKNIGHT) {
				축복소모효율 += 10;
			} else if (doll.getDollType() == L1DollInstance.DOLLTYPE_헌신1등급) {
				축복소모효율 += 8;
			} else if (doll.getDollType() == L1DollInstance.DOLLTYPE_ANTHARAS) {
				축복소모효율 += 10;
			}
		}
	...
	public void addDmgCritical(int i) { // close range crit
		dmgCritical = i;
	}
	...
	public void addBowDmgCritical(int i) { // ranged crit
		bowCritical = i;
	}
	...
	/** instance space Indicates what state the space is in. **/
	...
	/** Used for updates on notifications *//*
	...
	/** Used for updates on notifications */
	/** After receiving the event, the event type suddenly pops up Additional
	 * notification date and time Delete all notifications and re-register */
	...
	/** Update after checking the quest socket type */
	public void getQuset() {
		...
		if (getLevel() >= 10 && lv10_step != L1Quest.QUEST_END) {
			item = getInventory().storeItem(7325, 1); // 10th level reward
	...
	/** Entry Check Information for Indungeon */
	...
	public void isServerInter(int ServerType) {
		try {
			...
			if (!Client.getInterServer()) {
				...
				/** If the type number is 10, it's in dungeon, so let the party forcefully cancel it. */
	...
	/** New Claudia Quest */
	public FastMap<Integer, L1QuestInfo> quest_list = new FastMap<>();
	...
	public void quest_level(int lv) {
		try {
			if (lv > 1 && lv <= Config.CLAUDIALEVEL) {
	...
	/** Added Claudia-related teleport */
...
//			}
			...
			if (skill_check) {
				...
				GeneralThreadPool.getInstance().schedule(teleport, 30);
			} else {
				if (effect_check)
	...
	public boolean isHalpasArmorResult() {
		if (!is_halpas_armor() || getSkillEffectTimerSet().hasSkillEffect(L1SkillId.HALPAS_FAITH_DELAY))
			return false;

		if (get_halpas_armor_faith_delay() != null) {
//			 System.out.println("할파스딜레이 있음 : " + (get_halpas_armor_faith_delay().getTime()
//			 - System.currentTimeMillis()));
	...
	public void receiveDamage(L1PcInstance player, int i) {
