...
public class L1PetInstance extends L1NpcInstance {
						...
						if (targetItem.getItemId() == L1ItemId.ADENA) {
							if (_membersList.size() > 1) {
								...
								for (L1PcInstance element : _membersList) {
									if(element.getId() == Pc.getId()){
										int modNum = targetItem.getCount() % _membersList.size();
										groundInventory.tradeItem(targetItem, divAden + modNum, element.getInventory());
									}else groundInventory.tradeItem(targetItem, divAden, element.getInventory());
		...
		if (getCurrentHp() > 0 && !isDead()) {
			...
			if (newHp <= 0) {
				...
				if(Master == null){
					...
					GeneralThreadPool.getInstance().schedule(new Death(this), 20000);
				}
			} else {
				setCurrentHp(newHp);
			}
			...
			if(Master == null){
				...
				GeneralThreadPool.getInstance().schedule(new Death(this), 30000);
			}
		}
		if(isDogFight()) Broadcaster.wideBroadcastPacket(this, new S_HPMeter(this), true);
		if(Master != null) Master.sendPackets(new S_PetWindow(S_PetWindow.PatHp, this), true);
	}
	...
	public boolean isInvalidName(String name) {
		...
		if (isAlphaNumeric(name) || 5 < (numOfNameBytes - name.length()) || 12 < numOfNameBytes) {
			return false;
		}
		return true;
	}
	...
	/** Pet related hp recovery & fighting spirit related methods */
	...
	/** If the pet is in die state, the death effect is displayed in the fat information even if it is leased and connected */
	...
	/** 펫 스킬 관련 정리 */
	private final Map<Integer, L1PetSkill> _PetSkills = new HashMap<>();
	...
	public ArrayList<L1PetSkill> ArrayPetSkills(){
		ArrayList<L1PetSkill> ArraySkills = new ArrayList<>();
