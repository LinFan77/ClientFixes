...
public class L1FieldObjectInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance pc) {
		...
		case 4212015: { // 드래곤 포탈
			...
			if (pc.getLevel() <= Config.DRAGONRAIDLEVEL) {
			...
			L1Teleport.teleport(pc, 32668, 32675, pc.dragonmapid, 5,
					true);
		...
		case 4212016: { // 드래곤 포탈
			...
			L1Teleport.teleport(pc, 32920, 32672, pc.dragonmapid, 5,
					true);
		...
		case 910008:  // 드래곤 포탈 (발라)
			...
			pc.system = -1;// 초기화
			if(pc.getLevel() < Config.DRAGONRAIDLEVEL){
				pc.sendPackets(new S_SystemMessage("입장 불가: "+Config.DRAGONRAIDLEVEL+"레벨 이상 입장가능"), true);
				return;
			}
	...
	private synchronized void 안타레이드시작(L1PcInstance pc, int mapid) {
		int count = 0;
		AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(mapid);
		count = ar.countUsersInLair();
		if (ar.isAntharasUp()) {
			...
			AntharasRaidTimer antastart = new AntharasRaidTimer(null, ar, 5,
					120 * 1000);// 2분 체크
			antastart.begin();
			// 몹소환 안타 깨어나서 접근금지.
			AntharasRaidTimer antaendtime = new AntharasRaidTimer(null, ar, 6,
					1000 * 60 * 120);// 22분 체크
			ar.art = antaendtime;
			antaendtime.begin();
			// 안타 잡기 실패 모두 텔.
	...
	private synchronized void 파푸레이드시작(L1PcInstance pc, int mapid) {
		...
		int count = 0;
		FafurionRaid ar = FafurionRaidSystem.getInstance().getAR(mapid);
		...
		if (ar.isPaPoo()) {
			...
			FafurionTimer antastart = new FafurionTimer(null, ar, 4, 120 * 1000);// 2분
																			// 체크
			antastart.begin();
			// 몹소환 안타 깨어나서 접근금지.
			FafurionTimer antaendtime = new FafurionTimer(null, ar, 0,
					1000 * 60 * 120);// 22분 체크
			antaendtime.begin();
			// 안타 잡기 실패 모두 텔.
		}
