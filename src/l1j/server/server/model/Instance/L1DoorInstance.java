	...
	public void isPassibleDoor(boolean flag) {
		...
		if (size >= 0) {
			if (getDirection() == 0 || getDirection() == 4) {
				if (getDoorId() == 2010) {
					for (int x = leftEdgeLocation; x <= rightEdgeLocation; x++) {
						World.moveTheDoor(x, entranceY - 1, this.getMapId(), false,
								flag);
					}
				} else {
					for (int x = leftEdgeLocation; x <= rightEdgeLocation; x++) {
						World.moveTheDoor(x, entranceY, this.getMapId(), false, flag);
					}
				}
			} else {
				for (int y = leftEdgeLocation; y <= rightEdgeLocation; y++) {
					World.moveTheDoor(entranceX, y, this.getMapId(), true, flag);
				}
			}
		}
	}
