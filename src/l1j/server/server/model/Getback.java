...
public class Getback {
	...
	public static void loadGetBack() {
		...
		try {
			...
			// If area designation and non-specification are mixed in the same map, area_x1 DESC
			...
			_log.log(Level.SEVERE, "could not load Getback data", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	/**
	 * Get the return point from the current location of the pc.
	 *
	 * @param pc
	 * @param bScroll_Escape(unused)
	 * @return locx, locy, array stored in mapid order
	 */
	public static int[] GetBack_Location(L1PcInstance pc, boolean bScroll_Escape) {
		...
		try {
			...
			if (getbackList != null) {
				...
				//If town_id is specified, it is returned there.
			...
			// Return to SKT if there is no data in the getback table
			else {
	...
	public static int[] GetBack_Restart(L1PcInstance pc) {
		...
		try {
			...
			if (loc[2] != 70) { // Only when it's not forgotten
				if (pc.getClanid() != 0) { // belongs to clan
					int castle_id = 0;
					int house_id = 0;
					...
					if (castle_id != 0) { // Seongju Cranwon
						loc = L1CastleLocation.getCastleLoc(castle_id);
					} else if (house_id != 0) { // Cranwon owned by hideout
			...
			/** 2011.07.31 Fixed copy bug avoidance */
			loc[0] = 33437;
			loc[1] = 32812;
			loc[2] = 4;
			return loc;
