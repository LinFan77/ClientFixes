...
public class L1Trade {
	...
	private boolean chaTradeChk(L1PcInstance pc, L1PcInstance tradingPartner, int itemid) {
		...
		Account account = Account.load(pc.getAccountName()); // addition
		if (account.getquize() != null) {
			pc.sendPackets(new S_SystemMessage(".퀴즈인증 [설정한퀴즈] \\fY인증후 다시 거래하세요."));
			return false;
		} // When the seal is released, if the quiz is not set, it is impossible to apply

		if (itemid == 100904) { // low class
			if (pc.getLevel() > 69) {
				pc.sendPackets(new S_SystemMessage("Use the Advanced Character Exchange Certificate."));
				return false;
			}
		} else { // senior
			if (pc.getLevel() < 70) {
				pc.sendPackets(new S_SystemMessage("Use the lower level character exchange voucher."));
				return false;
			}
		}
		return true;
	...
	private boolean chaTrade(L1PcInstance pc, L1PcInstance tradingPartner, L1ItemInstance item) {
		...
		String chatText = "The trade target character's job is [" + title + "] level is [" + pc.getLevel() + "] The number of elixirs used [" + pc.getAbility().getElixirCount()
				+ "]no see. When trading characters, the connection is automatically terminated after the transaction is completed.";
		...
		chatText = "The current character is transferred to the other person's account, and the connection is automatically terminated after the transaction, and the other person's transaction items are stored in this account's warehouse.";
		...
		if (l1iteminstance != null && trading_partner != null) {
			if (trading_partner instanceof L1PcInstance) {
				...
				if (!l1iteminstance.isEquipped()) {
					if (l1iteminstance.getCount() < itemcount || 0 >= itemcount) { // Added virtual bug related
						return;
					}
					// Below is the source of the character exchange order form.
					if (l1iteminstance.getItemId() == 100903 || l1iteminstance.getItemId() == 100904) {
						...
						for (int cnt = 0; cnt < player_tradecount; cnt++) {
							...
							if (l1iteminstance1.getItemId() == 100903 || l1iteminstance1.getItemId() == 100904) {
								player.sendPackets(new S_SystemMessage("You can only upload one Character Exchange Voucher."));
								return;
							}
						}
				...
				if (l1iteminstance.getCount() < itemcount || 0 >= itemcount) { // virtual bug related
				...
				if (l1iteminstance.getItemId() != 40308) {
					...
					player.setTradeID(0);
					tradenpc.setTradeID(0);
					Broadcaster.broadcastPacket(tradenpc, new S_NpcChatPacket(tradenpc, "Please upload only Adena.", 0), true);
					return;
				}
				...
				if (l1iteminstance.getItemId() == 40308 && itemcount != 100000 && tradenpc.getNpcTemplate().get_npcId() == 7000067) {
					...
					tradenpc.setTradeID(0);
					player.setTradeReady(false);
					Broadcaster.broadcastPacket(tradenpc, new S_NpcChatPacket(tradenpc, "The cost is 200,000 Adena.", 0), true);
				} else {
					player.getInventory().tradeItem(l1iteminstance, itemcount, player.getTradeWindowInventory());
					player.sendPackets(new S_TradeAddItem(l1iteminstance, itemcount, 0), true);
				}
			...
			for (ShopItem shop : list) {
				...
				if ((shop.getPrice() <= itemcount && itemcount % shop.getPrice() == 0) && d - (int) d == 0.0) {
					...
					if (tradenpc.tradeSellItemid != 0 && shop.getItemId() != tradenpc.tradeSellItemid) {
						Broadcaster.broadcastPacket(tradenpc, new S_NpcChatPacket(tradenpc, "You can only purchase one type of item at a time.", 0), true);
						L1Trade trade = new L1Trade();
						trade.TradeCancel(player);
						tradenpc.Npc_trade = false;
						return;
	...
	public void TradeOK(L1PcInstance pc) {
		try {
			int cnt;
			boolean chaChk1 = false; // Character Exchange Order
			boolean chaChk2 = false; // Character Exchange Order
			L1Object trading_partner = L1World.getInstance().findObject(pc.getTradeID());
			if (trading_partner != null) {
				if (trading_partner instanceof L1PcInstance) {
					L1PcInstance tradepc = (L1PcInstance) trading_partner;
					try {
						...
						for (cnt = 0; cnt < player_tradecount; cnt++) {
							...
							if (chaChk2 && !(pcitem.getItemId() == 100903 || pcitem.getItemId() == 100904)) {
								PrivateWarehouse warehouse = WarehouseManager.getInstance().getPersonalStorage(tradepc.getAccountName());
					...
					} else if (tradenpc.getNpcTemplate().get_npcId() == 7000068) { // dice (dealer)
						Akduk2GameSystem gambling = new Akduk2GameSystem();
						gambling.Gambling(pc, pcitem.getCount());
					} else if (tradenpc.getNpcTemplate().get_npcId() == 7000069) { // somak (dealer)
						Akduk1GameSystem gambling = new Akduk1GameSystem();
						gambling.Gambling(pc, pcitem.getCount());
					} else if (tradenpc.getNpcTemplate().get_npcId() == 7000071) { // Somak 2 (Dealer)
						Akduk3GameSystem gambling = new Akduk3GameSystem();
						gambling.Gambling(pc, pcitem.getCount());
					} else if (tradenpc.getNpcTemplate().get_npcId() == 7000078) { // Mukchippa (Dealer)
						Akduk4GameSystem gambling = new Akduk4GameSystem();
						gambling.Gambling(pc, pcitem.getCount());
					} else if (tradenpc.getNpcTemplate().get_npcId() == 7000070) { // Buffs (Monarch, Illusionist, Dragon Knight)
