...
public class L1Clan {
	...
	private ArrayList<String> MarkSeeList = new ArrayList<>();
	...
	public int getOnlinePrivateShopXMemberCount() {
		...
		for (ClanMember element : clanMemberList) {
			if (element.online) {
				if (!element.player.isPrivateShop())
					count++;
			}
			// if (L1World.getInstance().getPlayer(clanMemberList.get(i).name)
			// != null) {
	...
	public L1PcInstance getonline간부() {
		...
		int oldrank = 0;
		for (ClanMember element : clanMemberList) {
			if ((element == null) || !element.online
					|| element.player == null)
				continue;
			pc = element.player;
	...
	public L1PcInstance[] getOnlineClanMember() {
		...
		for (int i = 0; i < clanMemberList.size(); i++) {
			if ((clanMemberList.get(i) == null) || !clanMemberList.get(i).online
	...
	public String getClanSubPrince() {
		for (ClanMember element : clanMemberList) {
			if (element.rank == CLAN_RANK_SUBPRINCE)
				return element.name;
		}
		return null;
	}
	...
	public int getGuardianCount() {
		int count = 0;
		for (ClanMember element : clanMemberList) {
			if (element.rank == CLAN_RANK_GUARDIAN)
				count++;
		}
		return count;
	}
	...
	public Integer[] Alliance() {
		Integer[] i = allianceList.toArray(new Integer[allianceList
				.size()]);
		return i;
	}
