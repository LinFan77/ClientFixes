...
public class WeaponSkill {
	...
	public static void getDeathKnightjin(L1PcInstance pc, L1Character cha) {
		...
		if (val >= chance) {
			...
			ArrayList<L1Character> arrcha = new ArrayList<>();
	...
	public static void 체이서(L1PcInstance pc, L1Character target, int enchanlvl, int cc) {
		...
		if (chance <= 1 + enchanlvl) {
			...
			// System.out.println("테베이팩터짐!");
			dmg = pcInt + randmg;
			dmg = calcMrDefense(targetMr, dmg);
	...
	public static double hyperionsDespair(L1PcInstance pc, L1Character cha, int effect,
			int enchant) {
		double dmg = 0.0D;
		int locx = cha.getX();
		int locy = cha.getY();
	...
	public static double halloweenCus(L1PcInstance pc, L1Character cha) {
		...
		if (8 >= chance) {// 조정
			...
			Broadcaster.broadcastPacket(pc, new S_SkillHaste(cha.getId(), 2, 64 * 1000));
	...
	private static boolean isFreeze(L1Character cha) {
		if (cha.getSkillEffectTimerSet().hasSkillEffect(STATUS_FREEZE) || cha.getSkillEffectTimerSet().hasSkillEffect(ABSOLUTE_BARRIER)
				|| cha.getSkillEffectTimerSet().hasSkillEffect(
						L1SkillId.STATUS_안전모드) || cha.getSkillEffectTimerSet().hasSkillEffect(ICE_LANCE)) {
			return true;
		}
