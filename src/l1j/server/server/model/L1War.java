		...
		if (type == 1 || type == 2 || type == 3) {
			...
			if (clan2 != null) {
				...
				for (L1PcInstance element : clan2_member) {
					if (type == 1) {
						element.sendPackets(new S_War(type,
								clan1_name, clan2_name));
					} else if (type == 2) {
						element.sendPackets(new S_War(type,
								clan1_name, clan2_name));
						if (attack_clan_num == 1) {
							element.sendPackets(new S_War(4,
		...
		if (defence_clan != null) {
			...
			for (L1PcInstance element : defence_clan_member) {
				for (String clanName : GetAttackClanList()) {
					element.sendPackets(new S_War(3,
				...
				L1World.getInstance()
						.broadcastPacketToAll(
								new S_ServerMessage(227, defence_clan_name,
										element));
				clan = L1World.getInstance().getClan(element);
				if (clan != null) {
		...
		for (String element : clanList) {
			if (element != null) {
				...
				if (clan != null) {
					...
					for (L1PcInstance element2 : clan_member) {
						if (element2 != null)
							element2.sendPackets(new S_War(3,
									element, defence_clan_name));
					}
				}
			}
		}
