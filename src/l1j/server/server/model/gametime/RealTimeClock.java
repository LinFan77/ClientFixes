		...
		if (isFieldChanged(Calendar.MONTH)) {
			for (TimeListener listener : _listeners) {
				try {
					...
					System.out.println("--RealTime Clock onMonthChaned Exception Error--");
					e.printStackTrace();
				}
			}
		}
		if (isFieldChanged(Calendar.DAY_OF_MONTH)) {
			for (TimeListener listener : _listeners) {
				try {
					...
					System.out.println("--RealTime Clock onDayChanged Exception Error--");
					e.printStackTrace();
				}
			}
		}
		if (isFieldChanged(Calendar.HOUR_OF_DAY)) {
			for (TimeListener listener : _listeners) {
				try {
					...
					System.out.println("--RealTime Clock onHourChanged Exception Error--");
					e.printStackTrace();
				}
			}
		}
		if (isFieldChanged(Calendar.MINUTE)) {
			for (TimeListener listener : _listeners) {
				try {
					...
					System.out.println("--RealTime Clock onMinuteChanged Exception Error--");
					e.printStackTrace();
				}
			}
		}
