...
public class L1Spawn {
	...
	protected void doSpawn(int spawnNumber, int objectId) {
		...
		try {
			...
			/**
			 * Among the mobs spawned in the wide area, mobs that increase the
			 * home point monsters: Cursed Scarecrow, Red Orc, Bugbear (for Chuseok event)
			 */
			...
			*/
			...
			if (objectId == 0) {
				if ((mob.getMapId() >= 7 && mob.getMapId() <= 13)
						...
						|| (mob.getMapId() == 777)
						|| (mob.getMapId() >= 451 && mob.getMapId() <= 456) // Radun1f
						|| (mob.getMapId() >= 460 && mob.getMapId() <= 466) // Radun2f
						|| (mob.getMapId() >= 470 && mob.getMapId() <= 478) // Radun3f
				) {
					Robot_Location.locationRegistration(mob.getX(), mob.getY(),
							mob.getMapId());
				}
				if (npcId == 80086) {
				...
				if (mob.getMapId() == 4) {
					if (mob instanceof L1ScarecrowInstance) {
						Robot_Location.locationRegistration(mob.getX(), mob.getY(), 4);
					}
			...
			if (npcId == 100716) { // Tell the area around when the Great Black Elder spawns
			...
			System.out.println("NPID: " + mob.getNpcId());
