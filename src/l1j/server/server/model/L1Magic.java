...
import static l1j.server.server.model.skill.L1SkillId.DESTROY;
...
import static l1j.server.server.model.skill.L1SkillId.DESTROY_HORROR;
...
public class L1Magic {
	...
	public boolean calcProbabilityMagic(int skillId) {
		...
		if (_calcType == PC_NPC && _targetNpc != null) {
			...
			if (npcId >= 145685 && npcId <= 145686  && !_pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.HALPAS_FOLLOWER_BUFF)) {
				return false;
			}
		...
		if (_calcType == PC_PC) { // does not overlap
		...
		if (_calcType == PC_PC) { // Eternity does not overlap
		...
		// During Asbind, it is invalid except for WB and Kiang Serration.
		if (_calcType == PC_PC || _calcType == NPC_PC) {
		...
		case CANCELLATION:
		case SILENCE:
		case L1SkillId.DEATH_HEAL:
			if (_calcType == PC_PC) {
				if (_targetPc instanceof L1RobotInstance) {
					if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.ERASE_MAGIC)) {
						probability = 100;
						rnd = 100;
					} else {
						probability = 40;
...
*/
		...
		if (!Config.ALT_ATKMSG || (_targetPc == null && _targetNpc == null))
			return isSuccess;
		...
		if (_pc != null && _pc.isGm()) {
			_pc.sendPackets(new S_SystemMessage("\\fT[" + _pc.getName() + "] ==> ["
					+ (_targetPc == null ? _targetNpc.getName() : _targetPc.getName()) + "][== " + msg2 + " ==][" + msg3 + "]"));
		}
		if (_targetPc != null && _targetPc.isGm()) {
			_targetPc.sendPackets(new S_SystemMessage("\\fY[" + _targetPc.getName() + "] <== ["
					+ (_pc == null ? _npc.getName() : _pc.getName()) + "][== " + msg2 + " ==][" + msg3 + "]"));
		}
		/*
		...
		if (_pc != null && _targetPc != null) {
			if (CharPosUtil.getZoneType(_pc) == 1
				...
				if (skillId == WEAPON_BREAK || skillId == SLOW
						...
						|| skillId == AREA_OF_SILENCE || skillId == SHADOW_STAFF
						...
						|| skillId == ICE_LANCE || skillId == DESTROY_HORROR
	...
	private int calcProbability(int skillId) {
		...
		if (_calcType == PC_PC || _calcType == NPC_PC) {
			defenderLevel = _targetPc.getLevel();
			defenseMr = _targetPc.getResistance().getEffectedMrBySkill();
		} else {
			defenderLevel = _targetNpc.getLevel();
			defenseMr = _targetNpc.getResistance().getEffectedMrBySkill();
			if (skillId == RETURN_TO_NATURE) {
				if (_targetNpc instanceof L1SummonInstance) {
					L1SummonInstance summon = (L1SummonInstance) _targetNpc;
					defenderLevel = summon.getMaster().getLevel();
				}
			}
		}

		switch (skillId) {
		case ERASE_MAGIC: // final
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 2  + Config.ERASE_MAGIC_CHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.ERASE_MAGIC_CHANCE;
			}
			if ((_calcType == PC_PC || _calcType == PC_NPC) && skillId != AREA_OF_SILENCE) {
				if (_pc instanceof L1PcInstance) {
					...
					if (pc.getAttackLevel() != 0) {
						attackerLevel += pc.getAttackLevel();
					}
				}
			}
			...
			if (probability > Config.ERASE_MAGIC_MAX) {
				probability = Config.ERASE_MAGIC_MAX;
			}
			if (probability < Config.ERASE_MAGIC_MIN) {
				probability = Config.ERASE_MAGIC_MIN;
			}
			break;
		...
		case POLLUTE_WATER:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 2 + Config.POLLUTE_MAGIC_CHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.POLLUTE_MAGIC_CHANCE;
			}
			if (_calcType == PC_PC || _calcType == PC_NPC) {
				if (_pc instanceof L1PcInstance) {
					L1PcInstance pc = _pc;
			...
			if (probability > Config.POLLUTE_MAGIC_MAX) {
				probability = Config.POLLUTE_MAGIC_MAX;
			}
			if (probability < Config.POLLUTE_MAGIC_MIN) {
				probability = Config.POLLUTE_MAGIC_MIN;
			}
			break;

		case DESPERADO:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.데페확률;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.데페확률;
			}
		...
		case DEMOLITION:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.DEMOLITIONCHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.DEMOLITIONCHANCE;
			}
		...
		case SHADOW_STAFF:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.쉐도우스탭확률;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.쉐도우스탭확률;
			}
		...
		case POWER_GRIP:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.GRIPCHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.GRIPCHANCE;
			}
			...
			break;
		case SHOCK_STUN: // final
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 2 + Config.SHOCK_STUN_CHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.SHOCK_STUN_CHANCE;
			}
			...
			if (probability > Config.SHOCK_STUN_MAX) {
				probability = Config.SHOCK_STUN_MAX;
			}
			if (_calcType == PC_PC || _calcType == NPC_PC) {
				int stunregi = _targetPc.getTechniqueTolerance();
				probability -= stunregi;
			}
			if (probability < Config.SHOCK_STUN_MIN) {
				probability = Config.SHOCK_STUN_MIN;
			}

			break;
		case JUDGEMENT:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.JUDGEMENTCHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.JUDGEMENTCHANCE;
			}
		...
		case PANTERA:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.PANTERACHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.PANTERACHANCE;
			}
		...
		case PHANTOM:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.PHANTOMCHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.PHANTOMCHANCE;
			}
		...
		case FORCE_STUN:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.FORCESTUNCHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.FORCESTUNCHANCE;
			}
		...
		case ETERNITY:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.ETERNITYCHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.ETERNITYCHANCE;
			}
		...
		case AVENGER:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.AVENGERCHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.AVENGERCHANCE;
			}
		...
		case EMPIRE:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.EMPIRECHANCE;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.EMPIRECHANCE;
			}
		...
		case EARTH_BIND:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.어바확률;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.어바확률;
			}
			if ((_calcType == PC_PC || _calcType == PC_NPC) && skillId != AREA_OF_SILENCE) {
		...
		case THUNDER_GRAB:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + 40;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + 40;
			}
			if ((_calcType == PC_PC || _calcType == PC_NPC) && skillId != AREA_OF_SILENCE) {
		...
		case AREA_OF_SILENCE:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + 45;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + 45;
			}
			if ((_calcType == PC_PC || _calcType == PC_NPC) && skillId != AREA_OF_SILENCE) {
		...
		case ELEMENTAL_FALL_DOWN:
			probability = 40 + (attackerLevel - defenderLevel);
			if ((_calcType == PC_PC || _calcType == PC_NPC) && skillId != AREA_OF_SILENCE) {
				if (_pc instanceof L1PcInstance) {
					L1PcInstance pc = _pc;
					probability += _pc.getSpiritHit();
		...
		case WIND_SHACKLE:
			probability = 1 + (attackerLevel - defenderLevel);
			if (_calcType == PC_PC || _calcType == PC_NPC) {
				probability += _pc.getBaseMagicHitUp();
			}
			break;
		case COUNTER_BARRIER:
			probability = Config.COUNTER_BARRIER_PROC;
			break;
		case HALPHAS:
			probability = 25;
			break;
		...
		case MORTAL_BODY:
			...
			break;
		case PANIC:
			probability = (int) (((l1skills.getProbabilityDice()) / 10D) * (attackerLevel - defenderLevel))
		...
		case CANCELLATION: {
			...
			if (attackerLevel >= defenderLevel) {
				defenseMr -= (attackerLevel - defenderLevel) * 5;
			} else if (attackerLevel < defenderLevel) {
				defenseMr -= (attackerLevel - defenderLevel) * 5;
			}

			if (_calcType == PC_PC || _calcType == PC_NPC) {
				int 마법명중 = CalcStat.magicHit(_pc.getAbility().getTotalInt());
				if (마법명중 > 14) {
					int temp = 마법명중 - 14;
					defenseMr -= temp * 4;
				}
			...
			if (_calcType == PC_PC || _calcType == PC_NPC) {
				if (_pc.isElf()) {
					...
					if (_calcType == PC_NPC) {
						if (_targetNpc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.ERASE_MAGIC)) {
							probability *= 2;
						}
					}
				}
		...
		case DESTROY:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 2 + 45;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 2 + 45;
			}
			if ((_calcType == PC_PC || _calcType == PC_NPC) && skillId != AREA_OF_SILENCE) {
		...
		case BONE_BREAK:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.BONEBREAK;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + Config.BONEBREAK;
			}
			if ((_calcType == PC_PC || _calcType == PC_NPC) && skillId != AREA_OF_SILENCE) {
		...
		case ARMOR_BREAK:
			if (attackerLevel >= defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + 55;
			} else if (attackerLevel < defenderLevel) {
				probability = (attackerLevel - defenderLevel) * 3 + 55;
			}
			if (_calcType == PC_PC || _calcType == PC_NPC) {
		...
		case TURN_UNDEAD:
			...
			if (_calcType == PC_PC || _calcType == PC_NPC) {
				probability = (int) ((attackInt * 2 + (attackerLevel * 2.5) + _pc
						.getBaseMagicHitUp())
						- (defenseMr + (defenderLevel / 2)) - 84);
				if (!_pc.isWizard()) {
					probability -= 20;
				}
			} else
				probability = (int) ((attackInt * 2 + (attackerLevel * 2.5))
						- (defenseMr + (defenderLevel / 2)) - 84);
	...
	public int calcMagicDamage(int skillId) {
		...
		if (_calcType == PC_PC || _calcType == PC_NPC) {
			...
			damage += CalcStat.calcMagicDmg(_pc.getAbility().getTotalInt());

			double balance = CalcStat.calcMagicBonus(_pc.getAbility().getTotalInt()) * 0.01;
	...
	private int calcPcMagicDamage(int skillId) {
		...
		if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(SPECIAL_COOKING)) { // Reduction of damage by cooking
			dmg -= 5;
		}
       ...
       if(_targetPc.getWeapon() != null) {
		if (_targetPc.isTitanMagic) {
			...
			if (hpRatio < TitanRatio) {
				...
				if (skillId == SHOCK_STUN || skillId == FOE_SLAYER
	...
	private int calcMagicDiceDamage(int skillId) {
		...
		if (_calcType == PC_PC || _calcType == PC_NPC) {
			if (skillId == L1SkillId.DISINTEGRATE
				...
				int crirnd = 12;
				crirnd += CalcStat.magicCrit(_pc.getAbility().getTotalInt());
		...
		if (_calcType == PC_PC || _calcType == PC_NPC) {
			int propCritical = CalcStat.magicCrit(_pc.ability.getTotalInt());
	...
	private double calcAttrResistance(int attr) {
		...
		if (_calcType == PC_PC || _calcType == NPC_PC) {
			...
			// Vulnerable attribute damage increased by 10%
	...
	private void commitPc(int damage, int drainMana, boolean ismanadrain) {
		if (_calcType == PC_PC) {
			...
			if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(EARTH_BIND) && damage >= 0) {
				damage = 0;
				drainMana = 0;
			}
		...
		if (_calcType == PC_NPC) {
			...
			if (_targetNpc instanceof L1MonsterInstance) {
				...
				if (mon.kir_counter_magic) {
					_pc.receiveDamage(_targetNpc, damage, true);
					drainMana = 0;
					damage = 0;
