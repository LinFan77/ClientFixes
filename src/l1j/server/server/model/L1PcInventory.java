...
public class L1PcInventory extends L1Inventory {
	...
	/** Include additional weights added to the item's weight */
	public int calcWeightpercent(int 추가무게) {
		if (Config.RATE_WEIGHT_LIMIT != 0) {
			...
			return WeightRatio;
		} else { // Weight is always 0 if weightrate is 0
			return 0;
		}
	}
	...
	public int checkAddItem(L1ItemInstance item, int count, boolean message) {
		...
		if (weight < 0 || (item.getItem().getWeight() * count) < 0 || (calcWeightpercent() >= 100)) {
			if (message) {
	...
	public L1ItemInstance CaoPenalty() {
		...
		while (!ok) {
			...
			ck++;
			if (penaltyItem.getItem().getItemId() == L1ItemId.ADENA || !penaltyItem.getItem().isTradable() || penaltyItem.getItem().getItemId() == 40312 || penaltyItem.getItem().getItemId() == 49312) {
				continue;
			}
