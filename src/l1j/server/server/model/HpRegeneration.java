...
public class HpRegeneration extends TimerTask {
	...
	public void regenHp() {
		...
		// 공복과 중량의 체크
		if (_pc.get_food() < 24 || isOverWeight(_pc) || _pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.BERSERKERS)) {
		...
		if (pc.getSkillEffectTimerSet().hasSkillEffect(
				...
				|| pc.getSkillEffectTimerSet().hasSkillEffect(
						L1SkillId.ADDITIONAL_FIRE) || pc.getSkillEffectTimerSet().hasSkillEffect(
								L1SkillId.AWAKEN_FAFURION) || pc.getInventory().checkEquipped(20049)) {
			return false;
		}
		if (isInn(pc)) {
			return false;
