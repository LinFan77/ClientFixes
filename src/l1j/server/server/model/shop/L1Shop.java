...
public class L1Shop {
	...
	public void sellItems(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		if (getNpcId() == 70014) {
			if (!판도라증서체크(pc, orderList))
				return;
			pandorasCertificate(pc.getInventory(), orderList);
			return;
		}
	...
	private void NpcShopSellItems(L1PcInstance pc, L1PcInventory inv, L1ShopBuyOrderList orderList) {
		...
		//pc.sendPackets(new S_SystemMessage("en npcshopsellitems"), true);
			...
			try {
				...
				for (L1ShopBuyOrder order : orderList.getList()) {
					...
					if (remaindcount == amount) {
						...
						try {
							NpcBuyShop.getInstance().deleteShopItem(_npcId, objid);
						} catch (Exception e) {
							e.printStackTrace();
						}
					} else {
						...
						try {
							NpcBuyShop.getInstance().storeItemUpdate(_npcId, objid, remaindcount - amount);
						} catch (Exception e) {
							e.printStackTrace();
				...
				if (_npcId >= 8000000 && _npcId < 8100000) {
					...
					if (_sellingItems.size() <= 0) {
						...
						NpcBuyShop.getInstance().deleteShopItem(_npcId);
	...
	private boolean NoTaxEnsureSell(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		if (_npcId == 5000136) {
			...
			if (_npcId == 5000136) {
				try {
					for (L1ShopBuyOrder element : orderList.getList()) {
						item = element;
	...
	private void pandorasCertificate(L1PcInventory inv, L1ShopBuyOrderList orderList) {
		if (!inv.consumeItem(600249, orderList.getTotalPrice())) {
			throw new IllegalStateException("I don't have enough Pandora's Certificates to purchase.");
		}
		/*
		 * if(inv.getOwner().getNetConnection().getAccount().berry <
		 * orderList.getTotalPrice()){ throw new
	...
	private void 주화(L1PcInventory inv, L1ShopBuyOrderList orderList) {
		...
		for (L1ShopBuyOrder order : orderList.getList()) {
			...
			inv.storeItem(item);
			LogTable.lognpcshop(inv.getOwner(), getNpcId(), item, order.getItem().getPrice(), amount, 0);
			//.getInstance().ShopAppend(item.getLogName(), amount, (amount * order.get_price()), String.valueOf(getNpcId()), inv.getOwner().getName());
		}
		/*
		 * try{ inv.getOwner().sendPackets(new S_PacketBox(S_PacketBox.낚시_베리,
		 * inv.getOwner().getNetConnection().getAccount().berry)); }catch(Exception e){}
	...
	public void buyItems(L1ShopSellOrderList orderList) {
		...
		if (0 < totalPrice) {
			...
			// 버경 엔피씨 제외
			if (getNpcId() != 70035 && getNpcId() != 70041 && getNpcId() != 70042)
				LogTable.adenShop(orderList.getPc(), totalPrice);
		}
	...
	public List<L1AssessedItem> assessTickets(L1PcInventory inv) {
		...
		for (L1ShopItem item : _purchasingItems) {
			for (L1ItemInstance targetItem : inv.findItemsId(item.getItemId())) {
				if (!isPurchaseableItem(targetItem) || (L1BugBearRace.getInstance().bugRound() == targetItem.getRoundId())) {
					continue;
				}
	...
	public List<L1AssessedItem> assessTicketsBugBearRace(L1PcInventory inv) {
		...
		for (L1ShopItem item : _purchasingItems) {
			for (L1ItemInstance targetItem : inv.findItemsId(item.getItemId())) {
				if (!isPurchaseableItem(targetItem) || (L1BugBearRace.getInstance().getRoundId() == targetItem.getSecondId()
	...
	public List<L1AssessedItem> assessTicketsDogFight(L1PcInventory inv) {
		...
		for (L1ShopItem item : _purchasingItems) {
			for (L1ItemInstance targetItem : inv.findItemsId(item.getItemId())) {
				if (!isPurchaseableItem(targetItem) || (DogFight.getInstance().getRoundId() == targetItem.getSecondId() && DogFight.getInstance().getBugRaceStatus() != DogFight.STATUS_NONE)) {
					continue;
				}
