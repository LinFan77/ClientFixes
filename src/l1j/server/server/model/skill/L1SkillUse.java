...
public class L1SkillUse {
	...
	public static Map<Integer, L1Object> _truetarget_list = new HashMap<>();

	private static final int[] CAST_WITH_INVIS = { 1, 2, 3, 5, 8, 9, 12, 13, DECREASE_WEIGHT, 19, 21, 26, 31, 32, 35, 37, 42, 43, 44, 48, 49, 52, 54, 55, 57, 60, 61, 63, 67,
			68, 69, 72, 73, 75, ABSOLUTE_BARRIER, 79, REDUCTION_ARMOR, MAJESTY, BOUNCE_ATTACK, SOLID_CARRIAGE, COUNTER_BARRIER, 97, 98, 99, 100, 101, 102, 104, 105, 106, 107,
			109, 110, 111, 113, 114, 115, 116, 117, 118, 129, 130, 131, 133, 134, 137, 138, 146, 147, 148, 149, 150, 151, 155, 156, 158, 159, 163, 164, 165,
			166, 168, 169, 170, 171, 181, SOUL_OF_FLAME, ADDITIONAL_FIRE, ILLUSION_OGRE, PATIENCE, ILLUSION_GOLEM, ILLUSION_LICH, ILLUSION_AVATAR, INFERNO,
			HALPHAS };

	// Magic that cannot be blocked by Counter Magic
	private static final int[] EXCEPT_COUNTER_MAGIC = { 1, 2, 3, 5, 8, 9, 12, 13, 14, 19, 21, 26, 31, 32, 35, 37, 42, 43, 44, 48, 49, 52, 54, 55, 57, 60, 61,
			63, 67, 68, 69, 72, 73, 75, ABSOLUTE_BARRIER, 79, FORCE_STUN, ETERNITY, AVENGER, DEMOLITION, SHOCK_STUN, EMPIRE, POWER_GRIP, DESPERADO, PANTERA, SHADOW_STAFF, PHANTOM, BLADE,
			REDUCTION_ARMOR, BOUNCE_ATTACK, SOLID_CARRIAGE, MAJESTY, COUNTER_BARRIER, HALPHAS, INFERNO, 97, 98, 99, 100, 101, 102, 104, 105, 106, 107, 109, 110, 111,
			ARMOR_BREAK, 113, 114, 115, 116, 117, 118, 129, 130, 131, 132, 134, 137, 138, 146, 147, 148, 149, 150, 151, 155, 156, 158, 159, 161, 163, 164, 165,
			166, 168, 169, 170, 171, 181, SOUL_OF_FLAME, ADDITIONAL_FIRE, FOE_SLAYER, AWAKEN_ANTHARAS, AWAKEN_VALAKAS, AWAKEN_FAFURION,
			MIRROR_IMAGE, ILLUSION_OGRE, PATIENCE, ILLUSION_GOLEM, ILLUSION_LICH, ILLUSION_AVATAR, INSIGHT, 10026, 10027, 10028, 10029, 30060, 30000,
			30078, 30079, 30011, 30081, 30082, 30083, 30080, 30084, 30010, 9278, 9279, 30002, 30086, 100091, 100092 };

	// Party application skill check
	private boolean _checkedPartyApplySkill = false;

	// Guild applied skill check
	...
	public boolean checkUseSkill(L1PcInstance player, int skillid, int target_id, int x, int y, String message, int time, int type, L1Character attacker) {
		...
		if (player instanceof L1PcInstance) {
			...
			if (jonje == null && player.getAccessLevel() != Config.GMCODE) {
				player.sendPackets(new S_SystemMessage("Bug detected forcing shutdown! Please reconnect."), true);
				player.sendPackets(new S_Disconnect(), true);
				return false;
			}
		}

		if (skillid == L1SkillId.FIRE_BLESS || skillid == L1SkillId.SANDSTORM) {
			if (player.getWeapon() == null) {
				player.sendPackets(new S_SystemMessage("Spell fizzled. You need a weapon equipped."));
				return false;
			} else {
				L1ItemInstance weapon = player.getWeapon();
				if (weapon.getItem().getType1() == 24) {
					player.sendPackets(new S_SystemMessage("Spell fizzled. You need a sword equipped."));
					return false;
				}
			}
		}
		...
		if (skillid == L1SkillId.COUNTER_BARRIER) {
			if (player.getWeapon() == null) {
				...
				if (weapon.getItem().getType1() != 50) {
					player.sendPackets(new S_SystemMessage("Spell fizzled. You need a two-handed sword equipped."));
					return false;
				}
			}
		}

		if (skillid == L1SkillId.HALPHAS) {
			if (player.getWeapon() == null) {
				player.sendPackets(new S_SystemMessage("Spell fizzled. You need a weapon equipped."));
				return false;
			} else {
				L1ItemInstance weapon = player.getWeapon();
				if (weapon.getItem().getType1() != 24) {
					player.sendPackets(new S_SystemMessage("Spell fizzled. You need a chainsword equipped."));
					return false;
				}
			}
		}

		if (skillid == L1SkillId.INFERNO) {
			if (player.getWeapon() == null) {
				player.sendPackets(new S_SystemMessage("Spell fizzled. You need a weapon equipped."));
				return false;
			} else {
				...
				if (weapon.getItem().getType1() != 4) {
					player.sendPackets(new S_SystemMessage("Spell fizzled. You need a one-handed sword equipped. Cannot use a dagger."));
					return false;
				}
			}
		}

		if (skillid == L1SkillId.BLOWATTACK) {
			if (player.getWeapon() == null) {
				player.sendPackets(new S_SystemMessage("Spell fizzled. You need a weapon equipped."));
				return false;
			}
	...
	/**
	 * When using a normal skill, it is determined whether the skill can be used from the user's state.
	 *
	 * @return false If the skill is unavailable
	 */
	...
	private boolean isNormalSkillUsable() {
		if (_user instanceof L1PcInstance) {
			...
			// If the weight is over, the skill cannot be used.
			...
			/** 2011.04.07 fixed number battle zone */

			// Cannot use certain magic in siege zone ---------
			int castle_id = L1CastleLocation.getCastleIdByArea(pc);
			if (castle_id != 0) {
				if (_skillId == 69 || _skillId == 157) {
					pc.sendPackets(new S_SystemMessage("\\fUYou cannot use that magic in a siege zone."), true);
					return false;
				}
			}
			...
			if (_skillId == 69 || _skillId == 157) {
				...
				if (l1object != null && l1object instanceof L1PcInstance) {
					...
					if (castle_id2 != 0) {
						pc.sendPackets(new S_SystemMessage("\\fUYou cannot use that magic on targets in the siege zone."), true);
						return false;
					}
				}
			}
			...
			if (pc.getMapId() == 5302 || pc.getMapId() == 5153 || pc.getMapId() == 5490) { // battle zone
				if (_skillId == 69 || _skillId == 78 || _skillId == 71 || _skillId == 39 || _skillId == 20 || _skillId == 33 || _skillId == 67 || _skillId == 61
					...
					S_ChatPacket s_chatpacket = new S_ChatPacket(pc, "Casting is not possible in the Battle Zone.", Opcodes.S_MESSAGE, 20);
					pc.sendPackets(s_chatpacket);
					return false;
				}
			}

			if (pc.getMapId() == 5001) { // battle zone waiting room
				if (_skillId == 60 || _skillId == 67) {
					S_ChatPacket s_chatpacket = new S_ChatPacket(pc, "Casting is not possible in the waiting room.", Opcodes.S_MESSAGE, 20);
					pc.sendPackets(s_chatpacket);
					return false;
				}
			}
			...
			if (pc.getMap().isSafetyZone(pc.getLocation())) {
				if (_skillId == 220 || _skillId == 215 || _skillId == 205 || _skillId == 11 || _skillId == 67) {
					pc.sendPackets(new S_SystemMessage("\\fUThat spell cannot be used in a Safety Zone."), true);
					return false;
				}
			}

			/*if (pc.getMapId() == 622) { // feather village
				if (_skillId == 69 || _skillId == 67) {
					pc.sendPackets(new S_SystemMessage("\\fUThis magic cannot be used in Feather Village."), true);
					return false;
				}
			}*/
	...
	private boolean isSpellScrollUsable() {
		L1PcInstance pc = (L1PcInstance) _user;
		if (pc.isParalyzed() || ((pc.isInvisble() || pc.isInvisDelay()) && !isInvisUsableSkill(pc))) {
			return false;
		}
	...
	public void handleCommands(L1PcInstance player, int skillId, int targetId, int x, int y, String message, int timeSecs, int type, L1Character attacker) {
		try {
			...
			// TODO Because the checkUseSkill() function stores global variables, the result of the isCheckedUseSkill() function is always false.
			...
			// Check whether or not the skill used is a guild member applied skill
			...
			case TYPE_NORMAL:
				...
				if (_target != null && isUseSkill && _user instanceof L1PcInstance) {
					if ((!CharPosUtil.isAreaAttack(_player, _target.getX(), _target.getY(), _target.getMapId())
							|| !CharPosUtil.isAreaAttack(_target, _player.getX(), _player.getY(), _player.getMapId()))) {
							...
							if (_skill.getTarget().equals("buff") && _skill.getType() != L1Skills.TYPE_HEAL) {
								...
								if (_skillId != 5 && _skillId != 69 && _skillId != 131) {
									...
									if (/*
										...
										if(_player.isSkillMastery(L1SkillId.DESTROY_HORROR)) {
			...
			// Initialization of party and blood source applied skill checks
	...
	private boolean isTarget(L1Character cha) throws Exception {
		...
		if ((_skill.getType() == L1Skills.TYPE_ATTACK && cha.getId() == _user.getId()) || (cha.getId() == _user.getId() && _skillId == HEAL_ALL)) {
			return false;
		}
		...
		if ((_skill.getTarget().equals("attack") || _skill.getType() == L1Skills.TYPE_ATTACK) && !(cha instanceof L1MonsterInstance)
				&& !(cha instanceof L1PeopleInstance) && !_isPK && _target instanceof L1PcInstance) {
		...
		if ((!CharPosUtil.isAreaAttack(_user, cha.getX(), cha.getY(), cha.getMapId())
				|| !CharPosUtil.isAreaAttack(cha, _user.getX(), _user.getY(), _user.getMapId())) && !_skill.isThrough()) {
		...
		if ((cha.getSkillEffectTimerSet().hasSkillEffect(ICE_LANCE)) && (_skillId == ICE_LANCE 
				|| _skillId == SHOCK_STUN 
				|| _skillId == DESPERADO 
				|| _skillId == EMPIRE 
				|| _skillId == POWER_GRIP 
		...
		if (cha.getSkillEffectTimerSet().hasSkillEffect(EARTH_BIND)) {
			if (_skillId != WEAPON_BREAK && _skillId != CANCELLATION // probability system
					&& _skillId != 208 && _skillId != 217 // Bonebreak, Panic
					&& _skill.getType() != L1Skills.TYPE_CHANGE) { // buff system
				return false;
			}
		}
		...
		if (!(cha instanceof L1TowerInstance || cha instanceof L1DoorInstance) && !cha.isDead()
		...
		if (cha instanceof L1PcInstance) {
			...
			if (pc.getSkillEffectTimerSet().hasSkillEffect(ABSOLUTE_BARRIER) || pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_안전모드)) {
				if (_skillId == CURSE_BLIND || _skillId == WEAPON_BREAK || _skillId == DARKNESS || _skillId == WEAKNESS || _skillId == FOG_OF_SLEEPING
						...
						|| _skillId == DETECTION || _skillId == DESTROY_HORROR || _skillId == COUNTER_DETECTION
						|| _skillId == ERASE_MAGIC || _skillId == EYE_OF_DRAGON || _skillId == ENTANGLE || _skillId == FEAR
						...
						|| _skillId == ANTA_SKILL_12 || _skillId == ANTA_SKILL_13 || _skillId == ANTA_SKILL_14 || _skillId == 40027 || _skillId == DEATH_HEAL
						|| _skillId == MISO_1_ATT || _skillId == MISO_2_DEF || _skillId == MISO_3_GRW) {
					return true;
				} else {
					return false;
				}
		...
		if (cha instanceof L1NpcInstance) {
			...
			if (hiddenStatus == L1NpcInstance.HIDDEN_STATUS_SINK) {
				if (_skillId == DETECTION || _skillId == COUNTER_DETECTION || _skillId == EYE_OF_DRAGON) {
					return true;
				} else {
					return false;
				}
		...
		if (_calcType == PC_PC) {
			if (CharPosUtil.getZoneType(_player) == 1 || CharPosUtil.getZoneType(cha) == 1) {
				if (_skillId == L1SkillId.CURE_POISON || _skillId == L1SkillId.MIND_BREAK || _skillId == L1SkillId.PANIC || _skillId == L1SkillId.PHANTASM
						...
						|| _skillId == L1SkillId.WIND_SHACKLE || _skillId == L1SkillId.SHADOW_STAFF || _skillId == L1SkillId.PANTERA || _skillId == L1SkillId.BLADE
	...
	private void makeTargetList() {
		...
		try {
			if ((_type == TYPE_LOGIN) || (_skill.getTargetTo() == L1Skills.TARGET_TO_ME && (_skill.getType() & L1Skills.TYPE_ATTACK) != L1Skills.TYPE_ATTACK)) {
			...
			if (_skill.getTarget().equals("buff") && _calcType == PC_PC
					&& (!CharPosUtil.isAreaAttack(_user, _target.getX(), _target.getY(), _target.getMapId())
	...
	private boolean isAttrAgrees() {
		...
		if (!_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= FOCUS_WAVE && _skillId <= SANDSTORM && magicattr != 0)
				&& (magicattr != _player.getElfAttr() && !_player.isGm())) {
			return false;
		} else if (_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= FOCUS_WAVE && _skillId <= SANDSTORM
	...
	private boolean isHPMPConsume() {
		...
		if (_user instanceof L1NpcInstance) {
			...
			if (!(_skillId >= SHOCK_STUN && _skillId <= COUNTER_BARRIER) && !(_skillId >= 225 && _skillId <= 230) && !(_skillId == DEMOLITION)
				...
				double balance = CalcStat.calcMPCostReduction(_player.getAbility().getTotalInt()) * 0.01;
	...
	/**
	 * Register skill timer and display buff icon
	 */
	private void addMagicList(L1Character cha, boolean repetition) {
		...
		if (_skillId == DEATH_HEAL || _skillId == SHOCK_STUN || _skillId == DESPERADO || _skillId == POWER_GRIP || _skillId == EMPIRE || _skillId == DEMOLITION
				|| _skillId == FORCE_STUN || _skillId == ETERNITY || _skillId == L1SkillId.SHADOW_STAFF || _skillId == PANTERA || _skillId == PHANTOM || _skillId == THUNDER_GRAB) {
	...
	private void sendIcon(L1PcInstance pc) {
		...
		/* 샤이닝 실드 */
		case SHINING_SHIELD: // 스킬 수정 by white
			if (pc.isInParty()) {
				for (L1PcInstance partyMember : pc.getParty().getMembers()) {
					...
					if (pc.getLocation().getTileDistance(partyMember.getLocation()) <= 14) {
						...
						if (!partyMember.getSkillEffectTimerSet().hasSkillEffect(SHINING_SHIELD)) {
						...
						// 스킬 아이콘 표시 패킷 전송
						partyMember.sendPackets(new S_SMPacketBox(S_SMPacketBox.RMSkillIconAura, SHINING_SHIELD, _getBuffIconDuration), true);
				...
				// 아이콘 표시
				pc.sendPackets(new S_SMPacketBox(S_SMPacketBox.RMSkillIconAura, SHINING_SHIELD, _getBuffIconDuration), true);
		...
		case BRAVE_MENTAL:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.BRAVE_MENTAL, true, _getBuffIconDuration), true);
			break;
		...
		case CLEAR_MIND:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.CLEAR_MIND, true, _getBuffIconDuration));
		...
		case REDUCTION_ARMOR:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.REDUCTION_ARMOR, true, _getBuffIconDuration));
			break;

		case SOLID_CARRIAGE:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.SOLID_CARRIAGE, true, _getBuffIconDuration));
			break;

		case ENCHANT_WEAPON:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.ENCHANT_WEAPON, true, _getBuffIconDuration));
			break;

		case HOLY_WEAPON:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.HOLY_WEAPON, true, _getBuffIconDuration));
			break;

		case BLESSED_ARMOR:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.BLESSED_ARMOR, true, _getBuffIconDuration));
			break;

		case PRIDE:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.PRIDE, true, _getBuffIconDuration));
			break;

		case DRESS_EVASION:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.DRESS_EVASION, true, _getBuffIconDuration));
			break;

		case AQUA_PROTECTER:// 5
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.AQUA_PROTECTER, true, _getBuffIconDuration));
			break;

		case SHADOW_FANG:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.SHADOW_FANG, true, _getBuffIconDuration));
			break;

		case ADVANCE_SPIRIT:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.ADVANCE_SPIRIT, true, _getBuffIconDuration));
			break;

		case GIGANTIC:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.GIGANTIC, true, _getBuffIconDuration));
			break;
		...
		case MISO_1_ATT:
			pc.sendPackets(new S_MaanIcons(L1SkillId.MISO_1_ATT, true, _getBuffIconDuration));
			break;

		case MISO_2_DEF:
			pc.sendPackets(new S_MaanIcons(L1SkillId.MISO_2_DEF, true, _getBuffIconDuration));
			break;

		case MISO_3_GRW:
			pc.sendPackets(new S_MaanIcons(L1SkillId.MISO_3_GRW, true, _getBuffIconDuration));
			break;

		case IMMUNE_TO_HARM:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.IMMUNE_TO_HARM, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("[Effect]Immune to Harm reduces damage received by a certain amount."));
			break;
		...
		case BLOWATTACK:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.BLOWATTACK, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("Blow Attack: Applies 1.5x close-range damage with a certain probability."));
			break;

		case LUCIFER:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.LUCIFER, true, 30));
			pc.sendPackets(new S_SystemMessage("루시퍼 : 받는 대미지가 일정량 감소함."));
			if (pc.LUCIFER_DESTINY) {
		...
		case POTENTIAL:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.POTENTIAL, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("포텐셜 : HP/MP/DG/ER/MR/SP +20%."));
			break;

		case ILLUSION_OGRE: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.ILLUSION_OGRE, true, _getBuffIconDuration));
		}
			break;

		case CUBE_OGRE: {  // Cube Ogre
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_OGRE, true, _getBuffIconDuration));
		}
			break;

		case ILLUSION_LICH: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.ILLUSION_LICH, true, _getBuffIconDuration));
		}
			break;

		case SOUL_BARRIER: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.SOUL_BARRIER, true, _getBuffIconDuration));
		}
			break;

		case CUBE_GOLEM: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_GOLEM, true, _getBuffIconDuration));
		}
			break;
		case ILLUSION_GOLEM: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.ILLUSION_GOLEM, true, _getBuffIconDuration));
		}
			break;

		case CUBE_LICH: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_LICH, true, _getBuffIconDuration));
		}
			break;

		case ILLUSION_AVATAR: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.ILLUSION_AVATAR, true, _getBuffIconDuration));
		}
			break;

		case CUBE_AVATAR: {
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_AVATAR, true, _getBuffIconDuration));
		}
			break;
		...
		case WIND_WALK:
			...
			pc.sendPackets(new S_SystemMessage("You feel the effect of Wind Walk."));
			break;

		case FREEZING_ARMOUR:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.FREEZING_ARMOUR, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("You feel the effect of Freezing Armour."));
			break;

		case ENCHANT_ACCURACY:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.ENCHANT_ACCURACY, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("You feel the effect of Enchant Accuracy."));
			break;

		case MOBIUS:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.MOBIUS, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("You feel the effects of Mobius."));
			break;

		case MAJESTY:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.MAJESTY, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("You feel the effects of Majesty."));
			break;

		case SHINING_ARMOUR:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.SHINING_ARMOUR, true, _getBuffIconDuration));
			break;

		case FOCUS_SPIRIT:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.FOCUS_SPIRIT, true, _getBuffIconDuration));
			pc.sendPackets(new S_SystemMessage("You feel the effects of Focus Spirit."));
			break;
		...
		/* Monarch skill => Processing when casting while applying skill*/
		case PRIME: // skill fix by white
			...
			// If a guild exists
			if (pc.getClanid() != 0) {
				// User confirmation shown on caster screen
				for (L1PcInstance member : L1World.getInstance().getVisiblePlayer(pc, 13)) {
					if (pc.getClanid() == member.getClanid()) {
						...
						// Applies only to new users when in use
						...
						// Prime skill buff effect update
						member.sendPackets(new S_SPMR(member), true);
						member.sendPackets(new S_SystemMessage("You feel the effects of Prime."));
						// Prime Effect Icon Update
			...
			pc.sendPackets(new S_SystemMessage("You feel the effects of Prime."));
			//Renew duration when used repeatedly
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.PRIME, true, _getBuffIconDuration), true);

			break;

		case GRACE_AVATAR: // skill fix by white
			if (pc.isInParty()) {
				for (L1PcInstance paty : pc.getParty().getMembers()) {

					if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 18) {
						if (paty.isDead()) {
							continue;
						}

						if (!paty.getSkillEffectTimerSet().hasSkillEffect(GRACE_AVATAR)) {
							int c = 5; // basic probability
							if (paty.getLevel() > 80) {
								c += (paty.getLevel() - 80);
							}

							if (c > 10) { // maximum probability
								c = 10;
							}

							// Duplicate Buff Effect Check Illusion Cube Delete if duplicate
							paty.GRACE = c;
							// Apply hit effect for impact
							paty.addAllTolerance(paty.GRACE);
						}

						paty.sendPackets(new S_NewSkillIcons(L1SkillId.GRACE_AVATAR, true, 60));
					}
				}
			} else {
				// Renew duration when used repeatedly
				pc.sendPackets(new S_NewSkillIcons(L1SkillId.GRACE_AVATAR, true, 60));
			}
	...
	private void sendGrfx(boolean isSkillAction) {
		...
		if (_user instanceof L1PcInstance) {
			...
			if (_skillId == THUNDER_GRAB) {
				...
				if (!pc.THUNDER_GRAB_BRAVE) {
					...
					if (_target instanceof L1PcInstance) {
						...
						if (isTargetCalc(target)) {
							if (!target.getSkillEffectTimerSet().hasSkillEffect(EARTH_BIND) && !target.getSkillEffectTimerSet().hasSkillEffect(ICE_LANCE)
									&& !target.getSkillEffectTimerSet().hasSkillEffect(EYE_OF_DRAGON)) {
								target.receiveDamage(_user, ran2, false);
							}
						}
					} else if (_target instanceof L1NpcInstance) {
			...
			if (_skill.getTarget().equals("attack") && _skillId != 18) {
				if (isPcSummonPet(_target)) {
					if (CharPosUtil.getZoneType(_player) == 1 || CharPosUtil.getZoneType(_target) == 1 || _player.skillismiss
							|| _player.checkNonPvP(_player, _target)) {
			...
			} else if (_skill.getTarget().equals("buff") && _skillId == SHADOW_STAFF) {
				...
				if (_skillId != 5 && _skillId != 69 && _skillId != 131) {
					...
					if (_skillId == INVISIBILITY) {
						...
						if (_skillId == COUNTER_BARRIER) {
							if (_player.COUNTER_BARRIER_VETERAN) {
								castgfx = 18965;
							}
						}
						if (_skillId == REDUCTION_ARMOR) {
							if (_player.REDUCTION_ARMOUR_VETERAN) {
								castgfx = 18966;
							}
						}

						if (_skillId == DOUBLE_BRAKE) {
							if (_player.DOUBLE_BREAK_DESTINY) {
								castgfx = 17224;
							}
						}

						if (_skillId == ARMOR_BREAK) {
							if (_player.ARMOUR_BREAK_DESTINY) {
								castgfx = 17226;
							}
						}

						// Skill sound output and timer settings for each state by white
						...
						if (_skillId == GLOWING_WEAPON) {
							_player.sendPackets(new S_SkillIconAura(113, _getBuffIconDuration), true);
						} else if (_skillId == BRAVE_MENTAL) {
							_player.sendPackets(new S_NewSkillIcons(L1SkillId.BRAVE_MENTAL, true, _getBuffIconDuration), true);
			...
			/* Skill action processing End */

			if (_skillId == REDUCE_WEIGHT || _skillId == DECREASE_WEIGHT || _skillId == COUNTER_MIRROR) {
	...
	/** Skills that cannot be stacked **/
	private static final int[][] repeatedSkills = {
			{ FIRE_WEAPON, WIND_SHOT, STORM_EYE, BURNING_WEAPON, STORM_SHOT, BLOWATTACK },
			{ SHIELD, IRON_SKIN },
			{ DECREASE_WEIGHT, /*ELVEN_GRAVITY,*/ REDUCE_WEIGHT },
			{ HOLY_WALK, BLOOD_LUST, MOVING_ACCELERATION, SANDSTORM, HURRICANE, STATUS_FRUIT, STATUS_BRAVE, STATUS_ELFBRAVE, FIRE_BLESS, FOCUS_WAVE },
			{ HASTE, GREATER_HASTE, STATUS_HASTE },
			{ PRIDE, GIGANTIC, ADVANCE_SPIRIT },
		...
		try {
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter.hasNext();) {
				...
				if (_skill.getType() == L1Skills.TYPE_ATTACK && _user.getId() != cha.getId()) {
					...
					if (_skill.getSkillId() != L1SkillId.TRIPLE_ARROW && _skill.getSkillId() != L1SkillId.FOE_SLAYER) {
					...
					if (cha.getSkillEffectTimerSet().hasSkillEffect(10518) || cha.getSkillEffectTimerSet().hasSkillEffect(DEATH_HEAL) || cha.isPhantomDeathed()) {
				...
				// Check skill timer registration and skill check
				try {
					if (cha.getSkillEffectTimerSet().hasSkillEffect(_skillId) && _skillId != TOMAHAWK && _skillId != BONE_BREAK && _skillId != POWER_GRIP
							&& _skillId != DESPERADO && _skillId != SHOCK_STUN && _skillId != THUNDER_GRAB && _skillId != DEATH_HEAL && _skillId != ABSOLUTE_BLADE
							&& _skillId != SOUL_BARRIER && _skillId != TITAN_RISING && _skillId != EMPIRE && _skillId != IMPACT && _skillId != FORCE_STUN
							&& _skillId != DEMOLITION && _skillId != ETERNITY && _skillId != SHADOW_STAFF && _skillId != PANTERA && _skillId != PHANTOM) {
				...
				// ●●●● Skills that work for both PC and NPC ●●●●
				// GFX Check (Made by HuntBoy)
				switch (_skillId) {
				/* Monarch skill processing START */
				case GRACE_AVATAR: { // Skill Modification by white
					L1PcInstance pc = (L1PcInstance) cha;
					...
					if (pc.isInParty()) {
						...
						// If there are no party members, buff effect only for yourself
						int c = 5; // basic probability
						if (pc.getLevel() > 80) {
							c += (pc.getLevel() - 80);
						}
					...
					if (pc.getClanid() != 0) {
						...
						for (L1PcInstance member : L1World.getInstance().getVisiblePlayer(pc, 13)) {
							if (pc.getClanid() == member.getClanid()) {
								...
								member.PRIME = 5;
								member.addTechniqueHit(pc.PRIME);
								// Prime skill buff effect applied
								member.sendPackets(new S_SPMR(member), true);
								member.sendPackets(new S_SystemMessage("Prime: Melee Damage/Hit +3, Ranged Damage/Hit +3, Spell Power +2, Skill Hit +2."));
								// Show prime effect icon
								member.sendPackets(new S_NewSkillIcons(L1SkillId.PRIME, true, _getBuffIconDuration), true);
							}
						}
					}
				...
				case AWAKEN_LINDVIOR:
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.addDg(7);
						pc.sendPackets(new S_OwnCharAttrDef(pc));
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.AWAKEN_LINDVIOR, true, 600));
				...
				case AREA_OF_SILENCE:
					if (cha instanceof L1PcInstance) {
						...
						if (attackLevel >= defenseLevel) {
							probability = (attackLevel - defenseLevel) * 5 + 50;
						} else if (attackLevel < defenseLevel) {
							probability = (attackLevel - defenseLevel) * 6 + 50;
						}
				...
				case SAEL: {
					L1PcInstance pc = (L1PcInstance) cha;
					if (pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.CRAY)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(L1SkillId.CRAY);
					}
					pc.addMaxHp(100);
					pc.addMaxMp(50);
					pc.addHpr(3);
				...
				case CRAY: {// 크레이 버프
					L1PcInstance pc = (L1PcInstance) cha;
					if (pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.SAEL)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(L1SkillId.SAEL);
					}
					pc.getAC().addAc(-8);
				...
				case ILLUSION_OGRE: { // 일루젼 오거
					...
					} else if (pc.getSkillEffectTimerSet().hasSkillEffect(CUBE_OGRE)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(CUBE_OGRE);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_OGRE, false, 128));
					}
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								} else if (paty.getSkillEffectTimerSet().hasSkillEffect(CUBE_OGRE)) {
									paty.getSkillEffectTimerSet().removeSkillEffect(CUBE_OGRE);
									paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_OGRE, false, 128));
								}

								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_OGRE, _getBuffIconDuration * 1000);
								}

								paty.addDmgup(4);
								paty.addHitup(4);
								paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_OGRE, true, 128));
						...
						pc.addDmgup(4);
						pc.addHitup(4);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_OGRE, true, 128));
					}
				}
					break;
				...
				case ILLUSION_LICH: { // 일루젼 리치
					...
					} else if (pc.getSkillEffectTimerSet().hasSkillEffect(CUBE_LICH)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(CUBE_LICH);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_LICH, false, 128));
					}
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								} else if (paty.getSkillEffectTimerSet().hasSkillEffect(CUBE_LICH)) {
									paty.getSkillEffectTimerSet().removeSkillEffect(CUBE_LICH);
									paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_LICH, false, 128));
								}

								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_LICH, _getBuffIconDuration * 1000);
								}

								paty.getAbility().addSp(2);
								paty.sendPackets(new S_SPMR(paty), true);
								paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_LICH, true, 128));
						...
						pc.getAbility().addSp(2);
						pc.sendPackets(new S_SPMR(pc), true);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_LICH, true, 128));
					}
				}
					break;

				case ILLUSION_GOLEM: { // 일루젼 골렘
					L1PcInstance pc = (L1PcInstance) cha;
					/** 중복 버프 효과 일류션 큐브 확인 중복이라면 삭제 */
					if (pc.getSkillEffectTimerSet().hasSkillEffect(ILLUSION_GOLEM)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(ILLUSION_GOLEM);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.ILLUSION_GOLEM, false, 128));
					} else if (pc.getSkillEffectTimerSet().hasSkillEffect(CUBE_GOLEM)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(CUBE_GOLEM);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_GOLEM, false, 128));
					}
					pc.getAC().addAc(-8);
					pc.sendPackets(new S_OwnCharAttrDef(pc), true);
					pc.sendPackets(new S_NewSkillIcons(L1SkillId.ILLUSION_GOLEM, true, 128));
				}
					break;
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								/** 중복 버프 효과 일류션 큐브 확인 중복이라면 삭제 */
								if (paty.getSkillEffectTimerSet().hasSkillEffect(ILLUSION_GOLEM)) {
									paty.getSkillEffectTimerSet().removeSkillEffect(ILLUSION_GOLEM);
									paty.sendPackets(new S_NewSkillIcons(L1SkillId.ILLUSION_GOLEM, false, 128));
								} else if (paty.getSkillEffectTimerSet().hasSkillEffect(CUBE_GOLEM)) {
									paty.getSkillEffectTimerSet().removeSkillEffect(CUBE_GOLEM);
									paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_GOLEM, false, 128));
								}

								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_GOLEM, _getBuffIconDuration * 1000);
								}
								paty.getAC().addAc(-8);
								paty.sendPackets(new S_OwnCharAttrDef(paty), true);
								paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_GOLEM, true, 128));
						...
						pc.getAC().addAc(-8);
						pc.sendPackets(new S_OwnCharAttrDef(pc), true);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_GOLEM, true, 128));
					}
				}
					break;
				...
				case ILLUSION_AVATAR: { // 일루젼 아바타
					...
					} else if (pc.getSkillEffectTimerSet().hasSkillEffect(CUBE_AVATAR)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(CUBE_AVATAR);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_AVATAR, false, 128));
					}
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								} else if (paty.getSkillEffectTimerSet().hasSkillEffect(CUBE_AVATAR)) {
									paty.getSkillEffectTimerSet().removeSkillEffect(CUBE_AVATAR);
									paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_AVATAR, false, 128));
								}

								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_AVATAR, _getBuffIconDuration * 1000);
								}
								paty.addDmgup(10);
								paty.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_AVATAR, true, 128));
						...
						pc.addDmgup(10);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CUBE_AVATAR, true, 128));
					}
				}
					break;
				...
				case ANTA_SKILL_1: { // 안타라스(리뉴얼) - 용언 스킬1
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 10, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_2: { // 용언스킬 2 티 세토르
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 11, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_3: { // 용언스킬 3 뮤즈삼
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 뮤즈삼 점프+스턴;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 12, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_4: { // 용언스킬 3 너츠삼
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 너츠삼 운석+스턴;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 13, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_5: { // 용언스킬 3 너츠삼
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 티프삼 점프+스턴+운석;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 14, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_6: { // 용언스킬 리라프
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 리라프 웨폰;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 15, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_7: { // 용언스킬 켄 로우
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 켄 로우 웨폰;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 16, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_8: { // 안타라스(리뉴얼) - 용언 스킬8
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 티기르 원투+고함;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 17, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_9: { // 안타라스(리뉴얼) - 용언 스킬9
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 켄 티기르 원투+브레스;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 18, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_10: { // 안타라스(리뉴얼) - 용언 스킬9
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 루오 타 독구름+고함;
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 19, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_11: { // 안타라스(리뉴얼) - 용언 스킬9
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 케네시 캔슬+마비
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 20, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_12: { // 안타라스(리뉴얼) - 용언 스킬9
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 뮤즈심 뮤즈 전단계
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 21, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ANTA_SKILL_13: { // 안타라스(리뉴얼) - 용언 스킬9
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 너츠심
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 22, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case ABSOLUTE_BLADE: {
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.ABSOLUTE_BLADE, true, 16));
				...
				case DEATH_HEAL: {
					if (cha instanceof L1PcInstance) {
						...
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.DEATH_HEAL, true, 8));
					}
				}
					break;
					...
					if (_calcType == PC_PC) {
						if (cha instanceof L1PcInstance) {
							...
							if(_user instanceof L1PcInstance) {
								...
								if(owner.isSkillMastery(L1SkillId.FEAR) || owner.isSkillMastery(L1SkillId.DESTROY_HORROR)) {
									pc.addDg(-20);
									pc.setDestroyFear(true);
								}

								if(owner.isSkillMastery(L1SkillId.DESTROY_HORROR)) {
				...
				case TITAN_RISING: {
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						if (pc.hasSkillEffect(L1SkillId.TITAN_RISING)) {
							cha.getSkillEffectTimerSet().removeSkillEffect(L1SkillId.TITAN_RISING);
						}
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.TITAN_RISING, true, 2400));
				...
				case ANTA_SKILL_14: { // 안타라스(리뉴얼) - 용언 스킬9
					if (_user instanceof L1NpcInstance) {
						if (!_npc.skilluse) {
							...
							// 오브 모크! 티프심
							AntharasRaid ar = AntharasRaidSystem.getInstance().getAR(_user.getMapId());
							AntharasRaidTimer antaendtime = new AntharasRaidTimer(_npc, ar, 23, 2000);// 2초
							antaendtime.begin();
						}
					}
				}
				...
				case MOB_SLOW_18: {
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						if (pc.is에틴인형() || (pc.getHasteItemEquipped() > 0)) {
							continue;
						}
					}
				...
				case SHAPE_CHANGE:
					...
					if (cha instanceof L1PcInstance) {
						...
						probability = (attackInt - (defenseMr / 6)) * 4;
				...
				case CURSE_PARALYZE:
					if (!cha.getSkillEffectTimerSet().hasSkillEffect(EARTH_BIND) && !cha.getSkillEffectTimerSet().hasSkillEffect(CURSE_PARALYZE)
							...
							&& !cha.getSkillEffectTimerSet().hasSkillEffect(ICE_LANCE) && !cha.getSkillEffectTimerSet().hasSkillEffect(EYE_OF_DRAGON)) {
						if (cha instanceof L1PcInstance) {
							int rrr = _random.nextInt(5) + 15;
							L1CurseParalysis.curse(cha, 8000, rrr * 1000);
						} else if (cha instanceof L1MonsterInstance) {
							L1CurseParalysis.curse(cha, 0, 8000);
						}
					}
					break;
				...
				case 40013: {
					if (!cha.getSkillEffectTimerSet().hasSkillEffect(EARTH_BIND) && !cha.getSkillEffectTimerSet().hasSkillEffect(CURSE_PARALYZE)
						...
						if (cha instanceof L1PcInstance) {
							...
							L1CurseParalysis.curse(cha, 5000, 8000);
						} else if (cha instanceof L1MonsterInstance) {
							Broadcaster.broadcastPacket(cha, new S_SkillSound(cha.getId(), 746));
							L1CurseParalysis.curse(cha, 0, 8000);
						}
				...
				case FREEZING_ARMOUR: {
					if (cha instanceof L1PcInstance) {
						...
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.FREEZING_ARMOUR, true, _getBuffIconDuration));
						pc.sendPackets(new S_SystemMessage("You feel the effect of Freezing Armour."));
						System.out.println("Freezing Armour call 1");
					}
				}
					break;
				...
				case DISINTEGRATE:
					if (_npc != null) {
						if (_npc.getNpcId() == 46141 || _npc.getNpcId() == 46142) {
							int ran = random.nextInt(100) + 1;
							if (ran < 3) {
								Broadcaster.broadcastPacket(_npc, new S_NpcChatPacket(_npc, "Cold wind, freezes even their spells!", 0), true);
							}
							dmg /= 3;
						}
					}
				...
				case GIGANTIC: {
					...
					int addhp = (int) (pc.getBaseMaxHp() * per);
					pc.setggHp(addhp);
					pc.addMaxHp(addhp);
					pc.sendPackets(new S_HPUpdate(pc.getCurrentHp(), pc.getMaxHp()), true);
					pc.sendPackets(new S_NewSkillIcons(L1SkillId.GIGANTIC, true, 300));
				...
				case TOMAHAWK: {// 레벨 * 2 / 6
					if (cha instanceof L1PcInstance) {
						L1PcInstance target = (L1PcInstance) cha;
						if (target.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.TOMAHAWK)) {
						...
						L1NpcInstance npc = (L1NpcInstance) cha;
						if (npc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.TOMAHAWK)) {
							if (npc.토마호크th != null) {
								npc.토마호크th.stop();
							}
						}
				...
				case THUNDER_GRAB: {
					if (cha instanceof L1PcInstance) {
						...
						if (attackLevel >= defenseLevel) {
							probability = (attackLevel - defenseLevel) * 5 + 45;
						} else if (attackLevel < defenseLevel) {
							probability = (attackLevel - defenseLevel) * 5 + 45;
						}
					...
					if (cha instanceof L1PcInstance) {
						...
						} else if (pc1.THUNDER_GRAB_BRAVE) {
				...
				case REDUCTION_ARMOR: // final
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						if (pc.REDUCTION_ARMOUR_VETERAN) {
				...
				case PRIDE: { // final
					...
					pc.sendPackets(new S_NewSkillIcons(L1SkillId.PRIDE, true, _getBuffIconDuration));
					pc.sendPackets(new S_SystemMessage("You feel the effect of Pride. Your maximum HP has increased."));
				}
					break;
				...
				case LUCIFER: {
					L1PcInstance pc = (L1PcInstance) cha;
					if(pc.LUCIFER_DESTINY && pc.getLevel() >= 85) {
				...
				case POTENTIAL: {
					...
					int addhp = (int) (pc.getMaxHp() * per);
					int addmp = (int) (pc.getMaxMp() * per);
					int addsp = (int) (pc.getAbility().getSp() * per);
					int adddg = (int) (pc.getDg() * per);
					int addmr = (int) (pc.getResistance().getAddedMr() * per);
				...
				case DESPERADO: {
					...
					if (diffLevel <= -10 && !_player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(1000) + 2000;
					} else if (diffLevel <= -10 && _player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(4000) + 3000;
					} else if (diffLevel <= -8 && !_player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(1000) + 2000;
					} else if (diffLevel <= -8 && _player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(4000) + 3000;
					} else if (diffLevel <= -6 && !_player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(1000) + 2000;
					} else if (diffLevel <= -6 && _player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(4000) + 3000;
					} else if (diffLevel <= -4 && !_player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(1000) + 2000;
					} else if (diffLevel <= -4 && _player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(4000) + 3000;
					} else if (diffLevel <= -2 && !_player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(1000) + 2000;
					} else if (diffLevel <= -2 && _player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(4000) + 3000;
					} else if (diffLevel == 0 && !_player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(1000) + 2000;
					} else if (diffLevel == 0 && _player.DESPERADOABSOLUTE) {
						_shockStunDuration = random.nextInt(4000) + 3000;
					} else if (diffLevel <= 2 && !_player.DESPERADOABSOLUTE) {
					...
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.DESPERADO_ATTACKER_LEVEL = _user.getLevel();
				...
				case BOUNCE_ATTACK: { // final
					L1PcInstance pc = (L1PcInstance) cha;
					pc.addHitup(6);
					pc.sendPackets(new S_PacketBox(S_PacketBox.BUFFICON, 11728));
				}
					break;
				case SHOCK_STUN: {
					try {
						...
						if (_user != cha && _user instanceof L1PcInstance) {
							L1PcInstance c = (L1PcInstance) _user;
							// 양손 여부 체크
							if ((c == null) || (c.getWeapon() == null) || !c.getWeapon().getItem().isTwohandedWeapon()) {
				...
				case SHADOW_STAFF: {
					try {
						...
						if (cha instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) cha;
							pc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.SHADOW_STAFF, _shockStunDuration);
							pc.sendPackets(new S_Paralysis(S_Paralysis.TYPE_BIND, true));
						} else if (cha instanceof L1MonsterInstance || cha instanceof L1SummonInstance || cha instanceof L1PetInstance) {
							L1NpcInstance npc = (L1NpcInstance) cha;
							npc.setParalyzed(true);
							npc.setParalysisTime(_shockStunDuration);
							npc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.SHADOW_STAFF, _shockStunDuration);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
				...
				case AVENGER: {
					try {
						...
						if (cha instanceof L1PcInstance) {
							...
							int avenger_HP1 = (int) (pc.getMaxHp() * per);
							...
							L1NpcInstance npc = (L1NpcInstance) cha;
							double per = 30.000 / 100.000;
							int avenger_HP2 = (int) (npc.getMaxHp() * per);
				...
				case 40003: // 불멸의리치, 냉혹의 아이리스 넉백스킬
					for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(_user)) {
						...
						if (pc instanceof L1RobotInstance) {
							...
							L1Teleport.로봇텔(rob, newX, newY, mapId, true);
						} else {
							pc.dx = newX;
							pc.dy = newY;
							pc.dm = mapId;
				...
				case CANCELLATION: {
					...
					for (int element : cancelForceDelete) {
						if (cha.getSkillEffectTimerSet().hasSkillEffect(element)) {
							cha.getSkillEffectTimerSet().removeSkillEffect(element);
						}
					}
				...
				if (_calcType == PC_PC || _calcType == NPC_PC) { // 여기부터
					...
					case CLEAR_MIND: {
						...
						pc.getAbility().addAddedStr((byte) 1);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.CLEAR_MIND, true, _getBuffIconDuration));
						pc.sendPackets(new S_SystemMessage("You feel the magic of Clear Mind."));
					}
						break;
					...
					case MAJESTY: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.MAJESTY, true, 300));
					}
						break;
					...
					case SHINING_ARMOUR:
						if (cha instanceof L1PcInstance) {
							...
							pc.Add_Er(10);
							pc.sendPackets(new S_PacketBox(S_PacketBox.char_ER, pc.get_PlusEr()), true);
							pc.sendPackets(new S_NewSkillIcons(L1SkillId.SHINING_ARMOUR, true, 300));
						}
						break;
					...
					case FOCUS_SPIRIT: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.addDmgup(5);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.FOCUS_SPIRIT, true, 300));
					...
					case AWAKEN_ANTHARAS: {
						...
						pc.getAC().addAc(-3);
						pc.sendPackets(new S_OwnCharAttrDef(pc), true);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.AWAKEN_ANTHARAS, true, 600));
					...
					case AWAKEN_FAFURION: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.AWAKEN_FAFURION, true, 600));
					...
					case AWAKEN_VALAKAS: {
						...
						pc.addHitup(5);
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.AWAKEN_VALAKAS, true, 600));
	...
	private boolean isNotCancelable(int skillNum) {
		return skillNum == ENCHANT_WEAPON || skillNum == BLESSED_ARMOR || skillNum == ABSOLUTE_BARRIER || skillNum == ADVANCE_SPIRIT || skillNum == MOBIUS
				|| skillNum == SHOCK_STUN || skillNum == SHADOW_STAFF || skillNum == PANTERA || skillNum == GIGANTIC || skillNum == FORCE_STUN || skillNum == DEMOLITION || skillNum == ETERNITY
				|| skillNum == PRIME || skillNum == HALPHAS || skillNum == POTENTIAL || skillNum == PHANTOM || skillNum == POWER_GRIP || skillNum == DESPERADO
				|| skillNum == EMPIRE || skillNum == AWAKEN_LINDVIOR || skillNum == STATUS_UNDERWATER_BREATH || skillNum == INFERNO || skillNum == SHADOW_FANG
				|| skillNum == REDUCTION_ARMOR || skillNum == SOLID_CARRIAGE || skillNum == COUNTER_BARRIER || skillNum == INFERNO || skillNum == SHADOW_ARMOR
				|| skillNum == ARMOR_BREAK || skillNum == DRESS_EVASION || skillNum == UNCANNY_DODGE || skillNum == AWAKEN_ANTHARAS
				|| skillNum == AWAKEN_FAFURION || skillNum == AWAKEN_VALAKAS || skillNum == BOUNCE_ATTACK || skillNum == ILLUSION_OGRE
				|| skillNum == ILLUSION_LICH || skillNum == ILLUSION_GOLEM || skillNum == ILLUSION_AVATAR || skillNum == STATUS_DRAGONPERL
				|| skillNum == PRIDE || skillNum == BLOWATTACK || skillNum == CUBE_OGRE || skillNum == CUBE_GOLEM || skillNum == CUBE_LICH
				|| skillNum == CUBE_AVATAR;
	}
	...
	private boolean isTargetFailure(L1Character cha) {
		...
		if ((_skillId == TURN_UNDEAD && (undeadType == 0 || undeadType == 2)) || (_skillId == TURN_UNDEAD && !isTU)
				|| ((_skillId == ERASE_MAGIC || _skillId == SLOW || _skillId == MOB_SLOW_1 || _skillId == MOB_SLOW_18
				|| _skillId == MANA_DRAIN || _skillId == WIND_SHACKLE) && !isErase) || (_skillId == MANA_DRAIN && !isManaDrain)) {
			return true;
		}
		return false;
	}
	...
	int cancelForceDelete[] = { STATUS_CURSE_BARLOG, STATUS_CURSE_YAHEE/* , STATUS_DRAGONPERL */ };
	...
	private static final double THIRDSPEED_RATE = 0.874; // by 사부
	...
	private static final int[] stunTimethunder2 = { 1000, 1000, 1000, 1000, 1000, 1500, 1500, 1500, // 5
	...
	private static final int[] partyApplySkillArray = { ARMOR_BREAK, GRACE_AVATAR, SHINING_SHIELD };
