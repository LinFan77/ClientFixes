...
public class AttackSkill {
	...
	private boolean isSkillUsable() {
		...
		if (_target instanceof L1PcInstance) {
			...
			if (_skillId == L1SkillId.DESPERADO
					|| _skillId == L1SkillId.SHOCK_STUN
					|| _skillId == L1SkillId.FORCE_STUN
					|| _skillId == L1SkillId.EMPIRE
		...
		if (!CharPosUtil.isAreaAttack(_player, _target.getX(), _target.getY(), _target.getMapId()) || !CharPosUtil.isAreaAttack(_target, _player.getX(), _player.getY(), _player.getMapId()) || !isHPMPConsume()) {
			return false;
		}
		return true;
	}
	...
	private boolean isHPMPConsume() {
		...
		if (_skillId >= HEAL && _skillId <= ICE_SPIKE) {
			...
			double balance = CalcStat.calcMagicBonus(_player.getAbility().getTotalInt()) * 0.01;
