...
public class Enchant extends L1ItemInstance {
	...
	public void SuccessEnchant(L1PcInstance pc, L1ItemInstance item, int i) {
		...
		if (item.getItem().getType2() == 1 && newEnchantLvl >= 10) {
			L1World.getInstance().broadcastPacketToAll(new S_ACTION_UI(S_ACTION_UI.LIMAWORLDMESSAGE, 4444, item), true);
		}
		/** 악세 7이상 인첸시 메세지 출력 **/ // by.초작살미남
		if (item.getItem().getType2() == 2) {
			if (item.getItem().getType() >= 8 && item.getItem().getType() <= 12) {
				if (newEnchantLvl >= 7) {
					L1World.getInstance().broadcastPacketToAll(new S_ACTION_UI(S_ACTION_UI.LIMAWORLDMESSAGE, 4445, item), true);
				}
		...
		if (newEnchantLvl > safe_enchant) {
			if (Config.enchantChatMonitor() > 0) {
				for (L1PcInstance gm : Config.toArrayEnchantChatMonitor()) {
					...
					gm.sendPackets(new S_SystemMessage("\\fV[" + pc.getName() + "] " + item.getName() + " : " + oldEnchantLvl + "->" + newEnchantLvl + " [인첸 성공]"), true);
				}
			}
		}
