...
public class ThunderWand extends L1ItemInstance {
	...
	private int virtue(L1PcInstance pc) {
		int virtue = 0;
		/** magic doll stone golem **/
		if (pc.getDollList().size() > 0) {
			for (L1DollInstance doll : pc.getDollList()) {
				virtue += doll.getDamageReductionByDoll();
			}
		}
		virtue += pc.getDamageReductionByArmor(); // Damage Reduction by AC
		...
		if (pc.getSkillEffectTimerSet().hasSkillEffect(REDUCTION_ARMOR)) {
			...
			/*int dmg2 = (targetPcLvl - 50) / 3 - 3;
			virtue += dmg2 > 0 ? dmg2 : 0;// +1*/
			virtue -= (targetPcLvl - 50) / 5 + 1;
		}
