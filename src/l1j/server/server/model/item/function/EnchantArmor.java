...
public class EnchantArmor extends Enchant {
	...
	public void clickItem(L1Character cha, ClientBasePacket packet) {
		if (cha instanceof L1PcInstance) {
			...
			if (l1iteminstance1.getBless() >= 128) { // Seal item
				pc.sendPackets(new S_SystemMessage("Nothing happened."), true);
				return;
			}
			...
			/** Knight's Accessory Magic Scroll **/
			...
			if (itemId == 5000500 || itemId == 60776) {
				if (((armorId >= 500007 && armorId <= 500010)
					...
					if (enchant_level >= Config.RUMTISMAXENCHANT) {
						pc.sendPackets(new S_SystemMessage("\\fW룸티스 귀걸이는 "
								+ Config.RUMTISMAXENCHANT + "이상은 인챈트 할 수 없습니다."), true);
						return;
					}
				} else if (((armorId >= 500007 && armorId <= 500010)
						|| (armorId >= 502007 && armorId <= 502010)) && itemId == 60776) {
					if (enchant_level >= Config.PROTECTIONSPELLMAXENCHANT) {
						pc.sendPackets(new S_SystemMessage("\\fW룸티스 보호 주문서는 "
								+ Config.PROTECTIONSPELLMAXENCHANT + "이상은 인챈트 할 수 없습니다."), true);
						return;
					}
				} else {
					pc.sendPackets(new S_SystemMessage("Nothing happened."),
			...
			if (itemId == 60417 || itemId == 60774) {
				if (armorId >= 21246 && armorId <= 21253 && itemId == 60417) {
					if (enchant_level >= Config.SNAPPERMAXENCHANT) {
						pc.sendPackets(new S_SystemMessage("\\fW스냅퍼반지는 "
								+ Config.SNAPPERMAXENCHANT + "이상은 인챈트 할 수 없습니다."), true);
						return;
					}
			...
			/** 티셔츠 전용 강화 주문서 */
			if (enchant_level >= Config.TRINKETMAXENCHANT) {
				if (armortype >= 8 && armortype <= 12) { // 강도 : 특
					pc.sendPackets(new S_SystemMessage("\\fW장신구는 "
							+ Config.TRINKETMAXENCHANT + "이상은 인챈트 할 수 없습니다."), true);
					return;
				}
			}
			...
			if (itemId == 500302) { // 고대의 서
			...
			} else if (itemId == L1ItemId.C_SCROLL_OF_ENCHANT_ARMOR || itemId == 2430041 || itemId == 2430042
				...
				if (itemId == 60774 || itemId == 60776 || itemId == 60778 || itemId == 60780) {
					if(enchant_level == 0) {
						chance = (chance*10) - Config.PROTORDERPROBABILITY0;
					} else if(enchant_level == 1) {
						chance = (chance*10) - Config.PROTORDERPROBABILITY1;
					} else if (enchant_level == 2) {
						chance = (chance*10) - Config.PROTORDERPROBABILITY2;
					} else if (enchant_level == 3) {
						chance = (chance*10) - Config.PROTORDERPROBABILITY3;
					} else if (enchant_level == 4) {
						chance = (chance*10) - Config.PROTORDERPROBABILITY4;
					} else if (enchant_level == 5) {
						chance = (chance*10) - Config.PROTORDERPROBABILITY5;
					} else if (enchant_level == 6) {
						chance = (chance*10) - Config.PROTORDERPROBABILITY6;
					}
				}
