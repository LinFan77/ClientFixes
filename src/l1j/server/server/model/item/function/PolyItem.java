...
public class PolyItem extends L1ItemInstance {
	...
	private void usePolyItem(L1PcInstance pc, int itemId) {
		...
		int polyId = 0;
		int time = 0;
		if (itemId == 60125) { // Sema Transformation Scroll
			polyId = 8168;
			time = polyLawfulTime(pc.getLawful(), 600, 200);
		} else if (itemId == 60264) { // Psy Gangnam Style
			polyId = 11232;
			time = polyLawfulTime(pc.getLawful(), 1200, 400);
		} else if (itemId == 60265) { // psy bird
			polyId = 11236;
			time = polyLawfulTime(pc.getLawful(), 1200, 400);
		...
		} else if (itemId == 500003) {// Little Blue
			polyId = 10869;
			time = 2700;
		} else if (itemId == 500004) {// little yellow
			polyId = 10871;
			time = 2700;
		} else if (itemId == 500005) {// little pink
			polyId = 10870;
			time = 2701;
			...
			if (pc.getLevel() >= 85) {
				...
				pc.sendPackets(new S_SystemMessage("Only available at level 85 and above."), true);
			}
			return;
		}
