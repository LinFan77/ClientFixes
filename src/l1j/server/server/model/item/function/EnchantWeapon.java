...
public class EnchantWeapon extends Enchant {
	...
	public void clickItem(L1Character cha, ClientBasePacket packet) {
		if (cha instanceof L1PcInstance) {
			...
			if (l1iteminstance1 == null || l1iteminstance1.getItem().getType2() != 1 || (l1iteminstance1.getBless() >= 128)) { // Seal item
				pc.sendPackets(new S_SystemMessage("Nothing happened."), true);
				return;
			}
			int safe_enchant = l1iteminstance1.getItem().get_safeenchant();
			if (safe_enchant < 0) { // cannot be enhanced
				pc.sendPackets(new S_SystemMessage("Nothing happened."), true);
				return;
			}
			int weaponId = l1iteminstance1.getItem().getItemId();
			if (weaponId >= 246 && weaponId <= 249) { // cannot be enhanced
				if (itemId == L1ItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON) {// scroll of trials
				} else {
					pc.sendPackets(new S_SystemMessage("Nothing happened."), true);
					return;
				}
			...
			/** Fantasy Weapon Magic Scroll **/
			if (weaponId >= 413000 && weaponId <= 413007) { // Cannot be strengthened other than
				if (itemId == L1ItemId.SCROLL_OF_ENCHANT_FANTASY_WEAPON) { // Fantasy Weapon Magic Scroll
				} else {
					pc.sendPackets(new S_SystemMessage("Nothing happened."), true); // \f1 nothing happened.
					return;
				}
			}
			...
			if (itemId == 60473) {// Fantasy Weapon Magic Scroll (10 swords)
				if (weaponId >= 284 && weaponId <= 290) { // Cannot be strengthened other than
				} else {
					pc.sendPackets(new S_SystemMessage("Nothing happened."), true);
					return;
				}
			...
			/** Fantasy Weapon Magic Scroll **/
			if (weaponId >= 90085 && weaponId <= 90092) { // Cannot be strengthened other than
				if (itemId == 160510) {// Fantasy Weapon Magic Scroll (10 swords)
				} else {
					pc.sendPackets(new S_SystemMessage("Nothing happened."), true);
					return;
				}
			...
			if (itemId == 500300) { // ancient book
				if (l1iteminstance1 != null && l1iteminstance1.getItem().getType2() == 1 || l1iteminstance1.getItem().getType2() == 2) {
					...
					if (enchant_level >= 12) { // Knowing the number of non-reinforcements...
						pc.sendPackets(new S_SystemMessage("\\fWCannot be enchanted further due to enchantment restrictions."));
						return;
					}
					Random random = new Random();
					int k3 = random.nextInt(100);
					if (k3 <= 5) { // 15% chance of being -1
						SuccessEnchant(pc, l1iteminstance1, -1);
						pc.sendPackets(new S_SystemMessage("\\fYThe item's enchantment value has been reduced by -1."));
						pc.getInventory().removeItem(useItem, 1);
					}
					if (k3 >= 6 && k3 <= 21) { // +1 5% chance of being
						SuccessEnchant(pc, l1iteminstance1, RandomELevel(l1iteminstance1, itemId));
						pc.sendPackets(new S_SystemMessage("\\fWThe enchantment value of the item has increased by +1."));
						pc.getInventory().removeItem(useItem, 1);
					}
					if (k3 >= 22 && k3 <= 100) { // know the odds
						pc.sendPackets(new S_SystemMessage("\\fWYou didn't feel any change."));
						pc.getInventory().removeItem(useItem, 1);
					}
			...
			if (itemId == 500301) { // Ancient Tome 100%
				if (l1iteminstance1 != null && l1iteminstance1.getItem().getType2() == 1 || l1iteminstance1.getItem().getType2() == 2) {
					// 1은 무기고 2는 방어구 입니다. 무기 고대의서 방어구 고대의 서 따로 만드실 분은 여기서 알아서 응용해서 하시면 됩니다..
			...
			if (itemId == 60510) { // Craftsman's Weapon Magic Scroll
				int chance = 0;
				...
				if (enchant_level == 9) {
					...
					if (_random.nextInt(100) < Config.ARTISANPROBABILITY + chance) {
						SuccessEnchant(pc, l1iteminstance1, 1);
					} else
	...
	private void AttrChange(L1PcInstance pc, L1ItemInstance item, int itemId) {
		...
		if (itemId == 60355) { // Wind Spirit Attribute Transformation
		...
		} else if (itemId == 60356) {// command property conversion
		...
		} else if (itemId == 60357) {// Recipient Attribute Transformation
		...
		} else if (itemId == 60358) { // Hwaryeong attribute conversion
