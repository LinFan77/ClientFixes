	...
	private void init() {
		for (Item each : getItems()) {
			...
			if (ItemTable.getInstance().getTemplate(each.getItemId()) == null) {
				...
				_log.warning("item ID " + each.getItemId() + "No template found.");
			}
		}
		if (getTotalChance() != 0 && getTotalChance() != 1000000) {
			_log.warning("ID " + getBoxId() + "do not add up to 100% of the probabilities.");
		}
	}

	public static void load() {
		PerformanceTimer timer = new PerformanceTimer();
		System.out.print("[L1TreasureBox] loading...");
		try {
						...
						if (item != null) {
							...
							if ((item.getItemId() >= 450008 && item.getItemId() <= 450013) || item.getItemId() == 450020) {
								...
								if (item.getItemId() == 266) {//
									...
									random = new Random(System.nanoTime());
									item.setAttrEnchantLevel(at[random.nextInt(4)]);
								} else if (item.getItemId() == 6001) { // cold key
									int[] at = { 3, 6, 9, 12 };
									random = new Random(System.nanoTime());
									item.setAttrEnchantLevel(at[random.nextInt(4)]);
