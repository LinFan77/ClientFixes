...
public class GeneralThreadPool {
	...
	private ScheduledExecutorService _pcScheduler; // ScheduledExecutorService for the player's monitor
	//Once in the L1J default state, the execution time occupied in 1 second by a PC that is not doing anything in map:4 is about
	// 6ms (AutoUpdate: about 6ms, ExpMonitor: minuscule)
	private final int _pcSchedulerPoolSize = 1 + Config.MAX_ONLINE_USERS / 20; //Appropriate (assignment of about 1 to 20 users)
	...
	private GeneralThreadPool() {
		...
		_scheduler = Executors.newScheduledThreadPool(SCHEDULED_CORE_POOL_SIZE, new PriorityThreadFactory("GerenalSTPool",
						Thread.NORM_PRIORITY));
		_pcScheduler = Executors.newScheduledThreadPool(_pcSchedulerPoolSize, new PriorityThreadFactory("PcMonitorSTPool",
						Thread.NORM_PRIORITY));
	}
	...
	// Dispatch from ThreadPoolManager
