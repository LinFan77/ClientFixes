...
public class CraftTable {
	...
	private ConcurrentHashMap<Integer, Craft> list = new ConcurrentHashMap<>();
	...
	public void store_item() {
		try {
			...
			System.out.println("[CraftTable Data] : " + list.size() + " 개 loading...OK!");
	...
	public Craft get_template(int action, int descid) {
		...
		for (Craft element : craft) {
			if (element.get_action() == action && element.get_descid() == descid)  return element;
		}
		return null;
	}

	public Craft[] get_template() {
		return list.values().toArray(new Craft[list.size()]);
	}
}
