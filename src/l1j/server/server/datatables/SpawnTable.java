...
public class SpawnTable {
	...
	public static void reload() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (spawnDat == null) {
					System.out.println("Null spawnID: " + spawnid);
					continue;
				}
	...
	private void fillSpawnTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				if ((!Config.ALT_HALLOWEENEVENT && spawnid == 1) || (!Config.ALT_CHUSEOKEVENT && spawnid == 2))
					continue;
				if (!Config.ALT_HALLOWEENEVENT2009 && spawnid == 3)
					continue;
				...
				if (template1 == null) {
					...
					if (count > 1 && spawnDat.getLocX1() == 0) {
						// In the case of multiple or fixed spawns, it is changed to spawns in the range of the number of objects * 6.
						// but don't let the range exceed 30
		...
		_log.fine("Total Monsters " + spawnCount);
	}
	...
	public void addNewSpawn(L1Spawn spawn) {
	    ...
	    _spawntable.putIfAbsent(spawn.getId(), spawn);
	    //_spawntable.put(spawn.getId(), spawn); //tek123
	}
