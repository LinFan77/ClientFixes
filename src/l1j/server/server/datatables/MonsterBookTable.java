...
public class MonsterBookTable {
	...
	private class Mbl {
		private HashMap<Integer, Integer> _monlist = new HashMap<>();
		private HashMap<Integer, Integer> _monquest = new HashMap<>();
	}
	...
	private ConcurrentHashMap<Integer, Mbl> _monsterBookLists = new ConcurrentHashMap<>();
	private ConcurrentHashMap<Integer, Mblt> _monBookTellList = new ConcurrentHashMap<>();
	private ConcurrentHashMap<Integer, Integer> _monsterList = new ConcurrentHashMap<>();
	...
	private MonsterBookTable() {
		...
		System.out.print("[MonsterBookTable] loading...");
		loadMonsterBookList();
	...
	public void saveMonsterBookList(int id) {
		...
		try (Connection con = L1DatabaseFactory.getInstance().getConnection();
			...
			if (mbl != null) {
				TreeMap<Integer, Integer> tree = new TreeMap<>(mbl);
