	...
	private void fillNpcSpawnTable() {
		...
		try {
			...
			do {
				...
				// Test Server NPCs
				if ((!Config.TEST_SERVER && npcTemplateid == 100395) || (!Config.TEST_SERVER && npcTemplateid == 4500162)) { // Giran shop
					continue;
				}
				if (!Config.TEST_SERVER && npcTemplateid == 4500166) { // Giran shop
					continue;
				}
				if (!Config.TEST_SERVER && npcTemplateid == 4220019) { // Giran shop
					continue;
				}
				if (!Config.TEST_SERVER && npcTemplateid == 4200101) { // Giran shop
					continue;
				}
				if (!Config.TEST_SERVER && npcTemplateid == 4200100) { // Giran shop
					continue;
				}
				if (!Config.TEST_SERVER && npcTemplateid == 4500309) { // Giran board server info
					continue;
				}
				// Event Legendary Crafting
				if (!Config.LEGENDARY_CRAFTING && npcTemplateid == 60245) { // Legendary crafter
					continue;
				}
				//Black Battle Ship entry
				if (!Config.BLACK_BATTLESHIP && npcTemplateid == 102008) { // Legendary crafter
					continue;
				}
	...
	public void ReloadSpawnTable(String name) {
		...
		try {
			...
			do {
				...
				if ((!Config.ALT_HALLOWEENEVENT
						&& npcTemplateid == 4200001) || (!Config.ALT_FANTASYEVENT
						&& npcTemplateid == 4200500)) {
					continue;
				}
