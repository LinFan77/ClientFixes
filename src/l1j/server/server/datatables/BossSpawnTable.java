		...
		try {
			...
			while (rs.next()) {
				if (rs.getString("cycle_type").equals(name)) {
							...
							if (npcId == 45488 && npc.getNpcTemplate().get_npcId() == 45488
								...
								if (Config.connectionChatMonitor() > 0) {
									for (L1PcInstance gm : Config
											.toArrayConnectionChatMonitor()) {
										if (gm.getNetConnection() == null) {
											Config.removeConnection(gm);
											continue;
										}
										gm.sendPackets(new S_SystemMessage("Boss already spawned" + npc.getName() + " x " + npc.getX()
														+ " y " + npc.getY() + " map" + npc.getMapId()));
										}
									}
								alreadySpawnCount++;
								}
					...
					if (npcId == 46142 && npc.getMapId() == 73 || npcId == 46141 && npc.getMapId() == 74) {
