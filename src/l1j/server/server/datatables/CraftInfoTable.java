...
public class CraftInfoTable {
	...
	private HashMap<Integer, L1CraftInfo> craftInfoList = new HashMap<>();
	private HashMap<Integer, int[]> craftNpcList = new HashMap<>();
	...
	private CraftInfoTable() {
		...
		System.out.print("[CraftInfoTable] loading...");
		loadCraftInfos();
		loadCraftNpcs();
		...
		try (Connection con = L1DatabaseFactory.getInstance().getConnection();
			...
			while (rs.next()) {
				...
				if (craftInfo.materialList == null || craftInfo.materialList.size() == 0) {
					System.out.println("Material item count error craftId:" + craftId);
					craftInfo = null;
				} else {
		...
		try (Connection con = L1DatabaseFactory.getInstance().getConnection();
			...
			while (rs.next()) {
				...
				if (craftList.length == 0 || craftList == null) {
					System.out.println("Production list count error npcId:" + npcId);
				} else {
	...
	private ArrayList<Material> materialList(L1CraftInfo ci, String materialList) {
		try {
			...
			ArrayList<Material> result = new ArrayList<>(); //여기에는 재료가 담긴다. 정보화해서.
