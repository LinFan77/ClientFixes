...
public class EvaSystemTable {
	...
	private final Map<Integer, L1EvaSystem> _evasystem = new ConcurrentHashMap<>();
	...
	private void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				eva.bossTime = eva.getEvaTime().getTimeInMillis() + (long) 60000 * (long) 60 * 2
						+ ((long) 60000 * (long) 30);
				eva.bosscheck = eva.getEvaTime().getTimeInMillis() + (long) 60000 * (long) 60 * 4;
	...
	public void updateExtend(int i) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE evasystem SET extend=? WHERE id=?");
			pstm.setInt(1, i);
			pstm.setInt(2, 1);
			pstm.executeUpdate();
		} catch (SQLException e) {
