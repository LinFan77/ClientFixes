...
public class DropTable {
	...
	private final HashMap<Integer, ArrayList<L1Drop>> _droplists; // monster drop list
	...
	public void setDrop(L1NpcInstance npc, L1Inventory inventory) {
		...
		for (L1Drop drop : dropList) {
			// Acquisition of drop item
			itemId = drop.getItemid();
			if (adenarate == 0 && itemId == L1ItemId.ADENA) {
				continue; // Adena rate is 0, and if the drop is adena
			}

			// Drop Chance Judgment
			randomChance = random.nextInt(0xf4240) + 1; // 1,000,000 + 1
			...
			if (itemId == L1ItemId.ADENA) { // If the drop is Adena, cast Adenarate.
			if (npc.getMapId() == 410) {
					itemCount = 0;
				} else {
					itemCount *= adenarate;
			...
			// Cheerful mobs won't come to Adena
			...
			if (Litem != null) {
				if (Litem.isStackable()) {
					...
					if (drop.getEnchant() != 0) {
						System.out.println("[Error] droplist: Enchanted by overlapping items.("+ item.getItemId() + ")");
		...
		if (Config.GAME_SERVER_TYPE == 1) {
			short mapid = npc.getMapId();
			if ((mapid >= 450 && mapid <= 478) || (mapid >= 490 && mapid <= 496)
					|| (mapid >= 530 && mapid <= 536)) {
				return;
			}
...
//		}
	...
	public boolean autook(L1PcInstance pc) {
		 ...
		 */
		if ((ckpc == null) || (ckpc.getNetConnection() == null) || ckpc.getNetConnection().isClosed() || ckpc.getSkillEffectTimerSet().hasSkillEffect(
	...
	public void dropShare(L1NpcInstance npc, ArrayList<?> acquisitorList, ArrayList<?> hateList, L1PcInstance pc) {
		...
		// Get the total of your height?
			...
			for (int i = hateList.size() - 1; i >= 0; i--) {
				...
				if (acquisitor != null && acquisitor.isDead()
						&& (npc.getNpcId() == 4200011 || npc.getNpcId() == 4039007
								...
								|| npc.getNpcId() == 400017 || npc.getNpcId() == 145684
					...
					hateList.remove(i);
						...
						&& (acquisitor.getLocation().getTileLineDistance(
								...
								|| npc.getNpcId() == 145684 || npc.getNpcId() == 4036016
								|| npc.getNpcId() == 4036017))) {
					totalHate += (Integer) hateList.get(i);
				} else { // Excluded if it was null or dead and distant
					acquisitorList.remove(i);
					hateList.remove(i);
				}
			}
			...
			for (int i = inventory.getSize(); i > 0; i--) {
				...
				/** Check if it is Adena in the item information list
				 * If you have Protection, you will receive Adena. The target you have is the last attacker */
				/** Adena Acquisition Amount when possessing Protection Items  */
				if (itemId == L1ItemId.ADENA && pc != null) {
					...
					if (pc.getInventory().checkItem(42015) && pc.getInventory().checkEquippedEnchant(421220,8)) {
						...
						blessingS = 2.0;
						blessing = true;
					} else if (pc.getInventory().checkItem(42015)) { // Advanced Immortal Protection
						// TODO
						blessingS = 1.5;
						blessing = true;
					} else if (pc.getInventory().checkItem(42017)) { // Immortal Protection
						blessingS = 1.1;
						blessing = true;
					} else blessing = false;
				...
				/** Watcher Reaper -> Transformed boss mobs no longer drop amulets. **/
				if (pc != null) {
					if (itemId == 60135) { // wafer
						if (!pc.isElf()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60137) { // Jeong Ok
						if (!pc.isElf()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60138) { // Madol Wizard, Fairy
						if (!pc.isElf() && !pc.isWizard()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 240016) { // wisdom
						if (!pc.isWizard()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60133) { // bad skin
						if (!pc.isCrown()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60134) { // bravery
						if (!pc.isKnight() && !pc.isWarrior()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60136) { // Yggdra Fruit
						if (!pc.isIllusionist()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60139) { // obsidian
						if (!pc.isDarkelf()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60140) { // bone fragment
						if (!pc.isDragonknight()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
					}
					if (itemId == 60157) { // attribute stone
						if (!pc.isIllusionist()) {
							inventory.removeItem(item, item.getCount());
							continue;
						}
				...
				if (item.getItem().getType2() == 0 && item.getItem().getType() == 2) { // light type item
				...
				if ((Config.AUTO_LOOT != 0 || AutoLoot.getInstance().isAutoLoot(itemId)) && totalHate > 0) {
					...
					for (int j = hateList.size() - 1; j >= 0; j--) {
						...
						if (chanceHate > randomInt) {
							...
							// If pet, change to the owner's authority
							...
							if (acquisitor.getInventory().checkAddItem(item, item.getCount()) == L1Inventory.OK) {
								...
								if (acquisitor instanceof L1PcInstance) {
									...
									if (l1iteminstance != null && l1iteminstance.getCount() > 2000000000) {
										...
										// cant have? drop to ground
										player.sendPackets(new S_ServerMessage(166, "Your total Adena exceeds 2,000,000,000."));
									} else {
										if (npc.getNpcId() == 4200011 // Item drop message..
												|| npc.getNpcId() == 4039007 || npc.getNpcId() == 100014
												|| npc.getNpcId() == 400016 || npc.getNpcId() == 400017
												|| npc.getNpcId() == 4036016 || npc.getNpcId() == 145684
												|| npc.getNpcId() == 4036017) {
										...
										} else if (player.isInParty()) { // in party?
				...
				if (item_Id == 40222 || item_Id == 41148 || item_Id == 60199 || item_Id == 7310 || item_Id == 600512
				 ...
				 || item_Id == 5000006  || item_Id == 5000007  || item_Id == 101010  || item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783|| item_Id == 60783) {
					L1World.getInstance().broadcastPacketToAll(new S_ACTION_UI(S_ACTION_UI.LIMAWORLDMESSAGE, 403,"which Aden warrior", item), true); // Message output when eating a drop item
				}
			}
		...
		public void setPainwandDrop(L1NpcInstance npc, L1Inventory inventory) {
			...
			for (L1Drop drop : dropList) {
				// Acquisition of drop item
				itemId = drop.getItemid();
				if ((adenarate == 0 && itemId == L1ItemId.ADENA) || (itemId != L1ItemId.ADENA)) {
					continue;
				}
				...
				/** If it's Adena, it will give Possession effect to inventory */
