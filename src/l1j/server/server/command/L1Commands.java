...
public class L1Commands {
	...
	public static L1Command get(String name) {
		/*
		 * For debugging or testing ease, I go to read into the DB every time.
		 * Theoretically lower performance than caching, but negligible Range.
		 */
		Connection con = null;
		...
		try {
			...
			_log.log(Level.SEVERE, "Error retrieving command from database", e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
