...
public class L1SpawnCmd implements L1CommandExecutor {
	...
	private void sendErrorMessage(L1PcInstance pc, String cmdName) {
		String errorMsg = cmdName + " error! Please use .spawn [npcID] [count]";
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (npc == null) {
				pc.sendPackets(new S_SystemMessage("That NPC was not found in the database."));
				return;
			}
			for (int i = 0; i < count; i++) {
				L1SpawnUtil.spawn(pc, npcid, randomrange, 0, false);
			}
			String msg = String.format("%s(%d) (%d) has been summoned. (Range: %d)",
			...
			pc.sendPackets(new S_SystemMessage("GMC: " + cmdName + " exception."));
		}
	}
}
