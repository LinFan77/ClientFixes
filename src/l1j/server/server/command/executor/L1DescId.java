		...
		try {
			...
			for (int i = 0; i < count; i++) {
				if (pc.getInventory().getSize() > 255) {
					pc.sendPackets(new S_SystemMessage("\\aGThe maximum number of inventory items is 256."));
					break;
				}
				countA++;
			...
			pc.sendPackets(new S_SystemMessage("\\aHEtsy " + descid + "~" + String.valueOf(descid + countA) + " was created."));
			...
			if (count > 0) {
				pc.sendPackets(new S_SystemMessage("\\aA이티시 확인용 아이템(" + count + ")이 삭제되었습니다."));
			} else {
				pc.sendPackets(new S_SystemMessage("GMC Error: " + cmdName + " [id] [count]"));
			}
		}
	}
}