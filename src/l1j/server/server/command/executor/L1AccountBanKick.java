		...
		try {
			...
			if (target != null) {
				...
				pc.sendPackets(new S_SystemMessage(target.getName()
						+ " account has been kicked and banned."));
				target.sendPackets(new S_Disconnect());
			} else {
				pc.sendPackets(new S_SystemMessage(
						"Character with that name does not exist in the world."));
			}
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage("GMC Error: Please use " + cmdName + " [PlayerName]"));
		}
	}
}