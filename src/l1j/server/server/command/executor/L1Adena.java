	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (adena != null) {
				...
				L1World.getInstance().broadcastServerMessage(count + "\\aG Adena has been created by GameMaster: " + name);
			}
			item(pc.getName(), "adena", count);
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage((new StringBuilder()).append(
					"GMC Exception: Please use " + cmdName + " [count]").toString()));
		}
	}
