...
public class L1AutoPcShop implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		pc.sendPackets(new S_SystemMessage( " .autoshop [Option]" ));
		pc.sendPackets(new S_SystemMessage( " [1:Start 2:AddItem 3.DeleteItem 4:Close]"));
		pc.sendPackets(new S_SystemMessage( " [5:Add 6:Modify 7:Arrange 8:Hide]"));
		pc.sendPackets(new S_SystemMessage("----------------------------------------------------"));
	}
	...
	private void toShopStart(L1PcInstance pc, StringTokenizer st) {
		try {
			...
			pc.sendPackets(new S_SystemMessage("Step: " + step + " Auto Shop has been loaded."));
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(".autoshop [step]"));
		}
	}
	...
	private void toShopAddItem(L1PcInstance pc, StringTokenizer st) {
		...
		try {
			...
			try {
				...
				if (itemid == 0) {
					pc.sendPackets(new S_SystemMessage("The item was not found."));
					return;
				}
			}
			int enchant = 0;
			...
			if(!rs.next()) {
				pc.sendPackets(new S_SystemMessage(charName + " ID is not created in autoshop list table."));
				return;
			}
			...
			if (temp != null) {
				...
				pc.sendPackets(new S_SystemMessage(charName + " to the store " +  item.getName() + " item " + price + " Added a sales list to the price."));
			} else {
				pc.sendPackets(new S_SystemMessage("\\aDThe item with the specified ID does not exist."));
			}

		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(".autoshop AddItem [Name] [0:Sell,1:Purchase] [itemID/name] [enchant] [count] [price]"));
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	private void toShopDelItem(L1PcInstance pc, StringTokenizer st) {
		...
		try {
			...
			try {
				...
				itemid = ItemTable.getInstance().findItemIdByNameWithoutSpace(nameid);
				if (itemid == 0) {
					pc.sendPackets(new S_SystemMessage("\\aDThat item was not found."));
					return;
				}
			}
			int enchant = 0;
			...
			pc.sendPackets(new S_SystemMessage(".autoshop 아이템삭제 [Name] [itemID,Name] [enchantment]"));
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	private void toShopEnd(L1PcInstance pc, StringTokenizer st) {
		try {
			...
			pc.sendPackets(new S_SystemMessage(".autoshop 상점종료 [단계]"));
		}
	}
	...
	private void toShopMake(L1PcInstance pc, StringTokenizer st) {
		...
		try {
			...
			if(rs.next()) {
				pc.sendPackets(new S_SystemMessage("Ready " + charName + " ID has been created in the autoshoplist table."));
				return;
			}
			...
			pc.sendPackets(new S_SystemMessage("You have created a shop: " + shop.getName()));
			shop = null;
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(".autoshop 상점추가 [Name] [0:군주, 1:랜덤] [상점메세지]"));
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	private void toShopUpdate(L1PcInstance pc, StringTokenizer st) {
		...
		try {
			...
			pc.sendPackets(new S_SystemMessage(shop.getName() + " store has been modified."));
			shop = null;
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(".autoshop 상점수정 [Name] [0:군주, 1:랜덤] [상점메세지]"));
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	private void toShopUpdateView(L1PcInstance pc, StringTokenizer st) {
		...
		try {
			...
			pc.sendPackets(new S_SystemMessage(charName + " store has been placed."));
		} catch (Exception e) {
	...
	private void toShopUpdateHidden(L1PcInstance pc, StringTokenizer st) {
		...
		try {
			...
			pc.sendPackets(new S_SystemMessage("You have hidden your " + charName + " store."));
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(".autoshop HideShop [Name]"));
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
