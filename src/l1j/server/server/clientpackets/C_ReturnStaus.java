...
public class C_ReturnStaus extends ClientBasePacket {
	...
	public C_ReturnStaus(byte[] decrypt, LineageClient client) {
		...
		if (pc == null || pc.getReturnStat() == 0 || (pc.getReturnStatus() != type)) {
			return;
		}
		...
		if (type == 1) {
			...
			if (str > 20 || intel > 20 || dex > 20 || wis > 20 || con > 20
				...
				pc.sendPackets(new S_SystemMessage("Incorrect base stat."));
	...
	public void statup(L1PcInstance pc, L1StatReset sr) {
		...
		Stathp = CalcStat.levelUpHP(pc.getType(), sr.getMaxHp(), (byte) sr.getCon());
		Statmp = CalcStat.levelUpMP(pc.getType(), sr.getMaxMp(), (byte) sr.getWis());
		sr.setAC(CalcStat.calcACFromDEX(sr.getDex()));
