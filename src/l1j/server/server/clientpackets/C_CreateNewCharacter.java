...
public class C_CreateNewCharacter extends ClientBasePacket {
		...
		try {
			...
			for (int i = 0; i < name.length(); i++) {
				if (name.charAt(i) == 'ㄱ' || name.charAt(i) == 'ㄲ' || name.charAt(i) == 'ㄴ' || name.charAt(i) == 'ㄷ' ||
						name.charAt(i) == 'ㄸ' || name.charAt(i) == 'ㄹ' || name.charAt(i) == 'ㅁ' || name.charAt(i) == 'ㅂ' ||
						name.charAt(i) == 'ㅃ' || name.charAt(i) == 'ㅅ' || name.charAt(i) == 'ㅆ' || name.charAt(i) == 'ㅇ' ||
						name.charAt(i) == 'ㅈ' || name.charAt(i) == 'ㅉ' || name.charAt(i) == 'ㅊ' || name.charAt(i) == 'ㅋ' ||
						name.charAt(i) == 'ㅌ' || name.charAt(i) == 'ㅍ' || name.charAt(i) == 'ㅎ' || name.charAt(i) == 'ㅛ' ||
						name.charAt(i) == 'ㅕ' || name.charAt(i) == 'ㅑ' || name.charAt(i) == 'ㅐ' || name.charAt(i) == 'ㅔ' ||
						name.charAt(i) == 'ㅗ' || name.charAt(i) == 'ㅓ' || name.charAt(i) == 'ㅏ' || name.charAt(i) == 'ㅣ' ||
						name.charAt(i) == 'ㅠ' || name.charAt(i) == 'ㅜ' || name.charAt(i) == 'ㅡ' || name.charAt(i) == 'ㅒ' ||
						name.charAt(i) == 'ㅖ' || name.charAt(i) == 'ㅢ' || name.charAt(i) == 'ㅟ' || name.charAt(i) == 'ㅝ' ||
						name.charAt(i) == 'ㅞ' || name.charAt(i) == 'ㅙ' || name.charAt(i) == 'ㅚ' || name.charAt(i) == 'ㅘ' ||
			...
			if (BadNamesList.getInstance().isBadName(name)) {
				S_CharCreateStatus s_charcreatestatus = new S_CharCreateStatus(S_CharCreateStatus.REASON_INVALID_NAME);
				_log.info("Bad character name, creation failed.");
				client.sendPacket(s_charcreatestatus, true);
				return;
			}
			...
			if (CharacterTable.RobotNameExist(name) || CharacterTable.RobotCrownNameExist(name) || NpcShopSpawnTable.getInstance().getNpc(name) || npcshopNameCk(name)) {
				S_CharCreateStatus s_charcreatestatus1 = new S_CharCreateStatus(S_CharCreateStatus.REASON_ALREADY_EXSISTS);
				client.sendPacket(s_charcreatestatus1, true);
				return;
			}
			...
			if (client.getAccount().countCharacters() >= 8) {
				_log.fine("Account: " + client.getAccountName() + " Trying to create more than 8 characters.");
			...
			if (!pc.getAbility().isNormalAbility(pc.getClassId(), pc.getLevel(), pc.getHighLevel(), total)) {
				_log.finest("Character has wrong stat.");
	...
	private static void initNewChar(LineageClient client, L1PcInstance pc) throws IOException, Exception {
		...
		if (pc.isWizard()) { // WIZ
			pc.sendPackets(new S_NewAddSkill(S_NewAddSkill.ADD_SKILL_NEW, pc, 4), true);
			...
			SkillsTable.getInstance().spellMastery(object_id, skill_id, skill_name, 0, 0); // Register in DB
		}
		/*
		 * if(GameServer.신규지원_경험치지급단){ Beginner.getInstance().신규경험치지급단GiveItem(pc);
		 * }else{ Beginner.getInstance().GiveItem(pc); }
		 */
		if (Config.NEWB_EXP_ITEMS) {
		...
		if (Config.NEWB_SUPPORT_BOOKMARKS) {
			L1ItemInstance item = ItemTable.getInstance().createItem(60080);
			Beginner.getInstance().GiveItem(pc, item, false);
		}

		if (pc.isCrown()) {
			int list[] = { 9094, 60156, 430005 };
			for (int i : list) {
		...
		sb.append("[").append(client.getAccountName()).append("] created character [").append(pc.getName()).append("] [").append(pc.getClassId()).append("]");
	...
	private static boolean isInvalidName(String name, LineageClient client) {
		...
		// XXX - Unconfirmed whether it is equivalent to the specifications of the central office
		// Make the name invalid if the full-width characters exceed 5 characters, or if the total exceeds 12 bytes
