	...
	public C_ChatWhisper(byte abyte0[], LineageClient client) throws Exception {
		...
		try {
			...
			if (targetName.equalsIgnoreCase("***")) {
				...
				whisperFrom.sendPackets(sm, true);
				//LinAllManager.getInstance().WhisperChatAppend(whisperFrom.getName(), "******", text);
				return;
			}
			...
			if (whisperTo == null) {
				...
				if (npc != null) {
					S_ChatPacket scp = new S_ChatPacket(npc, text, Opcodes.S_MESSAGE, 9);
			...
			if (whisperTo instanceof L1RobotInstance) {
				S_ChatPacket scp = new S_ChatPacket(whisperTo, text, Opcodes.S_MESSAGE, 9);
				whisperFrom.sendPackets(scp, true);
				return;
			}
			S_ChatPacket scp2 = new S_ChatPacket(whisperTo, text, Opcodes.S_MESSAGE, 9);
			whisperFrom.sendPackets(scp2, true);
			S_ChatPacket scp3 = new S_ChatPacket(whisperFrom, text, Opcodes.S_TELL, 16);
			whisperTo.sendPackets(scp3, true);
			//LinAllManager.getInstance().WhisperChatAppend(whisperFrom.getName(), whisperTo.getName(), text);
			if (Config.whisperChatMonitor() > 0) {
				S_SystemMessage sm = new S_SystemMessage(whisperFrom.getName() + " -> (" + whisperTo.getName() + ") " + text);
				for (L1PcInstance gm : Config.toArrayWhisperChatMonitor()) {
