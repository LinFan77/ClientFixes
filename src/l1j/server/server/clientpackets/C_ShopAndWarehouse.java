...
public class C_ShopAndWarehouse extends ClientBasePacket {
	...
	public C_ShopAndWarehouse(byte abyte0[], LineageClient clientthread) throws Exception {
		...
		try {
			...
			if(resultType == 0||resultType == 2){
			   if(size > 1000){
				   System.out.println("Kim Hana appeared!! ["+pc.getName()+"] IP "+clientthread.getIp());
				   clientthread.kick();
			   }
			}  //추가
			...
			switch (resultType) {
				case TYPE_BUY_SHP: // 상점 or 개인 상점 사기
					if (npcId == 70035 || npcId == 70041 || npcId == 70042) {
						...
						if (status != L1BugBearRace.STATUS_READY || !chk) {
							return;
						}
					} else if (npcId == 72320) {
						int status = DogFight.getInstance().getBugRaceStatus();
						boolean chk = DogFight.getInstance().buyTickets;
						if (status != DogFight.STATUS_READY || !chk) {
							return;
						}
					}

					if (pc.getClan() == null && pc.getLevel() >= Config.BLOODLESSHOP) {
						pc.sendPackets(new S_SystemMessage(Config.BLOODLESSHOP + "레벨 이상은 혈맹이 없으면 상점을 이용할 수 없습니다."));
						if (pc.isGm()) {
				...
				case TYPE_PUT_PWH: // 개인 창고 맡기기
					if (size != 0 && npcImpl.equalsIgnoreCase("L1Dwarf")) {
						depositPersonalStorage(pc, size);
					}
					break;
	...
	private void getItemToSupplementaryService(L1PcInstance pc, int size) {
		...
		if ((warehouse == null) || (size > 100)) return;

		ArrayList<L1ItemInstance> _List = new ArrayList<>();
	...
	private void getItemToPackageWarehouse(L1PcInstance pc, int size) {
		int objectId, count;
		L1ItemInstance item = null;
		PackageWarehouse w = WarehouseManager.getInstance().getPackageStorage(pc.getAccountName());
		if (w == null) {
			return;
		}
	...
	private void getItemToElfWarehouse(L1PcInstance pc, int size) {
		...
		ElfWarehouse elfwarehouse = WarehouseManager.getInstance().getElfStorage(pc.getAccountName());
		...
		for (int i = 0, number, count; i < size; i++) {
			...
			if (!isAvailableTrade(pc, Item, count) || !isAvailablePcWeight(pc, Item, count)) break;
			if (count > Item.getCount()) count = Item.getCount();
			ArrayList<String> name = new ArrayList<>();
			name = L1PcInstance.getPCs(pc.getAccountName());
			for (L1PcInstance _pc : L1World.getInstance().getAllPlayers()) {
	...
	private void putItemToElfWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, objectId, count; i < size; i++) {
			...
			if (!checkPetList(pc, item) || !isAvailableWhCount(elfwarehouse, pc, item, count)) {
				break;
			}
	...
	private void getItemToClanWarehouse(L1PcInstance pc, int size) {
		if ((pc.getLevel() < 5) || (pc.getClanRank() == L1Clan.CLAN_RANK_PROBATION)) return;
		if (size == 0) {
	...
	private void putItemToClanWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, objectId, count; i < size; i++) {
			...
			if (!checkPetList(pc, item) || !isAvailableWhCount(clanWarehouse, pc, item, count)) {
				break;
			}
			...
			if (itemExist != null && ((itemExist.getCount() + count) > 2000000000 || (itemExist.getCount() + count) < 0)) {
				pc.sendPackets(new S_SystemMessage("The number of possessed items will exceed 2 billion."));
				return;
			}
			...
			LogTable.logPledgeStorage(pc, item, count, 0);
		}
	...
	private void getItemToPrivateWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, number, count; i < size; i++) {
			...
			count = readD();
			Item = warehouse.getItems().get(number);
			if (!isAvailableTrade(pc, Item, count) || !isAvailablePcWeight(pc, Item, count)) {
				break;
			}
	...
	private void depositPersonalStorage(L1PcInstance pc, int size) {
		L1Object object = null;
		L1ItemInstance item = null;
		PrivateWarehouse warehouse = WarehouseManager.getInstance().getPersonalStorage(pc.getAccountName());
		...
		for (int i = 0, objectId, count; i < size; i++) {
			...
			object = pc.getInventory().getItem(objectId);
			item = (L1ItemInstance) object;
			if (pc.getLevel() < Config.STORAGE_MIN_LEVEL) {
				pc.sendPackets(new S_SystemMessage(Config.STORAGE_MIN_LEVEL + "You can't use the warehouse where the level is lowered."), true);
				break;
			}
			...
			/** Deda, Deme, De Jinju, Druby, Desapa, Asmodian Weapon Protection Order, Pandora Flower Scented Tea Storage **/
			if (!((item.getItemId() >= 437010 && item.getItemId() <= 437013)
				...
				if (!item.getItem().isTradable()) {
					// \f1%0may not be discarded or transferred to another person.
					pc.sendPackets(new S_ServerMessage(210, item.getItem().getName()), true);
					break;
				}
			}
			...
			if (okok(item.getItemId())) {
				pc.sendPackets(new S_SystemMessage("This item is unavailable for storage for the time being."), true);
				break;
			}
			...
			if (item.getItem().getItemId() == 430116
				...
				pc.sendPackets(new S_SystemMessage("This item cannot be used in the warehouse."), true);
				break;
			}

			if (!isAvailableTrade(pc, objectId, item, count) || !checkPetList(pc, item) || !isAvailableWhCount(warehouse, pc, item, count)) {
				break;
			}
		...
		synchronized (buyList) {
			for (int i = 0; i < size; i++) {
				...
				if (targetPc.getInventory().checkAddItem(item, count) == L1Inventory.OK) { // 용량 중량 확인 및 메세지 송신
					...
					if ((item.getItemId() != buyItem.getItemId()) || (!item.isStackable() && count !=1)) return;
	...
	private void buyItemFromPrivateShop(L1PcInstance pc, L1Object findObject, int size) {
		...
		synchronized (sellList) {
			...
			if ((pc.getPartnersPrivateShopItemCount() != sellList.size()) || (pc.getPartnersPrivateShopItemCount() < sellList.size())) {
				return;
			}
