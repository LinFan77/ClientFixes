...
public class C_DeleteChar extends ClientBasePacket {
	...
	public C_DeleteChar(byte decrypt[], LineageClient client) throws Exception {
		...
		try {
			...
			if (pc == null) {
				client.sendPacket(new S_Notice("This character does not exist."));
				return;
			}

			for (L1PcInstance target : L1World.getInstance().getAllPlayers3()) {
				if (target.getId() == pc.getId()) {
					client.sendPacket(new S_Notice("Characters cannot be deleted while connected."));
					return;
				}
			}
			...
			if (pc != null && pc.getLevel() >= 75 && Config.DELETE_CHARACTER_AFTER_7DAYS) {
				if (pc.getType() < 32) {
					...
					Timestamp deleteTime = new Timestamp(System.currentTimeMillis() + 604800000);
			...
			String msg = ("Character [" + name + "] deleted from IP: " + client.getIp());
			ErrorMessage.reportPlayerAction(msg);
