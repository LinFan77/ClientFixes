		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (item != null) {
					...
					if (!pc.isGm() && pc.getLevel() < Config.DROPLEVEL) {
						pc.sendPackets(new S_SystemMessage("레벨 " + Config.DROPLEVEL + "부터 버릴 수 있습니다."));
						return;
					}
					...
					//LinAllManager.getInstance().pickUpAppend(item.getLogName(), pc.getName(), count, 0 /*LinAllManager.Item_Drop*/);
