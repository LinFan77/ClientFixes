...
import static l1j.server.server.model.skill.L1SkillId.REVENGE_TIME;
...
public class C_ActionUi extends ClientBasePacket {
	...
	private static final String[] textFilter = { "anal", "boner", "chink", "cock", "fag", "faggot", "fagg", "fagot", "fuck", "fuk",
			"fucker", "incest", "insest", "kkk", "nigger", "nigga", "nigg", "nignog", "whore", "wigger" };
	...
	private static final int EVENT_NOTICE = 143;
	...
	private static final int PRIVATE_STORE = 817;
	private static final int SERVER_VERSION = 820;
	...
	private static final int RANKING_STATUS_WINDOW = 1021;
	private static final int REVENGE_BOARD = 1051;
	...
	private static final int PET_NAME = 2001;
	private static final int PET_STAT = 2003;
	private static final int PET_COMMAND = 2004;
	private static final int PetSkillLevel = 2007;
	private static final int PET_SKILL = 2008;
	...
	public C_ActionUi(byte abyte0[], LineageClient client) throws IOException {
			...
			try {
				...
				for (byte[] b : arrb) {
					...
					default:
						...
						System.out.println("[Undefined packet related to stats] op: " + i);
						break;
					}
				}
		...
		case CLAN_JOIN: {
			...
			try {
				...
				case 0:
					if (crown != null) {
						C_Attr.joinPledge(crown, pc, clan);
					...
					if (WarTimeController.getInstance().isNowWar(castle_id)) { // 전쟁 시간내
						...
						for (L1PcInstance element : clanMember) {
							if (L1CastleLocation.checkInWarArea(castle_id, element)) {
								...
								L1Teleport.teleport(element, locx, locy, mapid, element.getMoveState().getHeading(), true);
							}
						}
		...
		case ATTENDANCE_CHECK_COMPLETE: {
			try {
				if ((pc == null) || pc.hasSkillEffect(L1SkillId.ATTENDANCE_CHECK_DELAY)) {
					return;
				}

				pc.getSkillEffectTimerSet().setSkillEffect(L1SkillId.ATTENDANCE_CHECK_DELAY, 3000);
				readH();
				readC();
				...
				if (check_one || check_two) {
					System.out.println("★☆★ Repeater (attendance check) suspicious user : [" + account.getName() + "] ★☆★");
					return;
				}
		...
		case RANKING_STATUS_WINDOW: {
			...
			if (classRank == null && rank == null) {
				...
				pc.sendPackets(new S_NewCreateItem(S_NewCreateItem.상태창랭킹, classRank, rank, true));
			} else if (타입 == 6 && UserRankingController.rankingUpdate >= 시간) {
				pc.sendPackets(new S_NewCreateItem(S_NewCreateItem.상태창랭킹, classRank, rank, true));
			} else {
		...
		case START_PLAY_SUPPORT:
			...
			if (Config.PSS_CONTROLLER) {
				if (pc.isInvisble()) {
					pc.sendPackets(new S_SystemMessage("PSS cannot be used while you are invisible."));
					return;
				}
				if (SupportMapTable.getInstance().isSupportMap(pc.getMapId())) {
					...
					if (SM != null) {
						if (pc.getLevel() > 1 && pc.getLevel() <= Config.PSS_LEVEL_CAP) {
					...
					pc.sendPackets(new S_SystemMessage("The PSS is not avialable right now."), true);
				}
				} else {
				pc.sendPackets(new S_SystemMessage("The PSS has been disabled on this map."), true);
				}
			}
			break;
		...
		case WorldMapTeleport: {
			...
			String code = new String(actionName, "MS949");
		...
		case PRIVATE_STORE: {
			...
			if (shoptype == 0) { // 개시
				...
				try {
					...
					pc.sendPackets(new S_PacketBox(S_PacketBox.NUM_STORE_OPEN, pc.getNetConnection().getAccount().Shop_open_count), true);
		...
		case DOLL_RESULT: {
			...
			int chance2 = ((total * Config.DOLLPROBABILITY2)); // 인형합성 확률 설정
			int chance3 = ((total * Config.DOLLPROBABILITY3)); // 인형합성 확률 설정
			int chance4 = ((total * Config.DOLLPROBABILITY4)); // 인형합성 확률 설정
			int chance5 = ((total * Config.DOLLPROBABILITY5)); // 인형합성 확률 설정
			switch (step) {
			case 1:
		...
		case NP_LOGIN_REQ: {
			try {
				...
				String auth_token = new String(auth_token_byte, "MS949");
				...
				if (auth_token.toUpperCase().contains("SHOTGUN")) {
					// 두현팩 동봉 접속기 로그인 구현
					...
					//wli.setLanguage(Integer.parseInt(auth_token.split("\\|")[2]));

				//System.out.println("We've Made it to Step 1");
				} else {
					String _user = auth_token.split("\\s+")[0];
					String _password = auth_token.split("\\s+")[1];
					System.out.println("user: '" + _user + "'");
					System.out.println("password: '" + _password + "'");

					wli.setAccount(auth_token.split("\\s+")[0]);
					wli.setPassword(auth_token.split("\\s+")[1]);
					wli.setLanguage(0);
				}

				// 0(USA) 1(korea) 2(japan) 3(china)
		...
		case REVENGE_BOARD: {
			...
			if (pc.getSkillEffectTimerSet().hasSkillEffect(REVENGE_TIME)) {
		...
		case RevengeChase: {
			...
			if (target != null && target instanceof L1PcInstance) {
				...
				if (revenge.RemainCounter(pc.getId(), Targetpc.getId()) > 0) {
					...
					pc.getSkillEffectTimerSet().setSkillEffect(REVENGE_TIME, 600 * 1000);
		...
		case RevengeChase2: {
			...
			if (target != null && target instanceof L1PcInstance && pc.getSkillEffectTimerSet().hasSkillEffect(REVENGE_TIME)) {
				L1PcInstance Targetpc = (L1PcInstance) target;
		...
		case BMTYPE_DEL_CHECK_REQ: {
			...
			try {
				...
				String name = new String(name_byte, "MS949");
		...
		case EVENT_NOTICE: {
			...
			if (pc.getMapId() == 99 || (pc.getMapId() >= 2236 && pc.getMapId() <= 2237) || !pc.getMap().isEscapable()) {
		...
		case CRAFT_OK: {// 0x3a
			try {
				...
				HashMap<L1ItemInstance, Integer> deleteItemIntArrayList = new HashMap<>(); // 아이템,삭제할수
					...
					for (int itemId : bigSuccessItem) {
						if (itemId == item.getItemId()) {
							if (_Random.nextInt(100) < 100) { // 10% 확률로 대성공아이템.
								...
								L1World.getInstance().broadcastPacketToAll(new S_ACTION_UI(S_ACTION_UI.LIMAWORLDMESSAGE, 3599, createItem), true);
	...
	private boolean 인첸트지급(L1PcInstance pc, int item_id, int count, int EnchantLevel, int AttrEnchantLevel) {
		...
		if (item != null) {
			...
			if (pc.getInventory().checkAddItem(item, count) == L1Inventory.OK) {
				...
				pc.sendPackets(new S_ServerMessage(82)); //cannot carry anymore because my weight gauge is running low or my inventory is full.
				return false;
			}

			// pc.sendPackets(new S_SystemMessage("Item creation was successful."));
			pc.sendPackets(new S_ServerMessage(143, item.getLogName())); // You have obtained %0.
	...
	private boolean readCraftInfo(L1PcInstance pc) {
		materialDescIdList = new HashMap<>();

		readP(3); // dummy //including total size
		...
		if (GMCommands.제작체크) {
			System.out.println("Production ID - craftId : " + craftId);
		}
		...
		if (GMCommands.제작체크) {
			System.out.println("Production number - createItemCount : " + createItemCount);
		}
		...
		while (isRead(1)) {
			...
			if (GMCommands.제작체크) {
				System.out.println("Crafting Materials - descId : " + descId + " / enchant : " + enchant);
			}
	...
	private void craftErrMsg(L1PcInstance pc, int craftId, String msg) {
		System.out.println("Production failure: " + msg + " / craftId : " + craftId + " / user : " + pc.getName());
	}
	...
	private void resetAttendanceTime(Account account, L1PcInstance pc) {
		...
		if (cal.get(Calendar.HOUR_OF_DAY) == 0 && cal.get(Calendar.MINUTE) == 0) {
			...
			pc.sendPackets(new S_ACTION_UI(S_ACTION_UI.ATTENDANCE_COMPLETE, account, pc.PCROOM_BUFF));
		}
	}
	...
	private void getAttendancePCRoom(L1PcInstance pc, Account account) {
		if (!pc.PCROOM_BUFF || account.isAttendancePcHome())
			return;
	...
	private class AttendanceGiveItem implements Runnable {
		...
		public void run() {
			try {
				...
				l1skilluse.handleCommands(_pc, L1SkillId.ATTENDANCE_CHECK, _pc.getId(), _pc.getX(), _pc.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private void ChatWhisper(L1PcInstance whisperFrom, int chatType, int chatcount, byte[] chatdata, String text, String targetName) {
		try {
			...
			//LogTable.logWhisper(whisperFrom.getName(), whisperTo.getName(), text);
			//LinAllManager.getInstance().WhisperChatAppend(whisperFrom.getName(), whisperTo.getName(), text);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			clear();
	...
	private void Chat(L1PcInstance pc, int chatType, int chatcount, byte[] chatdata, String chatText, LineageClient clientthread) {
		//System.out.println("In c_actuionui chat with chattype " + chatType + " and chatText: " + chatText);

		try {
			if (pc.CHARACTER_NAME_CHANGE) {
				try {
					String chaName = chatText;
					if (pc.getClanid() > 0) {
						pc.sendPackets(new S_SystemMessage("혈맹탈퇴후 캐릭명을 변경할수 있습니다."));
						pc.CHARACTER_NAME_CHANGE = false;
						return;
					}
			...
			if (pc.isDeathMatch() && !pc.isGhost() && !pc.isGm()) {
				...
				pc.sendPackets(sm); // unable to chat
				sm = null;
				return;
			}
			...
			switch (chatType) {
			case 0: {
				//System.out.println("In c_actuionui: case 0");

				if (pc.isGhost() && !(pc.isGm() || pc.isMonitor())) {
					return;
				}

				//System.out.println("In c_actuionui: 1");

				if (chatText.startsWith(".time")) {
					StringBuilder sb = null;
					sb = new StringBuilder();
					TimeZone kst = TimeZone.getTimeZone("GMT-8");
					Calendar cal = Calendar.getInstance(kst);

					S_Translate translate = new S_Translate();
					int language = pc.getLanguage();

					sb.append("[Server Time]" + cal.get(Calendar.YEAR) + translate.Text(language, "year ") + (cal.get(Calendar.MONTH) + 1) + translate.Text(language, "month ") + cal.get(Calendar.DATE) + translate.Text(language, "day ")
				...
				//System.out.println("In c_actuionui: 2");
				//System.out.println("In c_actuionui:getaccesslevel: " + pc.getAccessLevel());
				//System.out.println("In c_actuionui:Config.GMCODE: " + Config.GMCODE);

				// GM커멘드Config.GMCODE
				if (chatText.startsWith(".") && (pc.getAccessLevel() == Config.GMCODE || pc.getAccessLevel() == 7777)) {
					//System.out.println("In c_actuionui: 2.1");
					String cmd = chatText.substring(1);
					GMCommands.getInstance().handleCommands(pc, cmd);
					return;
				}

				//System.out.println("In c_actuionui: 3");
				...
				//System.out.println("In c_actuionui: 4");

				Gamble(pc, chatText);
				if (chatText.startsWith(".") || chatText.startsWith("-")) { // 유저코멘트
					String cmd = chatText.substring(1);
					if (cmd == null) {
						return;
					}
				...
				//System.out.println("In c_actuionui: 5");
				...
				//System.out.println("In c_actuionui: 6");
				...
				//System.out.println("In c_actuionui: 7");
			...
			case 4: {
				if (pc.getClanid() != 0) { // 크란 소속중
					...
					//LinAllManager.getInstance().pledgeChatAppend(pc.getClanname(), pc.getName(), chatText);
			...
			case 17:
				if (pc.getClanid() != 0) { // 혈맹 소속중
					...
					if (clan != null && (pc.isCrown() && pc.getId() == clan.getLeaderId())) {
						...
						//LinAllManager.getInstance().pledgeChatAppend(pc.getClan().getClanName(), pc.getName(), chatText);
						for (L1PcInstance listner : clan.getOnlineClanMember()) {
							if (!listner.getExcludingList().contains(pc.getName())) {
