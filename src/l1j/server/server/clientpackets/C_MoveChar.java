...
public class C_MoveChar extends ClientBasePacket {
	...
	private static final S_SystemMessage sm = new S_SystemMessage("Please rent a room or hall first.");
	...
	public boolean ckok(int x, int y, int m, int h) {
		...
		if (m == 4) {
			if ((x == 33636 && y == 32673) || (x == 33627 && y == 32682)) {
				return true;
			}
			if (x == 33628 && y == 32674) {
				return true;
		...
		try {
			...
			if (Config.뚫어방어사용 && pc.getMapId() != 2100 && pc.getMapId() != 2699) {
				if (!World.isThroughObject(pc.getX(), pc.getY(), pc.getMapId(),
						heading)) {
					if (GMCommands.GUILTY_CHECK) {
						System.out.println("Gilting: Drill through 1 name : " + pc.getName());
					}
					pc.getMoveState().setHeading(heading);
					pc.sendPackets(new S_PacketBox(S_PacketBox.USER_BACKSTAFF, pc));
					Broadcaster.broadcastPacket(pc, new S_ChangeHeading(pc));
					return;
				}
				// If the tile is not immovable and there are none, it is set to moveable
				if (World.moveTheDoor(pc.getX(), pc.getY(), pc.getMapId(), heading)) {
					if (GMCommands.GUILTY_CHECK) {
						System.out.println("Gilting: Drill through 2 name: " + pc.getName());
					}
					pc.getMoveState().setHeading(heading);
					pc.sendPackets(new S_PacketBox(S_PacketBox.USER_BACKSTAFF, pc));
					Broadcaster.broadcastPacket(pc, new S_ChangeHeading(pc));
			...
			if (Config.유체이탈방어사용) {
				if (pc.tempx != 0 && pc.tempy != 0) {
					int calcx = pc.tempx - locx;
					int calcy = pc.tempy - locy;
					// System.out.println(calcx+" "+calcy);
					if (Math.abs(calcx) > 1 || Math.abs(calcy) > 1) {
						if (GMCommands.GUILTY_CHECK) {
							System.out.println("Gilting: Check out fluid name: "+ pc.getName());
						}
						pc.getMoveState().setHeading(heading);
						pc.sendPackets(new S_PacketBox(S_PacketBox.USER_BACKSTAFF, pc));
						Broadcaster
			...
			if (pc.getMapId() != 2100 && pc.getMapId() != 2699) {
				...
				if (ck) {
					if (GMCommands.GUILTY_CHECK) {
						System.out.println("Gilting: overlap check name: " + pc.getName());
					}
					// pc.updateObject();
					pc.getMoveState().setHeading(heading);
					pc.sendPackets(new S_PacketBox(S_PacketBox.USER_BACKSTAFF, pc));
					...
					try {
						...
						if (clan != null)
							clanWarehouse = WarehouseManager.getInstance()
									.getPledgeStorage(clan.getClanName());
			...
			if (pc.getLevel() >= 99) { // 지정 레벨
				...
				if (pc.getInventory().getSize() >= 180) {
					pc.sendPackets(
							new S_SystemMessage("Please empty your inventory before entering."), true);
			...
			if (Config.CHECK_MOVE_INTERVAL) {
				if (result == AcceleratorChecker.R_DISCONNECTED) {
					if (GMCommands.GUILTY_CHECK) {
						System.out.println("Suspicion Flag: Detected speed hacking on player: " + pc.getName());
						}
				pc.getMoveState().setHeading(heading);
				pc.sendPackets(new S_PacketBox(S_PacketBox.USER_BACKSTAFF, pc));
				Broadcaster.broadcastPacket(pc, new S_ChangeHeading(pc));
		  ...
		  /*  *//** SPR체크 **//*
			...
			// Escape route due to stomach upset
			if (pc.getMapId() == 5 || pc.getMapId() == 6 || pc.getMapId() == 83
				...
				if ((nowtime >= 30 * 360 + 20 && nowtime < 40 * 360)
					...
					if (pc.getMapId() == 5) { // Malseom -> Mainland
						pc.dx = 32561;
						pc.dy = 32727;
						pc.dh = 4;
						pc.dm = 4;
						timetel = true;
					} else if (pc.getMapId() == 84) { // Forget Island -> Mainland
						pc.dx = 33434;
						pc.dy = 33480;
						pc.dh = 4;
						pc.dm = 4;
						timetel = true;
					} else if (pc.getMapId() == 447) { // Hidden Dock -> Pirate Island
						pc.dx = 32297;
						pc.dy = 33074;
						pc.dh = 4;
						pc.dm = 440;
					...
					if (pc.getMapId() == 6) { // Mainland -> Malseom
						pc.dx = 32632;
						pc.dy = 32965;
						pc.dh = 4;
						pc.dm = 0;
						timetel = true;
					} else if (pc.getMapId() == 83) { // Mainland -> Forget Island
						pc.dx = 32621;
						pc.dy = 33014;
						pc.dh = 4;
						pc.dm = 1700;
						timetel = true;
					} else if (pc.getMapId() == 446) { // Pirate Island -> Hidden Pier
						pc.dx = 32751;
						pc.dy = 32859;
						pc.dh = 4;
						pc.dm = 445;
				...
				if (key != null) {
					...
					for (L1ItemInstance element : key) {
						if (element.getEndTime().getTime() > System.currentTimeMillis()) {
							if (((locx == 32600 && locy == 32930 && pc.getMapId() == 0) && (((short) element.getKey() >= 16384
									&& (short) element.getKey() <= 16684) || ((short) element.getKey() >= 16896 && (short) element.getKey() <= 17196)))
									|| ((locx == 33986 && locy == 33312 && pc.getMapId() == 4) && (((short) element.getKey() >= 19456 && (short) element.getKey() <= 19756)
											|| ((short) element.getKey() >= 19968 && (short) element.getKey() <= 20268)))
									|| ((locx == 33437 && locy == 32789 && pc.getMapId() == 4) && (((short) element.getKey() >= 18432 && (short) element
											.getKey() <= 18732) || ((short) element.getKey() >= 18944 && (short) element.getKey() <= 19244)))
									|| ((locx == 34066 && locy == 32254 && pc.getMapId() == 4) && (((short) element
											.getKey() >= 23552 && (short) element.getKey() <= 23852) || ((short) element
											.getKey() >= 24064 && (short) element.getKey() <= 24364)))
									|| ((locx == 32632 && locy == 32761 && pc.getMapId() == 4) && (((short) element
											.getKey() >= 17408 && (short) element
											.getKey() <= 17708) || ((short) element.getKey() >= 17920 && (short) element.getKey() <= 18220)))
									|| ((locx == 33605 && locy == 33275 && pc.getMapId() == 4) && (((short) element
											.getKey() >= 22528 && (short) element
											.getKey() <= 22828) || ((short) element.getKey() >= 23040 && (short) element.getKey() <= 23340)))
									|| ((locx == 33116 && locy == 33379 && pc.getMapId() == 4) && (((short) element.getKey() >= 21504 && (short) element
											.getKey() <= 21804) || ((short) element.getKey() >= 22016 && (short) element.getKey() <= 22316)))
									|| ((locx == 32628 && locy == 33167 && pc.getMapId() == 4) && (((short) element.getKey() >= 20480 && (short) element
											.getKey() <= 20780) || ((short) element.getKey() >= 20992 && (short) element.getKey() <= 21292)))
									|| ((locx == 32451 && locy == 33046 && pc.getMapId() == 440) && (((short) element.getKey() >= 24576 && (short) element
											.getKey() <= 24876) || ((short) element.getKey() >= 25088 && (short) element.getKey() <= 25388)))) {
					...
					if (keymap != 0) {
						...
						if (key.length == 0)
							...
							pc.sendPackets(new S_SystemMessage("You own the key to another inn."), true);
						key = null;
					}
				}
			} else if (Dungeon.getInstance().dg(locx, locy,
					pc.getMap().getId(), pc)) { // If teleported to dungeon
				pc.setTelType(1);
				pc.sendPackets(new S_SabuTell(pc), true);
				return;
			}
			...
			if (pc.getMapId() >= 10000 && pc.getMapId() <= 10005) {
				if ((pc.getX() == 32716 || pc.getX() == 32717)
						&& pc.getY() == 32912) { // Cave Entrance -> Rare Entrance
					pc.dx = 32735;
					pc.dy = 32847;
					pc.dm = pc.getMapId();
					pc.dh = 4;
					...
					return;
						...
						&& pc.getY() == 32846) { // Rare Entrance -> Cave Entrance
					pc.dx = 32716;
					pc.dy = 32914;
					pc.dm = pc.getMapId();
					pc.dh = 4;
			...
			if (DungeonRandom.getInstance().dg(locx, locy, pc.getMap().getId(), pc)) { // Teleport point with random teleport destination
				return;
			}
			...
			if (castleid != 0) {
				...
				if (pc.war_zone) {
					...
					if (pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.LORD_BUFF)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(L1SkillId.LORD_BUFF);
			...
			if(pc.getInventory().checkItem(5001130, 1) && (pc.getMapId()== 15410 || pc.getMapId()== 15420 || pc.getMapId()== 15430 || pc.getMapId()== 154140
					...
					|| pc.getMapId()== 475)) {
				     if(!pc.순간이동지배) {
				    	pc.순간이동지배 = true;
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.순간이동지배, true, -1));
				     }
		    	...
		    	if(pc.순간이동지배) {
			    	pc.순간이동지배 = false;
					pc.sendPackets(new S_NewSkillIcons(L1SkillId.순간이동지배, false, -1));
		    	}
			}
