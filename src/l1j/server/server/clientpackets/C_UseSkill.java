...
public class C_UseSkill extends ClientBasePacket {
	public C_UseSkill(byte abyte0[], LineageClient client) throws Exception {
		...
		try {
			...
			if (pc.isTeleport() || pc.isDead() || pc.isGhost() || !pc.isSkillMastery(skillId)) {
				return;
			}
			...
			if (GMCommands.트리플포우스핵 && (skillId == L1SkillId.TRIPLE_ARROW || skillId == L1SkillId.FOE_SLAYER)) {
			} else {
				int result;
			...
			if (!pc.AURACIA) {
				if (skillId == AWAKEN_ANTHARAS
						...
						|| skillId == AWAKEN_VALAKAS || skillId == AWAKEN_LINDVIOR) {
					if (pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_ANTHARAS)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_ANTHARAS);
						// return;
					} else if (pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_FAFURION)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_FAFURION);
						// return;
					} else if (pc.getSkillEffectTimerSet().hasSkillEffect(	AWAKEN_VALAKAS)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_VALAKAS);
					} else if (pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_LINDVIOR)) {
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_LINDVIOR);
				...
				if (skillId == AWAKEN_ANTHARAS
						|| skillId == AWAKEN_FAFURION
						|| skillId == AWAKEN_VALAKAS || skillId == AWAKEN_LINDVIOR) {
					if ((pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_ANTHARAS) && pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_FAFURION))
							||(pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_ANTHARAS) && pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_VALAKAS))
							||(pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_ANTHARAS) && pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_LINDVIOR))
							||(pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_FAFURION) && pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_VALAKAS))
							||(pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_FAFURION) && pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_LINDVIOR))
							||(pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_VALAKAS) && pc.getSkillEffectTimerSet().hasSkillEffect(AWAKEN_LINDVIOR))) {
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_ANTHARAS);
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_FAFURION);
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_VALAKAS);
						pc.getSkillEffectTimerSet().removeSkillEffect(AWAKEN_LINDVIOR);
			...
			try {
				...
				if (target2 != null && SkillsTable.getInstance().getTemplate(skillId).getTarget().equalsIgnoreCase("attack")) {
					if (skillId != L1SkillId.FIRE_WALL) {
						int calcx = pc.getX() - target2.getLocation().getX();
						int calcy = pc.getY() 	- target2.getLocation().getY();
