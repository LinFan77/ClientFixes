	...
	public C_Report(byte abyte0[], LineageClient clientthread) throws Exception {
		...
		try {
			...
			case 페어리: {
				...
				// -----------------52 From now on, only Fairy Growth Potion------------//
				...
				case 0x3c: {
					...
					count = 2;
					level = 50;
					L1BookMark.addBookmark(pc, 32609, 32771, 4, "\\d0\\fRLevel 50: Fishing Grounds");
				}
					break; // remember level 50 hunting ground
				case 0x3d: {
					itemid = 140100;
					count = 2;
					level = 51;
					L1BookMark.addBookmark(pc, 33443, 32829, 4, "\\d0\\fRLevel 51: Earth Dragon's Dungeon");
				}
					break; // Level 51 hunting ground memory
				case 0x3e: {
					itemid = 140100;
					count = 2;
					level = 52;
					L1BookMark.addBookmark(pc, 34068, 32271, 4, "\\d0\\fRLevel 52: Ice Crystal Cave");
				}
					break; // Level 52 hunting ground memory
				case 0x3f: {
					itemid = 140100;
					count = 2;
					level = 53;
					L1BookMark.addBookmark(pc, 32595, 33164, 4, "\\d0\\fULevel 53: Depo Rouge (Siege)");
				}
					break;// 53레벨 사냥터기억
				case 0x40: {
					itemid = 140100;
					count = 2;
					level = 54;
					L1BookMark.addBookmark(pc, 32578, 32924, 0, "\\d0\\fULevel 54: Yurie (Indungeon)");
				}
					break;// 54레벨 사냥터기억
				case 0x41: {
					itemid = 140100;
					count = 2;
					level = 55;
					L1BookMark.addBookmark(pc, 32884, 32782, 4, "\\d0\\fULevel 55: Level 55 Quest");
					L1BookMark.addBookmark(pc, 33617, 33247, 4, "\\d0\\fYLevel 55: Water Dragon's Dungeon");
				}
					break;// 55레벨 사냥터기억
				case 0x42: {
					itemid = 140100;
					count = 2;
					level = 56;
					L1BookMark.addBookmark(pc, 33427, 32827, 4, "\\d0\\fYLevel 56: Giran Prison");
				}
					break; // Level 56 hunting ground memory
				case 0x43: {
					itemid = 140100;
					count = 2;
					level = 57;
					L1BookMark.addBookmark(pc, 34051, 32265, 4, "\\d0\\fYLevel 57: Ivory Tower");
				}
					break; // Level 57 hunting ground memory
				case 0x44: {
					itemid = 140100;
					count = 2;
					level = 58;
					L1BookMark.addBookmark(pc, 32719, 32929, 4, "\\d0\\fYLevel 58: Gludio Dungeon");
				}
					break; // Level 58 hunting ground memory
				case 0x45: {
					itemid = 140100;
					count = 2;
					level = 59;
					L1BookMark.addBookmark(pc, 32476, 32857, 0, "\\d0\\fULevel 59: Talking Island Dungeon");
				}
					break; // Level 59 hunting ground memory
				/*
				...
				case 0x46: {
					...
					break; // level 45 guide
				case 0x47: {
					level = 46;
				}
					break; // Level 46 Guide
				case 0x53: {
					level = 47;
				}
					break; // Level 47 Guide
				case 0x48: {
					level = 48;
				}
					break;// Level 48 Guide
				case 0x55: {
					level = 49;
				}
					break;
				...
				case 0x59: {
					...
					break; // When to connect?
				case 0x56: {
				}
					break; // unknown number 1
				default:
					System.out.println("Fairy number not applied :[" + type2 + "]");
					break;
				}
				...
				break;
			case 케릭터비번생성: { // Create character password
				...
				break;
			case 케릭터비번변경: { // character password change
			...
			case 케릭터비번인증: { // Character password authentication
				String password = readS();
			...
			case show_clanMark: {
				...
				if ((clan == null) || !clan.getLeaderName().equalsIgnoreCase(pc.getName()))
					return;
				clan.setmarkon(on);
			...
			case dragon_menu:
				...
				switch (dragonType) {
				case 0: // 안타
					AntharasRaidSystem.getInstance().startRaid(pc);
			...
			case bookmark_save:
				...
				if (subType == 2) {
					...
					if (list.length > 0) {
						...
						FastTable<Integer> fs = new FastTable<>();
			...
			case 상점개설횟수:
				if (pc.getNetConnection() == null || pc.getNetConnection().getAccount() == null)
					return;
				pc.sendPackets(new S_PacketBox(S_PacketBox.NUM_STORE_OPEN, pc.getNetConnection().getAccount().Shop_open_count),
						true);
				break;
			default:
