...
public class L1BookMark {
	...
	public static void addBookmark(L1PcInstance pc, String s) {
		...
		if (pc.getBookMark(s) == null) {
			...
			pc.sendPackets(new S_SystemMessage("The same name already exists.")); // 같은 이름이벌써존재하고있습니다.
		}
	}
	...
	public static final String[] FAINT_MEMORY = { "\\fS[제작] \\fT룸티스스냅퍼",
	...
	// 32637 33327
	public static final String[] MYSTERIOUS_MEMORY = { "\\fV[Mystery] \\fWIce Lake Center",
			"\\fV[Mystery] \\fWIce Lake Outer", "\\fV[Mystery] \\fWIce cliff",
			"\\fV[Mystery] \\fWAden Dinghy", "\\fV[Mystery] \\fWHeine Guard Tower",
			"\\fV[Mystery] \\fWDragon Valley Cliff", "\\fV[Mystery] \\fWHidden Turtle Island",
			"\\fV[Mystery] \\fWIsland of Mind and Time" };
	public static final int[] mysteryX = { 34002, 33994, 34212, 34194, 33503, 33466, 33343, 32555 };
	public static final int[] mysteryY = { 32329, 32332, 32343, 33135, 33505, 32312, 33163, 32979 };

	public static synchronized boolean itemMemory(L1PcInstance pc, int type) {
		for (int i = 0; i < type; i++) {
			String name = "";
			if (type == 8) {
				name = MYSTERIOUS_MEMORY[i];
			} else {
				name = FAINT_MEMORY[i];
			}
			if (pc.getBookMark(name) != null) {
				pc.sendPackets(new S_SystemMessage(
						"Among the bookmarks to add, there is a duplicate name in the current window. > " + name), true);
				return false;
			}
		}
		for (int i = 0; i < type; i++) {
			...
			if (type == 8) {
				...
				bookmark.setLocX(mysteryX[i]);
				bookmark.setLocY(mysteryY[i]);
			} else {
				name = FAINT_MEMORY[i];
				bookmark.setName(name);
			...
			pc.addBookMark(bookmark);
			pc.sendPackets(new S_Bookmarks(name, bookmark.getLocX(), bookmark.getLocY(), bookmark.getMapId(), bookmark.getId()));
		}
		return true;
	}
	...
	public static synchronized void ItemaddBookmarkByTeleport(L1PcInstance pc, int itemid) {
		...
		FastTable<L1BookMark> list = new FastTable<>();
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
	...
	public static synchronized boolean ItemBookmarkLoad(L1PcInstance pc, int itemid) {
			...
			while (rs.next()) {
				try {
					...
					pc.sendPackets(new S_Bookmarks(bookmark.getName(), bookmark
							.getLocX(), bookmark.getLocY(), bookmark.getMapId(), bookmark.getId()));
				} catch (Exception e) {
				}
			}
		} catch (SQLException e) {
		...
		try {
			...
			_log.log(Level.SEVERE, "An error occurred while deleting the bookmark: ", e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
